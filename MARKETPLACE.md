...
### The three questions every risky commit raises

- **What could this change break?** CODEMD walks your real callgraph from the changed function outward — every direct caller and callee, ranked by how many places depend on it — instead of you guessing or re-reading the whole diff by hand.
- **Which existing tests actually cover that risk?** It looks up your own test suite against that same callgraph and tells you which files genuinely exercise the changed function — a real name/import/symbol match, not a keyword guess — so you know what to actually run before you commit.
- **What important risk remains untested, and what's the cheapest test to close that gap?** CODEMD flags any changed function with zero covering tests found, and — only on your click — has Claude write one test grounded in that function's real caller, then actually runs it for a pass/fail.
