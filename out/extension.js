 ...
 */
async function resolvePythonCallSites(folder, calleeSymbol, callerCompactNames) {
    const sites = new Map();
    ...
    const orderedCalls = Array.isArray(data?.ordered_calls) ? data.ordered_calls : [];
 ...
 */
async function findPythonCallersOf(folder, calleeSymbol, limit = 8) {
    const cgPath = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'python', 'python_callgraph.json');
    let data;
    try {
        data = JSON.parse(await fs.promises.readFile(cgPath, 'utf8'));
...
function serializeCallChains(chains) {
    return chains.map((c) => ({
...
class GraphsViewProvider {
    ...
    async generateCallPathTestForResult(result, requestId) {
        ...
        const callers = Array.isArray(result?.changeCard?.impactedFunctions)
            ...
            : [];
        const callSites = callers.length ? await resolvePythonCallSites(folder, symbol, callers) : new Map();
        let callerLine;
        if (!callers.length) {
    ...
    async writeGeneratedTest(folder, namePrefix, testSource) {
        const outDir = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'generated_tests');
        await fs.promises.mkdir(outDir, { recursive: true });
    ...
    async runFindTestsForResult(result, requestId) {
        ...
        try {
            ...
            this.post({
                ...
                matches: Array.isArray(data.matches) ? data.matches.slice(0, 5).map((m) => ({
                    file: String(m.file || ''),
    ...
    async computePythonTestRunPayload(message, folder) {
        ...
        let output = '';
        try {
            const covRun = await execCommandAsync(pythonPath, 
            ...
            ['-m', 'coverage', 'run', '--branch', `--data-file=${dataFile}`, '-m', 'pytest', target, '-v'], folder.uri.fsPath, env);
