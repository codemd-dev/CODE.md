...
class GraphsViewProvider {
    ...
    async setupMcpFromPanel() {
        ...
        if (folder) {
            ...
            const choice = await vscode.window.showInformationMessage(changed
                ...
                : 'CODE.md: MCP config is already up to date. Open a client terminal to connect/reconnect codemd.', ...(targets.includes('codex') ? ['Open Codex /mcp'] : []), ...(targets.includes('claude') ? ['Open Claude Code /mcp'] : []));
    ...
    async removeMcpFromPanel() {
        ...
        const choice = await vscode.window.showWarningMessage('CODE.md: remove CODE.md MCP setup from this workspace? This removes only CODE.md MCP entries from local config files. Other MCP servers are left unchanged.', { modal: true }, 'Remove MCP Setup');
    ...
    async cleanArtifactsFromPanel() {
        ...
        const choice = await vscode.window.showWarningMessage(`CODE.md: clean up ${ARTIFACT_OUTPUT_DIR}/ in this workspace? This deletes generated graphs, search indexes, logs, and MCP usage history. Source files and MCP config files are not changed.`, { modal: true }, `Clean up ${ARTIFACT_OUTPUT_DIR}`);
