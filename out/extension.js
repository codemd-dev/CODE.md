...
function mcpTargetsLabel(targets) {
...
function assertWorkspaceWriteAllowed(uri, operation, kind) {
    ...
    if (!workspaceWriteIsAllowed(normalized, kind)) {
        throw new Error(`Blocked ${operation} to workspace file "${relPath}". CODEMD only writes ${ARTIFACT_OUTPUT_DIR}/, ${WORKSPACE_MCP_CONFIG_FILE}, and ${WORKSPACE_CLAUDE_SETTINGS_FILE}.`);
    }
}
...
async function safeWorkspaceDeleteDirectory(uri) {
...
async function removeClaudeProjectMcp(folder) {
    const configUri = vscode.Uri.joinPath(folder.uri, WORKSPACE_MCP_CONFIG_FILE);
    if (!fs.existsSync(configUri.fsPath)) {
        return false;
    }
...
async function removeClaudeMcpApproval(folder) {
    const settingsUri = claudeLocalSettingsUri(folder);
    if (!fs.existsSync(settingsUri.fsPath)) {
        return false;
    }
    ...
    const nextEnabled = settings.enabledMcpjsonServers
        ...
        .filter((name) => !MCP_SERVER_NAMES.includes(name));
...
async function requestClaudeMcpApproval(folder) {
    ...
    const choice = await vscode.window.showInformationMessage(`CODEMD: allow the "${MCP_SERVER_NAME}" (${MCP_SERVER_LABEL}) server to run automatically with Claude Code in "${folder.name}"? ` +
...
async function removeCodexProjectMcp(folder) {
    const configUri = vscode.Uri.joinPath(folder.uri, ...WORKSPACE_CODEX_CONFIG_FILE.split('/'));
    if (!fs.existsSync(configUri.fsPath)) {
        return false;
    }
...
async function removeCodexUserMcp() {
    const configPath = codexUserConfigPath();
    if (!fs.existsSync(configPath)) {
        return { changed: false, path: configPath };
    }
...
async function setupProjectMcpConfigs(context, options) {
    ...
    const targets = new Set(options.targets && options.targets.length ? options.targets : ['claude', 'codex']);
...
async function removeProjectMcpConfigs(targetsInput) {
    const folders = vscode.workspace.workspaceFolders || [];
    if (folders.length === 0) {
    ...
    const changedFolders = [];
    const changedCodexUserConfigs = new Set();
    const targets = new Set(targetsInput && targetsInput.length ? targetsInput : ['claude', 'codex']);
    for (const folder of folders) {
        try {
...
const mcpClientTerminals = new Map();
...
function parseClaudeMaxTurnsError(stdout, maxTurns) {
    const envelope = extractClaudeResultEnvelope(stdout);
...
function formatReportEntryMarkdown(message) {
    ...
    if (message.type === 'fixTestResult') {
        ...
        if (message.ok === false) {
            ...
            if (message.defectResolvedKind === 'code') {
                block += `- ✓ Code bug fixed\n`;
            }
            else if (message.defectResolvedKind === 'test') {
                block += `- ✓ Test bug fixed — the generated test's own assumptions were wrong, not the code under test\n`;
            }
            if (message.summary) {
...
async function openMcpClientTerminal(client, folder) {
    ...
    if (!commandPath) {
        ...
        vscode.window.showWarningMessage(`CODEMD: MCP config is ready, but ${displayName} CLI "${client}" was not found by VS Code. Install ${displayName}, add "${client}" to PATH, or restart VS Code after updating PATH.`);
        return;
    }
 ...
 * typing '/mcp' into one.
 */
async function sendPromptToUsersClaudeTerminal(folder, prompt) {
...
function warnIfReactivatedInStaleWindow(context) {
    ...
    vscode.window
        .showInformationMessage('CODEMD was reinstalled/updated while this window was open. Reload the window to finish activating the graph view.', 'Reload Now')
        .then((choice) => {
...
async function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('codemdGraphs.setupMcp', () => provider.setupMcpFromPanel()));
    context.subscriptions.push(vscode.commands.registerCommand('codemdGraphs.removeMcp', () => provider.removeMcpFromPanel()));
    context.subscriptions.push(vscode.commands.registerCommand('codemdGraphs.cleanArtifacts', () => provider.cleanArtifactsFromPanel()));
...
function runCommand(cmd, args, cwd, onOutput, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        proc.on('exit', (code) => {
            ...
            if (timedOut) {
                reject(new Error(`"${cmd} ${args.join(' ')}" timed out after ${Math.round(options.timeoutMs / 1000)}s and was killed. Check the "CODEMD" output channel for what it was doing.`));
                return;
            }
            resolve(code ?? 1);
        });
...
async function ensureManagedVenvUnlocked(context, backendDir, onStatus) {
    ...
    if (requirementsHash && existingHash !== requirementsHash) {
        ...
        if (code !== 0) {
            throw new Error(`Failed to install backend dependencies (exit code ${code}). Check the "CODEMD" output channel. ` +
...
async function ensureServerRunningExclusive(context, onStatus, quiet) {
    ...
    if (!fs.existsSync(mainPyPath)) {
        throw new Error(`Could not find main.py in "${backendDir}". Set codemdGraphs.backendDir to the folder containing the CODEMD analyzer backend.`);
    }
    ...
    return new Promise((resolve, reject) => {
        const check = async () => {
            if (proc.exitCode !== null) {
                reject(new Error('The local FastAPI server exited before it became ready. Check the "CODEMD" output channel, ' +
...
function writeTestStatusCache(folder, cache) {
    try {
...
function validCachedGeneratedKinds(folder, cache, targetFile, symbol) {
    const out = {};
...
function hasValidCachedPassingRun(folder, cache, targetFile, symbol) {
    const entry = cache.entries[testStatusCacheKey(targetFile, symbol)];
    if (!entry?.runs) {
        return null;
    }
    ...
    for (const run of Object.values(entry.runs)) {
        ...
        if (run.targetHash !== currentTargetHash) {
            continue;
        }
        const currentTestHash = hashFileContents(resolveWorkspacePath(folder, run.testFile));
        if (currentTestHash && currentTestHash === run.testHash) {
            return run;
        }
    }
    return null;
...
function validCachedRunForKind(folder, cache, targetFile, symbol, kind) {
...
const pythonTestRunWaiters = [];
function acquirePythonTestRunSlot() {
...
function releasePythonTestRunSlot() {
...
function execCommandAsync(cmd, args, cwd, env, timeoutMs = 90000) {
...
function execCommandOnceAsync(cmd, args, cwd, env, timeoutMs, useShell) {
    return new Promise((resolve) => {
        let proc;
        try {
 ...
 */
let claudeCliQueueTail = Promise.resolve();
function runClaudeCliExclusive(task) {
...
function pythonModuleNameFor(relPath) {
...
function testLanguageFor(file) {
    const lower = file.toLowerCase();
    if (lower.endsWith('.py')) {
        return 'python';
    }
    if (/\.(jsx?|tsx?|mjs|cjs)$/.test(lower)) {
        return 'javascript';
    }
...
// repo root.
function jsTestRunnerSearchDirs(folder, targetFile) {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        // forever.
        if (parent === resolved || (!resolved.startsWith(rootResolved) && dirs.length > 1)) {
            break;
        }
        dir = parent;
    }
    if (!dirs.includes(rootResolved)) {
        dirs.push(rootResolved);
    }
    return dirs;
}
function detectJsTestRunner(folder, targetFile) {
...
// majority of runs.
const DETERMINISTIC_JS_RUNNERS = {
 ...
 * instead of guessing.
 */
function buildDeterministicJsRunCommand(folder, detected, testFile) {
    if (!detected.found || !detected.dir || !detected.runner) {
        return null;
    }
 ...
 */
function selectJsTestRunner(folder) {
    let pkg = {};
    try {
 ...
 * different container.
 */
function callgraphArtifactFor(language) {
    if (language === 'python') {
...
function topLevelModuleFor(file, language) {
    if (language === 'python') {
 ...
 */
function jsImportPathFor(folder, file) {
 ...
 */
function javaImportHint(folder, file, tail) {
    let packageName = '';
    try {
 ...
 */
function csharpImportHint(folder, file, tail) {
    let namespaceName = '';
    try {
        const source = fs.readFileSync(path.join(folder.uri.fsPath, file), 'utf8');
 ...
 */
function goImportHint(folder, file, tail) {
 ...
 */
function rustImportHint(folder, file, tail) {
    const absFile = path.join(folder.uri.fsPath, file);
 ...
 */
function ensureJsSymbolExported(folder, file, tail) {
...
const JS_RUNNER_DISPLAY_NAMES = { vitest: 'Vitest', jest: 'Jest', mocha: 'Mocha', ava: 'AVA' };
/**
 ...
 */
function testFrameworkNounFor(language, jsRunnerHint) {
    if (language === 'python') {
        return 'pytest';
    }
    if (language === 'java') {
        return 'JUnit 5 (Jupiter) — use `org.junit.jupiter.api.Test`/`Assertions`, not JUnit 4\'s `org.junit.Test`';
    }
    if (language === 'csharp') {
        return "this project's actual .NET test framework (xUnit's `[Fact]`, NUnit's `[Test]`, or MSTest's `[TestMethod]` — check the .csproj's PackageReference entries and existing test files to see which one it uses; default to xUnit with `[Fact]`/`Assert` if none is apparent)";
    }
    if (language === 'go') {
        return 'go test — a `func TestXxx(t *testing.T)` function, using table-driven subtests (`t.Run`) for multiple cases rather than separate top-level functions, matching Go\'s own convention';
    }
    if (language === 'rust') {
        return 'cargo test — a `#[test] fn` function (inside a `#[cfg(test)] mod tests` block if this project follows that convention, check existing tests)';
    }
    if (language === 'kotlin') {
        return 'JUnit 5 (Jupiter) — use `org.junit.jupiter.api.Test`/`Assertions`, exactly as for this project\'s Java code';
    }
 ...
 */
function javaTestClassNameFor(namePrefix) {
 ...
 */
function javaClassNameInstruction(language, namePrefix) {
    if (language !== 'java') {
        return '';
    }
 ...
 */
function generatedTestFileNameFor(namePrefix, language, ext) {
    if (language === 'java') {
 ...
 */
function expectedGeneratedTestPath(folder, namePrefix, language, ext) {
...
function generatorKindFromTestFileName(testFile) {
...
function generatedTestExtFor(file, language) {
    if (language === 'python') {
        return '.py';
    }
    if (language === 'java') {
 ...
 */
async function resolveCallSites(folder, language, calleeSymbol, callerCompactNames) {
    const sites = new Map();
    ...
    const orderedCalls = Array.isArray(data?.[key]) ? data[key] : [];
    ...
    for (const call of orderedCalls) {
        ...
        if (!wanted.has(callerCompact) || sites.has(callerCompact)) {
            continue;
        }
        const callee = String(call?.callee || '');
        if (callee !== calleeSymbol && compactSymbolName(callee) !== calleeCompact) {
            continue;
        }
 ...
 */
function readCallSiteSnippet(folder, relFile, line, contextLines = 12) {
    try {
 ...
 */
async function findCallersOf(folder, language, calleeSymbol, limit = 8) {
    const { relPath, key } = callgraphArtifactFor(language);
    const cgPath = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, relPath);
    let data;
    try {
    ...
    for (const call of orderedCalls) {
        ...
        if (!caller || callerCompact === calleeCompact || seen.has(callerCompact)) {
            continue;
        }
        const file = String(call?.file || '');
        const line = Number(call?.line || 0);
 ...
 * across languages.
 */
async function findSameCommitCallChains(folder, results) {
    ...
    for (const language of languagesPresent) {
        ...
        for (const call of orderedCalls) {
            ...
            if (!callerCompact || !calleeCompact || callerCompact === calleeCompact) {
                continue;
            }
 ...
 */
function estimatePythonBlockEndLine(absFilePath, startLine) {
    try {
 ...
 */
function estimateGoBlockEndLine(absFilePath, startLine) {
    try {
        const lines = fs.readFileSync(absFilePath, 'utf8').split(/\r?\n/);
        const startIdx = startLine - 1;
        if (startIdx < 0 || startIdx >= lines.length) {
 ...
 */
function parseGoCoverageProfile(profileText, targetPathPosix, rangeStart, rangeEnd) {
 ...
 */
function parseCoberturaCoverage(xmlText, targetPathPosix, rangeStart, rangeEnd) {
    let executedInRange = 0;
    let trackedInRange = 0;
 ...
 */
function parseJacocoXmlReport(xmlText, targetPathPosix, rangeStart, rangeEnd) {
    let executedInRange = 0;
    let trackedInRange = 0;
...
function findFileRecursive(dir, filename, maxDepth = 4) {
    if (maxDepth < 0) {
        return null;
    }
    let entries;
...
function execShellLineAsync(commandLine, cwd, env, timeoutMs = 120000) {
...
function estimateBraceBlockEndLine(absFilePath, startLine) {
    try {
        const lines = fs.readFileSync(absFilePath, 'utf8').split(/\r?\n/);
        const startIdx = startLine - 1;
        if (startIdx < 0 || startIdx >= lines.length) {
 ...
 */
function parseIstanbulCoverageForRange(jsonPath, targetAbsPath, rangeStart, rangeEnd) {
    let executedInRange = 0;
    let trackedInRange = 0;
    let data;
    try {
    ...
    const fileEntry = Object.values(data || {}).find((entry) => {
...
function looksLikeToolingFailure(stderr) {
...
function findCargoRootDir(absFilePath) {
...
function findJavaBuildRootDir(folder) {
    const root = folder.uri.fsPath;
    if (fs.existsSync(path.join(root, 'pom.xml'))) {
        return { root, tool: 'maven' };
    }
...
function findCsprojFor(absFilePath) {
    let dir = path.dirname(absFilePath);
    for (let i = 0; i < 20; i++) {
...
function targetFrameworkFor(csprojPath) {
    try {
 ...
 */
function detectJavaTestRunner(folder) {
    ...
    if (/junit-jupiter|org\.junit\.jupiter/.test(text)) {
        return { found: true, runner: 'JUnit 5' };
    }
    if (/<artifactId>\s*junit\s*<\/artifactId>|['"]junit:junit['"]/.test(text)) {
        return { found: true, runner: 'JUnit 4' };
    }
    if (/testng/i.test(text)) {
        return { found: true, runner: 'TestNG' };
    }
    return { found: false };
}
/**
 ...
 */
function insertMavenJUnitDependency(pomText) {
    const snippet = [
        '    <dependency>',
 ...
 */
function insertGradleJUnitDependency(buildText) {
 ...
 */
function parseLcovForRange(lcovText, targetPathPosix, rangeStart, rangeEnd) {
    let executedInRange = 0;
    let trackedInRange = 0;
 ...
 */
function buildChangesSummaryTiles(report, results, callChains) {
    const modified = Array.isArray(report?.modified) ? report.modified : [];
 ...
 */
function buildTestPlanSummaryLine(results, callChains) {
    const newFunctionCount = results.filter((r) => r.changeCard?.kind === 'added').length;
 ...
 */
function existingCallChainTestFile(folder, chain) {
...
function serializeCallChains(chains, folder) {
    return chains.map((c) => ({
...
function mcpSetupHelpText(folder, context, targets = ['claude', 'codex']) {
    ...
    if (selected.has('codex')) {
        lines.push('Codex CLI:', `1. Close the current Codex session for ${workspace}.`, `2. Open a new terminal in ${workspace} and run: codex`, `3. Codex should read "${MCP_SERVER_NAME}" (${MCP_SERVER_LABEL}) from: ${codexUserConfigPath()}`, '4. The current session will not hot-reload MCP config; it must be a new session.', `5. Type /mcp, or use the CODEMD button to send it automatically after launching Codex. If Codex asks whether to allow "${MCP_SERVER_NAME}", approve it there.`, 'If Windows says "codex is not recognized", install the Codex CLI or add it to PATH first.', '');
    }
    lines.push('Other MCP clients:', `Use a stdio MCP server named "${MCP_SERVER_NAME}" (${MCP_SERVER_LABEL}).`, `Command: node ${mcpServerArgs(context, workspace).join(' ')}`, 'Approval is controlled by the MCP client. CODEMD can register the server, but the user must approve/trust it inside the client when prompted.', '', 'What the number means:', 'The access count is historical. If the timestamp is old, Claude/Codex are not currently connected even though the config exists.', '', 
    ...
    'Optional: for CODEMD\'s testing tools specifically, consider adding this short pointer to your project\'s CLAUDE.md/AGENTS.md', '(after Regenerate has run at least once, so .codemd/agent_policy/testing.md exists):', '"When working with CODEMD testing, use Claude for software-engineering judgment and CODEMD for deterministic testing machinery. '
...
class GraphsViewProvider {
    ...
    staleCallgraphRefreshPromise = null;
    ...
    activeTestGenCancelers = new Map();
    ...
    ensureUncommittedChangeWatcher(folder = vscode.workspace.workspaceFolders?.[0]) {
    ...
    getClaudeAgentMode() {
        const config = vscode.workspace.getConfiguration('codemdGraphs');
        const inspected = config.inspect('claudeAgentMode');
        const explicitlySet = Boolean(inspected && (inspected.workspaceValue !== undefined
            || inspected.globalValue !== undefined
            || inspected.workspaceFolderValue !== undefined));
        if (explicitlySet) {
            const value = config.get('claudeAgentMode', 'manual');
            if (value === 'promptOnly' || value === 'agent' || value === 'manual') {
                return value;
            }
        }
        return config.get('claudeAgentConsent', false) === true ? 'agent' : 'manual';
    }
    /**
     ...
     */
    async setClaudeAgentMode(value) {
        const mode = value === 'promptOnly' ? 'promptOnly' : value === 'agent' ? 'agent' : 'manual';
        const config = vscode.workspace.getConfiguration('codemdGraphs');
     ...
     */
    postTestRunResult(payload, message) {
        ...
        if (requestId) {
            this.testRunStartTimes.delete(requestId);
        }
     ...
     */
    recordDefectIfGenuine(payload, message) {
    ...
    postDefects() {
        ...
        try {
            ...
            Object.keys(statusIndex).forEach((key) => {
                if (seenKeys.has(key) || statusIndex[key]?.status !== 'fixed') {
                    return;
                }
                const entry = statusIndex[key];
                countFixed(entry?.kind);
    ...
    async setupMcpFromPanel() {
        ...
        if (folder) {
            ...
            const choice = await vscode.window.showInformationMessage(changed
                ...
                : 'CODEMD: MCP config is already up to date. Open a client terminal to connect/reconnect codemd.', ...(targets.includes('codex') ? ['Open Codex /mcp'] : []), ...(targets.includes('claude') ? ['Open Claude Code /mcp'] : []));
    ...
    async removeMcpFromPanel() {
        const targets = await this.pickMcpSetupTargets('remove');
        if (!targets) {
            return;
        }
        const choice = await vscode.window.showWarningMessage('CODEMD: remove CODEMD MCP setup from this workspace? This removes only CODEMD MCP entries from local config files. Other MCP servers are left unchanged.', { modal: true }, 'Remove MCP Setup');
    ...
    async cleanArtifactsFromPanel() {
        ...
        const choice = await vscode.window.showWarningMessage(`CODEMD: clean up ${ARTIFACT_OUTPUT_DIR}/ in this workspace? This deletes generated graphs, search indexes, logs, and MCP usage history. Source files and MCP config files are not changed.`, { modal: true }, `Clean up ${ARTIFACT_OUTPUT_DIR}`);
        ...
        try {
            ...
            this.postSearchSuggestions(folder);
            this.refreshMcpUsage(folder);
            vscode.window.showInformationMessage(mcpConfigured
    ...
    async pickMcpSetupTargets(action) {
        const folder = vscode.workspace.workspaceFolders?.[0];
        if (!folder) {
    ...
    async approveClaudeFromPanel() {
        ...
        try {
            await approveClaudeMcpServer(folder);
            this.refreshMcpUsage();
            vscode.window.showInformationMessage(`CODEMD: "${MCP_SERVER_NAME}" (${MCP_SERVER_LABEL}) is pre-approved for Claude Code in "${folder.name}". Start a new Claude Code session to pick it up.`);
        }
        catch (err) {
    ...
    async approveCodexFromPanel() {
        ...
        try {
            ...
            this.refreshMcpUsage();
            vscode.window.showInformationMessage(`CODEMD: "${MCP_SERVER_NAME}" (${MCP_SERVER_LABEL}) tools are auto-approved for Codex in "${folder.name}". Start a new Codex session to pick it up.`);
        }
        catch (err) {
    ...
    async runGenerate(options) {
        ...
        try {
            ...
            if (quiet) {
                ...
                await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Generate CODEMD', cancellable: false }, async (progress) => {
                    await runBody((text) => {
     ...
     */
    async resolveSearchQueryAsCommit(trimmed) {
     ...
     */
    async refreshCallgraphIfStale(folder) {
    ...
    async refreshCallgraphIfStaleInner(folder) {
        if (this.busy) {
            return;
        }
        const outDirUri = vscode.Uri.joinPath(folder.uri, ARTIFACT_OUTPUT_DIR);
    ...
    async runResultGraph(result, title = '') {
        if (!this.ownerName || !this.repoName) {
            return;
        }
        const folder = vscode.workspace.workspaceFolders?.[0];
     ...
     */
    async generateRegressionTestForResult(result, requestId) {
        ...
        try {
            ...
            const entry = Array.isArray(report?.modified) ? report.modified.find((m) => m?.symbol === symbol) : null;
     ...
     */
    async generateCallPathTestForResult(result, requestId) {
        const symbol = String(result?.fullName || result?.graphSymbol || '').trim();
        ...
        try {
            ...
            const handleChunk = (text, stream) => {
                if (stream === 'stderr') {
                    if (text.trim()) {
                        this.post({ type: 'testGenProgress', requestId, chunk: text });
                    }
                    return;
                }
                ...
                for (const line of lines) {
                    ...
                    if (formatted) {
                        this.post({ type: 'testGenProgress', requestId, chunk: formatted + '\n' });
                    }
                }
            };
            ...
            const genPromise = runClaudeCliExclusive(() => {
                ...
                ], folder.uri.fsPath, { ...process.env }, CALL_PATH_TEST_TIMEOUT_MS, handleChunk);
            ...
            if (typeof envelope.duration_ms === 'number') {
                generationMs = envelope.duration_ms;
            }
     ...
     */
    async runClaudeTestGeneration(folder, prompt, requestId, expectedPath) {
        if (this.isReadOnlyMode()) {
        ...
        try {
            ...
            if (prettyOutput.length > 8000) {
                prettyOutput = prettyOutput.slice(0, 8000) + '\n… (truncated)';
            }
            if (run.stderr.trim()) {
    ...
    async writeGeneratedTest(folder, namePrefix, testSource, ext = '.py', language = null) {
        const outDir = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'generated_tests');
        await fs.promises.mkdir(outDir, { recursive: true });
     ...
     */
    async generateNewFunctionTestForResult(result, requestId) {
        const symbol = String(result?.fullName || result?.graphSymbol || '').trim();
        const file = String(result?.file || '');
        const startLine = Number(result?.line || 0) || 0;
        const folder = vscode.workspace.workspaceFolders?.[0];
     ...
     */
    async generateCallChainTestForResult(chain, requestId) {
     ...
     */
    async generateContractTestForResult(result, requestId) {
        const symbol = String(result?.fullName || result?.graphSymbol || '').trim();
        const file = String(result?.file || '');
        const startLine = Number(result?.line || 0) || 0;
        const folder = vscode.workspace.workspaceFolders?.[0];
        const issues = Array.isArray(result?.changeCard?.callSiteIssues) ? result.changeCard.callSiteIssues : [];
        if (!symbol || !file || !folder) {
     ...
     */
    async generateBroadCoverageTestForResult(result, requestId) {
        const symbol = String(result?.fullName || result?.graphSymbol || '').trim();
        const file = String(result?.file || '');
        const startLine = Number(result?.line || 0) || 0;
        const folder = vscode.workspace.workspaceFolders?.[0];
        ...
        for (const name of candidatePool) {
            if (selected.length >= MAX_BROAD_COVERAGE_CALLERS) {
                break;
            }
            const site = callSites.get(name);
            if (!site) {
            ...
            if (selected.length >= 2 && totalSnippetLines + snippetLines > MAX_BROAD_COVERAGE_SNIPPET_LINES) {
                break;
            }
        ...
        const callerBlocks = selected.map(({ name, site, snippet }) => `--- Caller: ${name} (${site.file}, around line ${site.line}) ---\n${snippet}`).join('\n\n');
        const targetLocation = startLine ? ` at line ${startLine}` : '';
        const prompt = [
     ...
     */
    async runExistingTestsForResult(result, requestId) {
        const query = String(result?.graphSymbol || result?.fullName || result?.symbol || result?.name || '').trim();
        const file = String(result?.file || '');
        const startLine = Number(result?.line || 0) || 0;
        const folder = vscode.workspace.workspaceFolders?.[0];
    ...
    async runTestFileForResult(message) {
        const requestId = String(message?.requestId || '');
        if (requestId) {
            this.testRunStartTimes.set(requestId, Date.now());
        }
    ...
    async runPythonTestFileForResult(message, folder) {
        const requestId = String(message?.requestId || '');
     ...
     */
    async installTestDependencyAndRetry(message) {
        const requestId = String(message?.requestId || '');
        if (requestId) {
            this.testRunStartTimes.set(requestId, Date.now());
        }
     ...
     */
    async setUpJsTestRunnerForResult(message, folder) {
        const requestId = String(message?.requestId || '');
        if (this.isReadOnlyMode()) {
     ...
     */
    async setUpJavaTestRunnerForResult(message, folder) {
        const requestId = String(message?.requestId || '');
        if (this.isReadOnlyMode()) {
            this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: 'Read-only mode is on — CODEMD will not modify your repo. Turn off codemdGraphs.readOnlyMode in Settings to enable this.' });
            return;
     ...
     */
    async findPythonImportRootFix(pythonPath, folder, targetPath, env) {
     ...
     */
    async execPythonTestCommand(pythonPath, args, folder, env, targetPath) {
     ...
     */
    async computePythonTestRunPayload(message, folder) {
    ...
    async computePythonTestRunPayloadImpl(message, folder) {
        const file = String(message?.file || '');
        ...
        let output = '';
        try {
            const covRun = await this.execPythonTestCommand(pythonPath, 
            ...
            ['-m', 'coverage', 'run', '--branch', `--data-file=${dataFile}`, '-m', 'pytest', target, '-v'], folder, env, targetPath);
            ...
            let trackedInRange;
            if (funcEntry) {
                const fnExecuted = Array.isArray(funcEntry.executed_lines) ? funcEntry.executed_lines : [];
                const fnMissing = Array.isArray(funcEntry.missing_lines) ? funcEntry.missing_lines : [];
                executedInRange = fnExecuted.length;
                trackedInRange = fnExecuted.length + fnMissing.length;
            }
            else {
                const executedLines = Array.isArray(entry?.executed_lines) ? entry.executed_lines : [];
                const missingLines = Array.isArray(entry?.missing_lines) ? entry.missing_lines : [];
                const rangeEnd = estimatePythonBlockEndLine(absTargetPath, targetStartLine) || targetStartLine;
                const inRange = (n) => n >= targetStartLine && n <= rangeEnd;
     ...
     */
    async runGoTestFileForResult(message, folder) {
        const requestId = String(message?.requestId || '');
    ...
    async computeGoTestRunPayload(message, folder) {
     ...
     */
    async runRustTestFileForResult(message, folder) {
        const requestId = String(message?.requestId || '');
        const file = String(message?.file || '').replace(/\\/g, '/');
        const nodeId = String(message?.nodeId || '');
        const targetPath = String(message?.targetPath || '').replace(/\\/g, '/');
        ...
        try {
            let run = await execCommandAsync('cargo', ['llvm-cov', '--lcov', '--output-path', lcovPath, '--', ...testFilterArgs], crateRoot, env, 150000);
    ...
    async ensureMavenArtifactCached(groupId, artifactId, version, classifier, buildRootDir, env) {
    ...
    async runJvmCoverageConfirmedLauncher(consoleJar, buildRootDir, runClasspath, fqcn, classDirs, targetPath, targetStartLine, absTargetFile, env) {
        ...
        try {
            ...
            for (const dir of classDirs) {
                reportArgs.push('--classfiles', dir);
            }
    ...
    async runJavaTestFileForResult(message, folder) {
        const requestId = String(message?.requestId || '');
    ...
    async computeJavaTestRunPayload(message, folder) {
        const file = String(message?.file || '');
        const targetPath = String(message?.targetPath || '').replace(/\\/g, '/');
        const targetStartLine = Number(message?.targetStartLine || 0);
        const buildRoot = findJavaBuildRootDir(folder);
        ...
        try {
            ...
            const launcherResult = await this.runJvmCoverageConfirmedLauncher(consoleJar, buildRoot.root, runClasspath, fqcn, classDirs, targetPath, targetStartLine, absTargetFile, env);
     ...
     */
    async runKotlinTestFileForResult(message, folder) {
        const requestId = String(message?.requestId || '');
    ...
    async computeKotlinTestRunPayload(message, folder) {
        const file = String(message?.file || '');
        const targetPath = String(message?.targetPath || '').replace(/\\/g, '/');
        const targetStartLine = Number(message?.targetStartLine || 0);
        const buildRoot = findJavaBuildRootDir(folder);
     ...
     * production.
     */
    async runCsharpTestFileForResult(message, folder) {
        const requestId = String(message?.requestId || '');
    ...
    async computeCsharpTestRunPayload(message, folder) {
        const file = String(message?.file || '');
        const targetPath = String(message?.targetPath || '');
        const targetStartLine = Number(message?.targetStartLine || 0);
        const absTestFile = path.join(folder.uri.fsPath, file);
        ...
        try {
            ...
            const run = await execCommandAsync('dotnet', ['test', '-v', 'minimal', '--collect:XPlat Code Coverage', '--results-directory', resultsDir], workDir, env, 180000);
            output = `${run.stdout}\n${run.stderr}`.trim();
            if (run.spawnError) {
     ...
     */
    async runTestViaClaudeForResult(message, folder) {
        const requestId = String(message?.requestId || '');
        if (requestId) {
            this.testRunStartTimes.set(requestId, Date.now());
        }
        ...
        if (deterministic) {
            ...
            try {
                const discover = await runClaudeCliExclusive(() => execCommandAsync(claudeCommand, [
                    '-p', discoveryPrompt,
                    '--max-turns', '6',
                    '--allowedTools', 'Read,Grep,Glob',
                    '--output-format', 'json',
    ...
    async fixTestFailureForResult(message, folder) {
        ...
        try {
            ...
            const promise = runClaudeCliExclusive(() => {
                if (canceledBeforeStart) {
                    return Promise.resolve({ status: null, stdout: '', stderr: '', timedOut: false, canceled: true });
                }
                const spawned = execCommandCancelableAsync(claudeCommand, [
    ...
    buildChangeResults(report, folder, options) {
        ...
        for (const res of results) {
            ...
            if (!sym || !res.file || !res.changeCard || (res.changeCard.kind !== 'modified' && res.changeCard.kind !== 'added')) {
                continue;
            }
    ...
    async runAutoMode() {
        ...
        try {
            ...
            const queue = allResults
                .filter((r) => r.changeCard && (r.changeCard.kind === 'modified' || r.changeCard.kind === 'added' || r.changeCard.kind === 'removed') && !r.changeCard.cosmeticOnly && !isLikelyTestFilePath(String(r.file || '')))
                .sort((a, b) => {
            ...
            for (let i = 0; i < queue.length; i++) {
                ...
                let covered = false;
                const coveredEntry = { symbol, file, status: 'unknown', steps: [] };
                const cache = readTestStatusCache(folder);
                ...
                if (kind === 'modified') {
                    if (file.toLowerCase().endsWith('.py') && hasBrokenCallers) {
                        genTypes.push('contract');
                    }
                    if (impactedCount >= 2) {
                        genTypes.push('broad');
                    }
                ...
                for (const genType of genTypes) {
                    ...
                    if (cachedTestFile) {
                        ...
                        else if (genType === 'broad') {
                            void this.generateBroadCoverageTestForResult(r, genReqId);
                        }
                        else {
                            void this.generateCallPathTestForResult(r, genReqId);
                        }
