...
function hasExistingCallgraphArtifacts(folder: vscode.WorkspaceFolder): boolean {
...
async function safeWorkspaceWriteFileAtomic(uri: vscode.Uri, content: Uint8Array): Promise<void> {
  assertWorkspaceWriteAllowed(uri, 'file write', 'file');
...
let debugLogPath = '';
let staleServerCleanupPromise: Promise<void> | null = null;
let serverStartPromise: Promise<string> | null = null;

function initializeDebugLog(context: vscode.ExtensionContext): void {
 ...
 */
function killStaleServerOnPort(port: number): Promise<void> {
  if (process.platform !== 'win32') {
...
function mcpServerScriptPath(workspaceRoot: string): string {
  return path.join(workspaceRoot, ARTIFACT_OUTPUT_DIR, MCP_OUTPUT_DIR, 'codemd-mcp-server.js');
}

function mcpServerArgs(context: vscode.ExtensionContext, workspaceRoot: string, serverName = MCP_SERVER_NAME): string[] {
...
async function setupClaudeProjectMcp(context: vscode.ExtensionContext, folder: vscode.WorkspaceFolder): Promise<boolean> {
  ...
  const serverConfigForName = (serverName: string) => ({
    command: 'node',
...
async function approveClaudeMcpServer(folder: vscode.WorkspaceFolder): Promise<void> {
  ...
  const enabled: string[] = Array.isArray(settings.enabledMcpjsonServers)
    ? settings.enabledMcpjsonServers.map((name: unknown) => String(name))
    : [];
...
function isCodexConfigApproved(configPath: string): boolean {
...
export function removeCodexMcpServerTables(existing: string): string {
...
async function approveCodexMcpServer(context: vscode.ExtensionContext, folder: vscode.WorkspaceFolder): Promise<void> {
  const codexDir = vscode.Uri.joinPath(folder.uri, WORKSPACE_CODEX_DIR);
  ...
  await safeWorkspaceCreateDirectory(codexDir);
  await safeWorkspaceWriteFile(
    projectConfigUri,
    Buffer.from(codexConfigWithMcp(existingProject, codexMcpBlock(context, folder.uri.fsPath, 'auto')), 'utf8'),
  );
  ...
  await fs.promises.writeFile(
    userConfigPath,
    codexConfigWithMcp(existingUser, codexMcpBlock(context, folder.uri.fsPath, 'auto')),
    'utf8',
  );
}

function isCodexMcpServerApproved(folder: vscode.WorkspaceFolder): boolean {
...
async function mirrorMcpServerScript(context: vscode.ExtensionContext, folder: vscode.WorkspaceFolder): Promise<void> {
...
function resolveBundledClaudeCli(): string | null {
  if (process.platform !== 'win32') {
    return null;
  }
  const extensionRoot = path.join(os.homedir(), '.vscode', 'extensions');
...
function extractClaudeResultEnvelope(stdout: string): any {
...
function tokensFromEnvelope(envelope: any): number | undefined {
...
function parseClaudeBudgetError(stdout: string, capUsd: number): string | null {
...
// Same idea as parseClaudeBudgetError, for the other way a genuinely-working
// run exits non-zero: it used up its whole --max-turns budget (subtype
// "error_max_turns") before reaching a final answer — e.g. investigating a
// wrong test-runner config in a monorepo, or a failure that turned out to
// need a live backend server. Without this, that case fell through to the
// same generic "may not be logged in" message a real auth failure gets,
// which is actively misleading after Claude has visibly just spent minutes
// reading files and running commands — confirmed against a real run where
// this happened (a 220s Fix attempt against dtc-starter's storefront that
// hit its turn cap while chasing the wrong jest config, reported as a login
// problem). A real auth failure exits on turn 1 with no tool activity, so
// this check never fires for that case.
function parseClaudeMaxTurnsError(stdout: string, maxTurns: number): string | null {
  const envelope = extractClaudeResultEnvelope(stdout);
  if (envelope?.subtype !== 'error_max_turns') { return null; }
  return `Claude used all ${maxTurns} turns without finishing — it may have been investigating something that turned out to be more involved than expected (or genuinely unfixable in this environment) rather than hitting an error. Check "View output" above for what it was doing, then it's safe to just try again if it looked close.`;
}
...
function formatClaudeStreamEvent(obj: any): string | null {
...
function summarizeClaudeToolInput(name: string, input: any): string {
  ...
  if (val) { return val.length > 140 ? val.slice(0, 140) + '…' : val; }
...
// through. testRunResult entries get file/targetSymbol/targetPath/durationMs/
// defectRecorded merged onto them by postTestRunResult specifically so this
// function has something useful to report beyond bare pass/fail — see that
// function's doc comment.
export function formatReportEntryMarkdown(message: any): string | null {
  if (!message || typeof message.type !== 'string') { return null; }
  const ts = new Date().toISOString().replace('T', ' ').replace(/\.\d+Z$/, ' UTC');
  const asSeconds = (ms: unknown) => (typeof ms === 'number' ? `${(ms / 1000).toFixed(1)}s` : undefined);
  const asUsage = (usd: unknown) => (typeof usd === 'number' ? `~$${usd.toFixed(2)} API-equivalent usage` : undefined);
  const statLine = (...parts: Array<string | undefined>) => {
    const kept = parts.filter((p): p is string => Boolean(p));
    return kept.length ? `- ${kept.join(' · ')}\n` : '';
  };

  // Fixed taxonomy from the README's "5 kinds of tests" table — keep these
  // labels in sync with that table's category names, not with the internal
  // message-type names, since two message types (callPathTestResult and
  // callChainTestResult) both fall under "Call-Path Test" there.
  const genKindLabels: Record<string, string> = {
    callPathTestResult: 'Call-Path Test',
    newFunctionTestResult: 'New-Function Test',
    callChainTestResult: 'Call-Path Test',
    contractTestResult: 'Contract Test',
    broadCoverageTestResult: 'Multi-Caller Test',
    regressionTestResult: 'Regression Test',
  };

  if (message.type === 'testRunResult') {
    if (message.ok === false && !message.error) { return null; }
    const status = message.ok === false ? '⚠ Error'
      : message.passed === true ? '✓ Passed'
      : message.passed === false ? '✗ Failed'
      : '— Ran (result unclear)';
    const target = message.targetSymbol || message.targetPath || '';
    let block = `### ${ts} — Test run: ${status}` + (target ? ` — \`${target}\`` : '') + '\n';
    const testFile = typeof message.file === 'string' ? message.file : '';
    const generatorKind = testFile ? generatorKindFromTestFileName(testFile) : '';
    const category = generatorKind ? (TEST_KIND_LABELS[generatorKind] || generatorKind)
      : message.viaClaude ? 'Claude-discovered test command'
      : testFile ? 'Existing test'
      : '';
    if (category) { block += `- Category: ${category}\n`; }
    if (testFile) { block += `- Test file: ${testFile}\n`; }
    if (message.error) { block += `- Error: ${message.error}\n`; }
    if (message.reason) { block += `- ${message.reason}\n`; }
    if (typeof message.coverageConfirmed === 'boolean') {
      block += `- Coverage confirmed: ${message.coverageConfirmed ? 'yes' : 'no'}\n`;
    }
    if (message.viaClaude) { block += '- Run via: Claude-discovered command\n'; }
    if (message.defectRecorded) {
      // Not yet known to be a code bug vs. a bad assumption in the generated
      // test itself — that classification only happens once a fix attempt
      // resolves it (see resolveDefectIfApplicable / the "Bug fixed" line
      // below), so this can't claim "Code bug" up front without sometimes
      // being wrong.
      block += `- ⚠ Bug found — recorded to ${message.defectRecorded}\n`;
    }
    return block + statLine(asSeconds(message.durationMs)) + '\n';
  }
  ...
  if (message.type === 'fixTestResult') {
    ...
    if (message.ok === false) {
      ...
      if (filesChanged.length) { block += `- Files: ${filesChanged.join(', ')}\n`; }
      if (message.defectResolvedKind === 'code') { block += `- ✓ Code bug fixed\n`; }
      else if (message.defectResolvedKind === 'test') { block += `- ✓ Test bug fixed — the generated test's own assumptions were wrong, not the code under test\n`; }
      if (message.summary) { block += `- Summary: ${message.summary}\n`; }
    }
...
function resolveCommand(command: string): Promise<string | null> {
  ...
  return new Promise((resolve) => {
    const proc = process.platform === 'win32'
      ? spawn('where.exe', [command])
      : spawn('sh', ['-lc', `command -v ${JSON.stringify(command)}`]);
    let stdout = '';
    let settled = false;
...
function terminalLaunchCommand(commandPath: string): string {
...
function openMcpApprovalCommand(terminal: vscode.Terminal): void {
  setTimeout(() => {
...
async function mcpSetupStatus(folder: vscode.WorkspaceFolder): Promise<{
  registered: boolean;
...
async function openMcpClientTerminal(client: McpClientCli, folder: vscode.WorkspaceFolder): Promise<void> {
...
type ClaudeAgentMode = 'manual' | 'promptOnly' | 'agent';

/**
 * Prompt-Only Mode's core mechanic: CODEMD builds the exact same prompt it
 * would otherwise send to a `claude` subprocess itself, but instead types it
 * into an interactive `claude` CLI session running in a VS Code terminal —
 * via Terminal.sendText(prompt, false), where `false` means "type this text
 * but do not press Enter" — so the user reads it, can edit it, and only
 * runs it by pressing Enter themselves. This spends zero of CODEMD's own
 * Claude budget: no subprocess is spawned, no --max-turns/--max-budget-usd
 * apply, and there is no captured stdout for CODEMD to parse afterward —
 * whatever happens next is entirely between the user and their own Claude
 * Code session. This only works with the `claude` CLI running in a
 * terminal, not the separate Claude Code VS Code extension's own chat
 * panel — a webview has no API surface another extension can type into.
 *
 * Reuses the same "CODEMD Claude Code" terminal openMcpClientTerminal uses
 * for MCP setup (mcpClientTerminals), so a user who already has that
 * terminal open gets their prompt typed into the SAME session instead of a
 * second, redundant one. Only launches `claude` itself when creating a
 * brand-new terminal — a reused terminal is assumed to already be running
 * it, the same assumption openMcpApprovalCommand already makes before
 * typing '/mcp' into one.
 */
async function sendPromptToUsersClaudeTerminal(folder: vscode.WorkspaceFolder, prompt: string): Promise<{ ok: true } | { ok: false; error: string }> {
  ...
  const existing = mcpClientTerminals.get('claude');
  const createdTerminal = !existing || existing.exitStatus !== undefined;
  const terminal: vscode.Terminal = createdTerminal
    ? vscode.window.createTerminal({ name: 'CODEMD Claude Code', cwd: folder.uri.fsPath })
    : existing;
  if (createdTerminal) {
    mcpClientTerminals.set('claude', terminal);
  }
  terminal.show();
  if (createdTerminal) {
    terminal.sendText(terminalLaunchCommand(claudeCommand));
    // Same boot delay openMcpApprovalCommand uses before typing '/mcp' into a
    // freshly launched session — claude's own REPL needs a moment to come up
    // before it can accept typed input.
...
type ClaudeUsageTotals = { tokens: number; ms: number };
...
function extensionBundleSignature(context: vscode.ExtensionContext): string {
  try {
 ...
 */
function warnIfReactivatedInStaleWindow(context: vscode.ExtensionContext): void {
  const currentVersion = String(context.extension.packageJSON?.version || '');
  const currentSignature = extensionBundleSignature(context);
  const stored = context.globalState.get<{ version: string; sessionId: string; signature?: string }>(LAST_ACTIVE_VERSION_KEY);
  ...
  outputChannel?.appendLine(
    `[activate] detected extension bundle change within the same window (${storedSignature} -> ${currentSignature}); prompting for reload.`,
  );
  vscode.window
...
function setupGitCommitRegenerateWatcher(
  context: vscode.ExtensionContext,
  provider: GraphsViewProvider,
  ...
  const attach = () => {
    try {
      ...
      const repo = gitApi.repositories.find((r: any) => r.rootUri.fsPath === folder.uri.fsPath) || gitApi.repositories[0];
      if (!repo) {
        return;
      }
  ...
  if (gitExtension.isActive) {
    ...
    gitExtension.activate().then(attach, (err: any) => {
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('codemdGraphs.openDebugLog', async () => {
      if (!debugLogPath) {
        initializeDebugLog(context);
      }
  ...
  if (vscode.workspace.workspaceFolders?.length) {
    ...
    if (config.get('autoReveal') !== false) {
      ...
      provider.reveal().then(
        () => outputChannel.appendLine('[activate] provider.reveal() resolved'),
        (err) => outputChannel.appendLine(`[activate] provider.reveal() REJECTED: ${err?.message || err}`),
      );
    }
...
function runCommand(
  ...
  options: { env?: NodeJS.ProcessEnv; detached?: boolean; timeoutMs?: number } = {},
): Promise<number> {
  return new Promise((resolve, reject) => {
    const proc = spawn(cmd, args, { cwd, env: options.env, detached: options.detached });
    trackProcess(proc);
    let timedOut = false;
 ...
 */
export function resolveServerPort(config: vscode.WorkspaceConfiguration, folder?: vscode.WorkspaceFolder): number {
 ...
 * observe the server unreachable and not-yet-tracked (isServerReachable and
 * the serverProcess check below both involve `await`s), and each spawn
 * their own uvicorn process racing to bind the same port.
 */
async function ensureServerRunning(
  ...
  const attempt = ensureServerRunningExclusive(context, onStatus, quiet).finally(() => {
    serverStartPromise = null;
  });
  serverStartPromise = attempt;
  return attempt;
}

export async function ensureServerRunningExclusive(
  context: vscode.ExtensionContext,
  onStatus: (message: string) => void,
  quiet: boolean,
): Promise<string> {
...
function analysisOutputDirUri(outDirUri: vscode.Uri): vscode.Uri {
...
function readLocalAnalysisResult(outDirUri: vscode.Uri): any | null {
...
function localScimFunctionsFileUri(outDirUri: vscode.Uri): vscode.Uri {
  return vscode.Uri.joinPath(outDirUri, ...SCIM_FUNCTIONS_RELATIVE_PATH.split('/'));
}

// ---------------------------------------------------------------------------
// Test-status cache — disk-backed, so "already generated" / "already passed"
// survive a VS Code restart or webview reload, not just the current session's
...
//     for a pre-existing/discovered test, not one CODEMD generated) + hashes
//     of both the target file and the test file at run time — a hand-edit to
//     either invalidates the cached result.
const TEST_STATUS_CACHE_FILE = 'generated_tests/.test-status-cache.json';

interface TestStatusGenEntry { testFile: string; targetHash: string; generatedAt: string; }
interface TestStatusRunEntry { testFile: string; targetHash: string; testHash: string; passed: boolean; coverageConfirmed: boolean; executed: boolean; ranAt: string; }
interface TestStatusEntry { generated?: Record<string, TestStatusGenEntry>; runs?: Record<string, TestStatusRunEntry>; }
interface TestStatusCache { version: 1; entries: Record<string, TestStatusEntry>; }

function testStatusCacheFileUri(folder: vscode.WorkspaceFolder): vscode.Uri {
  return vscode.Uri.joinPath(folder.uri, ARTIFACT_OUTPUT_DIR, ...TEST_STATUS_CACHE_FILE.split('/'));
}

function testStatusCacheKey(targetFile: string, symbol: string): string {
  return `${String(targetFile || '').replace(/\\/g, '/')}::${symbol}`;
}

function hashFileContents(absPath: string): string | null {
  try {
...
function resolveWorkspacePath(folder: vscode.WorkspaceFolder, maybeRelative: string): string {
...
function findScimFunctionSpan(folder: vscode.WorkspaceFolder, targetFile: string, symbol: string): { startLine: number; endLine: number } | null {
...
function hashFunctionSpan(folder: vscode.WorkspaceFolder, targetFile: string, symbol: string): string | null {
...
function readTestStatusCache(folder: vscode.WorkspaceFolder): TestStatusCache {
  try {
    const parsed = JSON.parse(fs.readFileSync(testStatusCacheFileUri(folder).fsPath, 'utf8'));
    if (parsed && typeof parsed === 'object' && parsed.entries && typeof parsed.entries === 'object') {
      return { version: 1, entries: parsed.entries };
    }
  } catch {
    // Missing or corrupt — every caller treats an empty cache as "nothing
    // known yet" and just does the real work, so this is always safe.
  }
  return { version: 1, entries: {} };
}

function writeTestStatusCache(folder: vscode.WorkspaceFolder, cache: TestStatusCache): void {
  try {
    const uri = testStatusCacheFileUri(folder);
    fs.mkdirSync(path.dirname(uri.fsPath), { recursive: true });
    fs.writeFileSync(uri.fsPath, `${JSON.stringify(cache, null, 2)}\n`, 'utf8');
  } catch {
    // Best-effort — losing a write just means the next check re-verifies
    // for real instead of trusting a stale/missing fact. Never unsafe.
  }
}

function recordGeneratedTestInCache(folder: vscode.WorkspaceFolder, targetFile: string, symbol: string, kind: string, testFile: string): void {
  const targetHash = hashFunctionSpan(folder, targetFile, symbol);
  if (!targetHash) { return; }
  const cache = readTestStatusCache(folder);
  const key = testStatusCacheKey(targetFile, symbol);
  const entry: TestStatusEntry = cache.entries[key] || {};
  entry.generated = entry.generated || {};
  entry.generated[kind] = { testFile, targetHash, generatedAt: new Date().toISOString() };
  cache.entries[key] = entry;
  writeTestStatusCache(folder, cache);
}

function recordTestRunInCache(
  folder: vscode.WorkspaceFolder, targetFile: string, symbol: string, kind: string, testFile: string,
  result: { passed: boolean; coverageConfirmed: boolean; executed: boolean },
): void {
  const targetHash = hashFunctionSpan(folder, targetFile, symbol);
  const testHash = hashFileContents(resolveWorkspacePath(folder, testFile));
  if (!targetHash || !testHash) { return; }
  const cache = readTestStatusCache(folder);
  const key = testStatusCacheKey(targetFile, symbol);
  const entry: TestStatusEntry = cache.entries[key] || {};
  entry.runs = entry.runs || {};
  entry.runs[kind] = {
    testFile, targetHash, testHash,
    passed: !!result.passed, coverageConfirmed: !!result.coverageConfirmed, executed: !!result.executed,
...
// Kind -> test file path, for every generator kind whose cached test still
// matches the target file's current content and still exists on disk.
function validCachedGeneratedKinds(folder: vscode.WorkspaceFolder, cache: TestStatusCache, targetFile: string, symbol: string): Record<string, string> {
  const out: Record<string, string> = {};
  const entry = cache.entries[testStatusCacheKey(targetFile, symbol)];
  if (!entry?.generated) { return out; }
  const currentHash = hashFunctionSpan(folder, targetFile, symbol);
  if (!currentHash) { return out; }
  for (const [kind, g] of Object.entries(entry.generated)) {
    if (g.targetHash === currentHash && fs.existsSync(resolveWorkspacePath(folder, g.testFile))) {
      out[kind] = g.testFile;
    }
  }
  return out;
...
// True if ANY cached run for this (target, symbol) — any kind — is still
// valid (both target and test file unchanged since) and was a confirmed
// pass. This is the "already covered, don't redo the work" signal.
function hasValidCachedPassingRun(folder: vscode.WorkspaceFolder, cache: TestStatusCache, targetFile: string, symbol: string): TestStatusRunEntry | null {
  const entry = cache.entries[testStatusCacheKey(targetFile, symbol)];
  if (!entry?.runs) { return null; }
  const currentTargetHash = hashFunctionSpan(folder, targetFile, symbol);
  if (!currentTargetHash) { return null; }
  for (const run of Object.values(entry.runs)) {
    if (!run.passed || !run.coverageConfirmed || !run.executed) { continue; }
    if (run.targetHash !== currentTargetHash) { continue; }
    const currentTestHash = hashFileContents(resolveWorkspacePath(folder, run.testFile));
    if (currentTestHash && currentTestHash === run.testHash) { return run; }
  }
  return null;
}

// Same validity check as hasValidCachedPassingRun, but scoped to one
// specific generator kind rather than "any kind passed" — used where the
// caller cares whether THIS kind's test specifically still needs a re-run
// (e.g. Auto Mode deciding whether to re-run a Contract test it's about to
// reuse), regardless of whether some other kind also happens to pass.
function validCachedRunForKind(folder: vscode.WorkspaceFolder, cache: TestStatusCache, targetFile: string, symbol: string, kind: string): TestStatusRunEntry | null {
  const run = cache.entries[testStatusCacheKey(targetFile, symbol)]?.runs?.[kind];
  if (!run) { return null; }
  const currentTargetHash = hashFunctionSpan(folder, targetFile, symbol);
  const currentTestHash = hashFileContents(resolveWorkspacePath(folder, run.testFile));
  if (!currentTargetHash || !currentTestHash) { return null; }
  return (run.targetHash === currentTargetHash && run.testHash === currentTestHash) ? run : null;
}

async function readMcpUsage(
  folder: vscode.WorkspaceFolder,
): Promise<{
  configured: boolean;
  totalCalls: number;
  ...
  toolsByClient: Record<string, Record<string, number>>;
  setup: Awaited<ReturnType<typeof mcpSetupStatus>>;
  ...
  try {
    ...
    const toolsByClient: Record<string, Record<string, number>> = {};
    for (const [clientName, tools] of Object.entries(toolsByClientRaw)) {
      if (!tools || typeof tools !== 'object') {
        continue;
      }
      toolsByClient[String(clientName)] = Object.entries(tools as Record<string, unknown>)
        .reduce<Record<string, number>>((normalized, [toolName, calls]) => {
...
function normalizeBackendSearchResult(item: any): NormalizedSearchResult {
...
function normalizeBackendTextResult(item: any): NormalizedSearchResult {
  const file = String(item?.file || item?.path || '');
  const line = String(item?.line || item?.start_line || '');
...
export function rewriteHtmlArtifactForWebview(content: string, relPath: string): string {
 ...
 */
function execGitAsync(args: string[], cwd: string): Promise<{ status: number | null; stdout: string; stderr: string }> {
  return new Promise((resolve) => {
 ...
 */
function execNodeCliAsync(scriptPath: string, args: string[], cwd: string, timeoutMs = 20000): Promise<{ status: number | null; stdout: string; stderr: string; timedOut: boolean }> {
  return new Promise((resolve) => {
 ...
 */
/**
 * Bounds how many local pytest/coverage subprocesses computePythonTestRunPayload
 * can have running at once. The existing-tests check (runExistingTestsForResult,
 * fired by clicking "Check tests" on a Python symbol) can fire once per changed/added
 * Python symbol in a result batch — with no cap of
 * its own, since it's local-only and was assumed cheap — and each fire runs up to 3
 * matched test files in parallel via Promise.all. A large diff or first-time scan
 * across a big repo can therefore spawn hundreds of `coverage run -m pytest`
 * processes simultaneously and exhaust the system. This queues excess requests
 * instead of firing them all at once; nothing is dropped, just serialized.
 */
const MAX_CONCURRENT_PYTHON_TEST_RUNS = 3;
let activePythonTestRuns = 0;
const pythonTestRunWaiters: Array<() => void> = [];

function acquirePythonTestRunSlot(): Promise<void> {
  if (activePythonTestRuns < MAX_CONCURRENT_PYTHON_TEST_RUNS) {
    activePythonTestRuns++;
    return Promise.resolve();
  }
  return new Promise((resolve) => { pythonTestRunWaiters.push(resolve); });
}

function releasePythonTestRunSlot(): void {
  const next = pythonTestRunWaiters.shift();
  if (next) {
    next(); // hand the slot straight to the next waiter — activePythonTestRuns stays the same
  } else {
    activePythonTestRuns--;
  }
}
...
function execCommandOnceAsync(
  ...
  return new Promise((resolve) => {
    ...
    trackProcess(proc);
    // No caller of this function ever writes to stdin — end it immediately
    // so a CLI that defensively waits for piped stdin before proceeding
    // (observed with `claude -p ...`: "Warning: no stdin data received in
    // 3s, proceeding without it") sees EOF right away instead of stalling
    // every single invocation for ~3s.
    proc.stdin?.end();
    let stdout = '';
    let stderr = '';
    let timedOut = false;
 ...
 */
function execCommandCancelableAsync(
  cmd: string,
  args: string[],
  cwd: string,
  env: NodeJS.ProcessEnv,
...
): { promise: Promise<{ status: number | null; stdout: string; stderr: string; timedOut: boolean; canceled: boolean; spawnError?: string }>; cancel: () => void } {
  let killCurrent: (() => void) | null = null;
  const runOnce = (useShell: boolean) => new Promise<{ status: number | null; stdout: string; stderr: string; timedOut: boolean; canceled: boolean; spawnError?: string }>((resolve) => {
    let proc: ChildProcessWithoutNullStreams;
    try {
      proc = spawn(cmd, args, { cwd, env, shell: useShell });
    } catch (err: any) {
    ...
    trackProcess(proc);
    // See the matching comment in execCommandOnceAsync — end stdin right
    // away so `claude -p ...` doesn't stall ~3s per call waiting for piped
    ...
    const timer = setTimeout(() => {
      timedOut = true;
      if (proc.pid) { killProcessTree(proc.pid); } else { proc.kill(); }
    }, timeoutMs);
    killCurrent = () => {
      canceled = true;
      clearTimeout(timer);
    ...
    proc.stdout?.on('data', (chunk: Buffer) => { const text = chunk.toString(); stdout += text; onChunk?.(text, 'stdout'); });
    proc.stderr?.on('data', (chunk: Buffer) => { const text = chunk.toString(); stderr += text; onChunk?.(text, 'stderr'); });
...
/**
 * Every `claude -p ...` spawn in this file (test generation, the fix flow,
 * and the "find a test command" discovery step) funnels through this queue
 * instead of ever running two invocations at once. This is process-wide, not
 * per-panel/per-request, so it also covers the case Auto Mode alone can't
 * guard against: a manual generate/fix chip clicked while Auto Mode's own
 * serial loop is mid-call, or two webview panels each driving their own
 * pipeline. Concurrent `claude` CLI processes were observed to intermittently
 * exit non-zero with what looks like an auth failure ("may not be logged
 * in") even though the account is fine — this makes that reproducible-only-
 * under-overlap failure mode go away by construction rather than trying to
 * detect and retry it after the fact.
 */
let claudeCliQueueTail: Promise<void> = Promise.resolve();
function runClaudeCliExclusive<T>(task: () => Promise<T>): Promise<T> {
  const turn = claudeCliQueueTail.then(task, task);
  claudeCliQueueTail = turn.then(() => undefined, () => undefined);
  return turn;
}
 ...
 */
async function resolveUvProjectPython(folder: vscode.WorkspaceFolder): Promise<string | null> {
  if (!fs.existsSync(path.join(folder.uri.fsPath, 'uv.lock'))) {
    return null;
  }
  try {
    const probe = await execCommandAsync(
      'uv',
...
/** Languages the Claude-based test generators below understand. Adding one
 * means: a real callgraph artifact with caller/call-site data (see
 * callgraphArtifactFor), a module/import-name resolver, and a test-framework
 * hint for the prompt — not just an extension check. */
type TestLanguage = 'python' | 'javascript' | 'java' | 'csharp' | 'go' | 'rust' | 'kotlin';

function testLanguageFor(file: string): TestLanguage | null {
...
/**
 * Cheap, filesystem-only check for whether a JS/TS repo has ANY test runner
 * set up at all — no Claude call. A repo with zero test tooling is the
 * common case CODEMD exists for, not an edge case, so this needs to answer
 * honestly and immediately rather than let "Ask Claude to run" spend 90-120s
 * discovering nothing, or let "Fix with Claude" freelance an install of its
 * own choosing (see fixTestFailureForResult's environment-issue prompt,
 * which is explicitly barred from that — see the comment there). CODEMD
 * never installs a test framework into a user's repo on its own; this only
 * reports what's already there.
 */
// Directories to check, nearest first: the target file's own package up to
// (and including) the workspace root. Confirmed against a real monorepo
// (dtc-starter, a pnpm/Turborepo workspace): its root package.json declares
// "test": "turbo test", which only actually runs apps/backend's Jest suite —
// apps/storefront has no test runner at all. Checking only the workspace
// root (the original behavior) read that root-level script and concluded
// the WHOLE repo "has a runner", so a storefront file's Run/generate-test
// flow skipped the free, deterministic "Set up test runner" offer entirely
// and went straight into a Claude-driven command-discovery/Fix cycle that
// burned ~90-220s and real usage per attempt before arriving at the same
// "there's no runner here" conclusion the cheap check should have given
// immediately. Real per-package tooling (Jest, ESLint, npm workspaces
// itself) resolves config the same way: nearest directory wins, not the
// repo root.
function jsTestRunnerSearchDirs(folder: vscode.WorkspaceFolder, targetFile?: string): string[] {
  const root = folder.uri.fsPath;
  if (!targetFile) { return [root]; }
  ...
  for (let i = 0; i < 12; i++) {
    ...
    dirs.push(resolved);
    if (resolved === rootResolved) { break; }
    const parent = path.dirname(resolved);
    // Stop at the workspace root even if it's reached via a symlink/case
    // mismatch that fails the exact-string check above, and stop at the
    // filesystem root so a target file outside the workspace can't walk
    // forever.
    if (parent === resolved || (!resolved.startsWith(rootResolved) && dirs.length > 1)) { break; }
    dir = parent;
  }
...
function detectJsTestRunner(folder: vscode.WorkspaceFolder, targetFile?: string): { found: boolean; runner?: string; dir?: string } {
  const KNOWN_RUNNER_DEPS = ['jest', 'vitest', 'mocha', 'ava', 'tape', 'uvu', '@vscode/test-cli', '@vscode/test-electron'];
  const CONFIG_FILES: { file: string; runner: string }[] = [
    { file: 'jest.config.js', runner: 'jest' },
    { file: 'jest.config.cjs', runner: 'jest' },
    ...
    { file: 'vitest.config.mjs', runner: 'vitest' },
    { file: '.mocharc.json', runner: 'mocha' },
    { file: '.mocharc.js', runner: 'mocha' },
    { file: '.mocharc.yml', runner: 'mocha' },
    { file: '.mocharc.yaml', runner: 'mocha' },
    { file: '.vscode-test.mjs', runner: '@vscode/test-cli' },
    { file: '.vscode-test.js', runner: '@vscode/test-cli' },
  ];
  for (const dir of jsTestRunnerSearchDirs(folder, targetFile)) {
    try {
      const pkgPath = path.join(dir, 'package.json');
      if (fs.existsSync(pkgPath)) {
        const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
        const allDeps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
        for (const runner of KNOWN_RUNNER_DEPS) {
          if (allDeps[runner]) { return { found: true, runner, dir }; }
        }
        // npm's own scaffolded placeholder ("Error: no test specified") is not
        // a real test script — everything else counts, even a thin wrapper
        // around a runner not in KNOWN_RUNNER_DEPS (e.g. a custom test.js).
        // A monorepo root delegating via "turbo test"/"pnpm -r test" is
        // deliberately NOT treated as a real answer for a specific package —
        // only this exact directory's own script counts, which is why the
        // root is checked last (see jsTestRunnerSearchDirs) rather than
        // short-circuiting the search.
        const testScript = String(pkg.scripts?.test || '');
        if (testScript && !/no test specified/i.test(testScript) && !/^(turbo|pnpm -r|npm run -ws|yarn workspaces)\b/.test(testScript.trim())) {
          return { found: true, runner: 'package.json "test" script', dir };
        }
      }
    } catch {
      // Malformed package.json — fall through to the config-file scan below
      // rather than treating it as "no runner found".
    }
    for (const { file, runner } of CONFIG_FILES) {
      if (fs.existsSync(path.join(dir, file))) {
        return { found: true, runner, dir };
      }
    }
  }
  return { found: false };
...
// Known runners whose CLI accepts a bare file path as a positional filter —
// covers the actual generated-test case (CODEMD always knows exactly which
// one file it wants run) without guessing at an arbitrary custom "test"
// script, which might not accept a path argument at all (e.g. `node
// test.js` ignores extra args silently). That ambiguous case, and any
// runner outside this list, still falls back to asking Claude to work out
// the right invocation — this only replaces the mechanical, deterministic
// majority of runs.
const DETERMINISTIC_JS_RUNNERS: Record<string, (relPath: string) => string> = {
  vitest: (relPath) => `npx vitest run "${relPath}"`,
  jest: (relPath) => `npx jest "${relPath}"`,
  mocha: (relPath) => `npx mocha "${relPath}"`,
  ava: (relPath) => `npx ava "${relPath}"`,
};

/**
 * Builds the exact shell command to run one JS/TS test file with zero
 * Claude involvement, for the common case where CODEMD already knows the
 * runner (from detectJsTestRunner) and the runner has a well-known,
 * deterministic single-file invocation. Returns null for anything less
 * certain (a generic package.json "test" script, or a runner not in
 * DETERMINISTIC_JS_RUNNERS) so the caller can fall back to Claude discovery
 * instead of guessing.
 */
function buildDeterministicJsRunCommand(
  folder: vscode.WorkspaceFolder,
  detected: { found: boolean; runner?: string; dir?: string },
  testFile: string,
): { command: string; cwdRel: string } | null {
  if (!detected.found || !detected.dir || !detected.runner) { return null; }
  const builder = DETERMINISTIC_JS_RUNNERS[detected.runner];
  if (!builder) { return null; }
  const root = folder.uri.fsPath;
  const absTestFile = path.isAbsolute(testFile) ? testFile : path.join(root, testFile);
  const relFromDir = path.relative(detected.dir, absTestFile).split(path.sep).join('/');
  const cwdRel = path.relative(root, detected.dir).split(path.sep).join('/');
  return { command: builder(relFromDir), cwdRel };
}

interface JsTestRunnerPlan {
  runner: 'vitest' | 'jest';
  packages: string[];
  configFile: string;
...
/**
 * Deterministically picks ONE lightweight test runner for a JS/TS repo that
 * has none — a fixed rule, not a Claude judgment call, so the same repo
 * shape always produces the same answer and the footprint stays small on
 * purpose (this exists to run the handful of tests CODEMD itself generates,
 * not to stand up a full test culture). Priority mirrors real ecosystem
 * convention instead of a single hardcoded default:
 * - An existing Babel/CRA/Next.js setup -> Jest, since that's the path of
 *   least resistance there (those stacks ship Jest presets already) and
 *   fighting that convention would cost more setup than it saves.
 * - Otherwise -> Vitest: needs no separate ts-jest/babel-jest compile step
 *   for TypeScript, and installs a single package instead of three. This
 *   also covers ESM/Vite-native projects, where Vitest fits directly.
 */
function selectJsTestRunner(folder: vscode.WorkspaceFolder): JsTestRunnerPlan {
  let pkg: any = {};
  try {
    pkg = JSON.parse(fs.readFileSync(path.join(folder.uri.fsPath, 'package.json'), 'utf8'));
  } catch {
    // No/invalid package.json — fall through to the plain-Node default below.
  }
  const allDeps = { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) };
  const isEsm = pkg.type === 'module';
  const hasViteConfig = ['vite.config.js', 'vite.config.ts', 'vite.config.mjs'].some((f) => fs.existsSync(path.join(folder.uri.fsPath, f)));
  const hasBabelSetup = ['.babelrc', '.babelrc.js', '.babelrc.json', 'babel.config.js', 'babel.config.json']
    .some((f) => fs.existsSync(path.join(folder.uri.fsPath, f)))
    || Boolean(allDeps['next'] || allDeps['react-scripts']);
  ...
  if (hasBabelSetup) {
    return {
      ...
      configContents: "module.exports = {\n  preset: 'ts-jest',\n  testEnvironment: 'node',\n  testMatch: ['<rootDir>/.codemd/generated_tests/**/*.test.[jt]s?(x)'],\n};\n",
      testScript: 'jest',
      reason: 'This project already has a Babel/CRA/Next.js setup, where Jest is the path of least resistance.',
    };
  }
  return {
    runner: 'vitest',
    packages: ['vitest'],
    // .mts (not .ts) — verified live: a plain .ts config with ESM `export
    // default` in a package.json with no "type": "module" makes Vite warn
    // on every single run ("uses features unsupported by configLoader:
    // 'native'"). .mts sidesteps it entirely, no other change needed.
    // globals: true — closest match to how CODEMD's own prompts already
    // write JS/TS tests (bare describe/it/expect, Jest's default ergonomics)
    // so a generated test needs no extra import line for those.
    configFile: 'vitest.config.mts',
    configContents: "import { defineConfig } from 'vitest/config';\n\nexport default defineConfig({\n  test: {\n    environment: 'node',\n    globals: true,\n    include: ['.codemd/generated_tests/**/*.test.{ts,tsx,js,jsx}'],\n  },\n});\n",
    testScript: 'vitest run',
    reason: isEsm || hasViteConfig
      ? 'This project is already ESM/Vite-native, which Vitest fits directly.'
      : 'No existing JS/TS test framework or bundler convention found — Vitest needs no separate compile step for TypeScript and is the smaller install of the two.',
  };
}

/**
 * Repo-relative path (under ARTIFACT_OUTPUT_DIR) to the callgraph artifact
 * holding real caller/call-site data for a language, and the JSON key its
 * ordered call list lives under. Python's build_python_callgraph embeds
 * `ordered_calls` directly inside python_callgraph.json; the JS/TS pipeline
 * instead writes a `calls` array to a separate ordered-call-sequence file
 * (see write_ordered_call_sequence in backend/main.py) — same per-call
 * shape ({caller, callee, file, line, call_text}) either way, just a
 * different container.
 */
function callgraphArtifactFor(language: TestLanguage): { relPath: string; key: 'ordered_calls' | 'calls' } {
  if (language === 'python') {
    return { relPath: path.join('python', 'python_callgraph.json'), key: 'ordered_calls' };
  }
  if (language === 'java') {
    // build_tree_sitter_java_callgraph (backend/main.py) writes its ordered
    // call list the same way the JS/TS pipeline does — a separate `calls`
    // array via the shared write_ordered_call_sequence, not embedded under
    // `ordered_calls` like Python's.
    return { relPath: path.join('tree_sitter_java', 'tree_sitter_java_ordered_call_sequence.json'), key: 'calls' };
  }
  if (language === 'csharp') {
    // build_csharp_callgraph (backend/main.py) is regex-based, not
    // tree-sitter, but writes through the same shared
    // write_ordered_call_sequence as Java/JS — confirmed by reading it —
    // so the container/key convention is identical.
    return { relPath: path.join('csharp', 'csharp_ordered_call_sequence.json'), key: 'calls' };
  }
  if (language === 'go') {
    return { relPath: path.join('tree_sitter_go', 'tree_sitter_go_ordered_call_sequence.json'), key: 'calls' };
  }
  if (language === 'rust') {
    return { relPath: path.join('tree_sitter_rust', 'tree_sitter_rust_ordered_call_sequence.json'), key: 'calls' };
  }
  if (language === 'kotlin') {
    return { relPath: path.join('tree_sitter_kotlin', 'tree_sitter_kotlin_ordered_call_sequence.json'), key: 'calls' };
  }
  return { relPath: path.join('javascript', 'javascript_ordered_call_sequence.json'), key: 'calls' };
}

/** Rough "top-level area of the repo" for a file, used only to flag when a
 * call chain crosses a module boundary — doesn't need to be a precise
 * package/namespace concept, just enough to tell "same area" from
 * "different area" apart. */
function topLevelModuleFor(file: string, language: TestLanguage): string {
  if (language === 'python') {
    return pythonModuleNameFor(file).split('.')[0];
  }
...
/**
 * Repo-relative JS/TS source path -> the relative import specifier a test
 * saved under .codemd/generated_tests/ needs to reach it. Unlike Python's
 * dotted module names (a package concept), JS/TS imports are real relative
 * file paths, so this walks the actual directory distance from the
 * generated-tests output folder to the source file instead of reusing
 * pythonModuleNameFor's approach.
 */
function jsImportPathFor(folder: vscode.WorkspaceFolder, file: string): string {
  const outDir = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'generated_tests');
  const sourceAbs = path.join(folder.uri.fsPath, file);
  let rel = path.relative(outDir, sourceAbs).replace(/\\/g, '/');
  rel = rel.replace(/\.(jsx|tsx|mjs|cjs|js|ts)$/i, '');
  return rel.startsWith('.') ? rel : `./${rel}`;
}

/**
 * Java's import is `import <package>.<ClassOrMember>;`, derived from the
 * source file's own `package` declaration — not a directory-convention
 * guess like topLevelModuleFor uses, since getting this wrong produces a
 * test that doesn't compile at all (unlike Python/JS, where a slightly-off
 * import at least fails informatively at runtime). Reads the file once to
 * extract it; falls back to an honest "check the file" hint rather than
 * guessing a package from the path when the file is unreadable or has none
 * (the default/unnamed package is rare but valid Java).
 */
function javaImportHint(folder: vscode.WorkspaceFolder, file: string, tail: string): string {
  let packageName = '';
  try {
    const source = fs.readFileSync(path.join(folder.uri.fsPath, file), 'utf8');
    const match = source.match(/^\s*package\s+([\w.]+)\s*;/m);
    if (match) { packageName = match[1]; }
  } catch {
    // Unreadable — fall through to the no-package-found phrasing below.
  }
  return packageName
    ? `importable as \`import ${packageName}.${tail};\` (this file declares \`package ${packageName};\`)`
    : `in the default (unnamed) Java package as far as could be determined — check "${file}" for its actual \`package\` declaration before writing the import (a test outside that package will not compile)`;
}

/**
 * C#'s equivalent of javaImportHint — reads the file's own `namespace`
 * declaration (either the classic block form `namespace X { ... }` or the
 * C# 10+ file-scoped form `namespace X;`) since a test needs `using
 * {namespace};` (or the fully-qualified `{namespace}.{tail}`) to reach the
 * target. Unlike Java, C# does not require the file/class name to match, so
 * this is the one place C# genuinely needs its own logic — everything about
 * saving the file (generatedTestFileNameFor) can reuse the loose
 * Python/JS-style naming untouched.
 */
export function csharpImportHint(folder: vscode.WorkspaceFolder, file: string, tail: string): string {
  let namespaceName = '';
  try {
    const source = fs.readFileSync(path.join(folder.uri.fsPath, file), 'utf8');
    const fileScoped = source.match(/^\s*namespace\s+([\w.]+)\s*;/m);
    const blockScoped = source.match(/\bnamespace\s+([\w.]+)\s*\{/);
    namespaceName = (fileScoped || blockScoped)?.[1] || '';
  } catch {
    // Unreadable — fall through to the no-namespace-found phrasing below.
  }
  return namespaceName
    ? `importable via \`using ${namespaceName};\` (or fully qualified as \`${namespaceName}.${tail}\`) — this file declares \`namespace ${namespaceName}\``
    : `in no namespace as far as could be determined — check "${file}" for its actual \`namespace\` declaration before writing the \`using\` directive`;
}

/**
 * Go's import needs the full module path, not just the package name —
 * `import "pkg"` alone only works for stdlib; a real project's own package
 * needs `import "{module}/{dir}"` from go.mod's own `module` directive,
 * since a generated test lives outside the module's normal tree
 * (.codemd/generated_tests/) and can't infer this from its own location the
 * way a colocated _test.go file could.
 */
function goImportHint(folder: vscode.WorkspaceFolder, file: string, tail: string): string {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if (fs.existsSync(goModPath)) {
      try {
        const goModText = fs.readFileSync(goModPath, 'utf8');
        const moduleMatch = goModText.match(/^\s*module\s+(\S+)/m);
        if (moduleMatch) {
          const dirRel = path.relative(dir, path.dirname(absFile)).replace(/\\/g, '/');
          modulePath = dirRel ? `${moduleMatch[1]}/${dirRel}` : moduleMatch[1];
        }
      } catch {
        // Unreadable go.mod — fall through with whatever modulePath we have.
      }
      break;
    }
  ...
  if (modulePath && packageName) {
    return `importable as \`import "${modulePath}"\` (package \`${packageName}\`), calling it as \`${packageName}.${tail}\``;
  }
  if (packageName) {
    return `in package \`${packageName}\`, but no go.mod module path could be determined — check the module path yourself before writing the import`;
  }
  return `in a package that could not be determined — check "${file}" for its actual \`package\` declaration`;
}

/**
 * Rust's import needs the crate name (from Cargo.toml's own `name`, with
 * hyphens normalized to underscores per Cargo's own convention) plus the
 * module path from the crate's `src/` root to this file — a best-effort
 * derivation, not a guarantee, since Rust's actual module tree is declared
 * via `mod` statements that don't always mirror the file layout exactly
 * (re-exports, `#[path = ...]` overrides). Same honesty tier as this
 * extension's existing Rust run-path caveat: good enough to point Claude in
 * the right direction, not a substitute for it checking the file's own
 * `mod`/`pub use` declarations.
 */
export function rustImportHint(folder: vscode.WorkspaceFolder, file: string, tail: string): string {
  const absFile = path.join(folder.uri.fsPath, file);
  const crateRoot = findCargoRootDir(absFile);
  let crateName = '';
  if (crateRoot) {
    try {
      const cargoToml = fs.readFileSync(path.join(crateRoot, 'Cargo.toml'), 'utf8');
      const nameMatch = cargoToml.match(/^\s*name\s*=\s*"([^"]+)"/m);
      if (nameMatch) { crateName = nameMatch[1].replace(/-/g, '_'); }
    } catch {
      // Unreadable Cargo.toml — fall through with no crate name.
    }
  }
  let modPath = '';
  if (crateRoot) {
    const srcDir = path.join(crateRoot, 'src');
    const dirRel = path.relative(srcDir, path.dirname(absFile)).replace(/\\/g, '/');
    const fileStem = path.basename(absFile, '.rs');
    const segments = [dirRel, ['lib', 'main', 'mod'].includes(fileStem) ? '' : fileStem].filter((s) => s && s !== '.');
    modPath = segments.join('/').replace(/\//g, '::');
  }
  if (crateName && modPath) {
    return `importable as \`use ${crateName}::${modPath}::${tail};\` — a best-effort guess from this file's location under src/, not a guarantee; check "${file}"'s own \`mod\`/\`pub use\` declarations before trusting it`;
  }
  if (crateName) {
    return `in crate \`${crateName}\`, but its module path could not be determined from this file's location — check "${file}"'s \`mod\` declarations before writing the \`use\` statement`;
  }
  return `in a crate that could not be determined — check "${file}" for the nearest Cargo.toml`;
}

/** How to phrase "importable as ..." for the target symbol in a prompt —
 * Python uses `from {dotted.module} import {name}`; JS/TS uses a relative
 * import path and may be ESM or CommonJS, so the prompt is told to check
 * rather than assuming one. */
/**
 * A private `function foo(...)` (no `export`) compiles fine under esbuild's
 * type-stripping transform — it's not a type error, so vitest/jest never
 * catch it. `import { foo } from './file'` just silently resolves to
 * `undefined`, and the generated test fails with a confusing "foo is not a
 * function" at the call site instead of a clear "not exported" error at
 * import time. Every test-gen prompt also tells Claude "Do not modify any
 * files" (so it can't fix this itself even if it noticed), so this
 * mechanically exports the target here, deterministically, before the
 * prompt is even built. Widening a function/const/class from private to
 * exported cannot change its behavior, so this is safe to do unconditionally
 * rather than asking Claude to reason about it.
 */
function ensureJsSymbolExported(folder: vscode.WorkspaceFolder, file: string, tail: string): void {
  ...
  const alreadyExported = new RegExp(`export\\s+(default\\s+)?(async\\s+)?(function\\*?|class|const|let|var)\\s+${escaped}\\b`).test(source)
    || new RegExp(`export\\s*\\{[^}]*\\b${escaped}\\b[^}]*\\}`).test(source)
    || new RegExp(`(module\\.exports|exports)\\.${escaped}\\s*=`).test(source)
    || new RegExp(`module\\.exports\\s*=\\s*\\{[^}]*\\b${escaped}\\b[^}]*\\}`).test(source);
  if (alreadyExported) { return; }

  const declPattern = new RegExp(`^([ \\t]*)(async\\s+function\\*?|function\\*?|class|const|let|var)(\\s+${escaped}\\b)`, 'm');
  const match = declPattern.exec(source);
  if (!match) { return; }
  const insertAt = match.index + match[1].length;
  const updated = source.slice(0, insertAt) + 'export ' + source.slice(insertAt);
  try {
    fs.writeFileSync(abs, updated, 'utf8');
  } catch {
    // Best effort — a failed write leaves the same "not a function" failure
 ...
 */
function jsFileRequiresVscodeStub(folder: vscode.WorkspaceFolder, file: string): boolean {
  const abs = path.join(folder.uri.fsPath, file);
  try {
...
function importHintFor(language: TestLanguage, file: string, folder: vscode.WorkspaceFolder, tail: string): string {
  if (language === 'python') {
    return `importable as \`from ${pythonModuleNameFor(file)} import ${tail}\``;
  }
  if (language === 'java') {
    return javaImportHint(folder, file, tail);
  }
  if (language === 'csharp') {
    return csharpImportHint(folder, file, tail);
  }
  if (language === 'go') {
    return goImportHint(folder, file, tail);
  }
  if (language === 'rust') {
    return rustImportHint(folder, file, tail);
  }
  if (language === 'kotlin') {
    // Kotlin's package syntax is identical to Java's -- same extraction
    // logic applies verbatim.
    return javaImportHint(folder, file, tail);
  }
  // Verified against a real generation (backend/static/autotrack.js's
  // codevalCleanText): without the "exact path, do not recompute" framing,
  // Claude reasonably assumed a conventional Jest-colocated `__tests__/`
  // location in its own reasoning and re-derived the relative import from
  // THAT imagined path instead of trusting the one given here — which
  // produced an import one directory level off (resolved outside the repo
  // entirely) once the file was actually saved flatly under
 ...
 * prompt to detect which JS/TS framework is actually in use rather than
 * assuming Jest. */
// Labels for the runners detectJsTestRunner can confidently name (matching
// its own KNOWN_RUNNER_DEPS) — anything else (a generic "package.json test
// script", tape/uvu, or no detection at all) falls back to having Claude
// work it out itself in testFrameworkNounFor below.
const JS_RUNNER_DISPLAY_NAMES: Record<string, string> = { vitest: 'Vitest', jest: 'Jest', mocha: 'Mocha', ava: 'AVA' };

/**
 * jsRunnerHint, when given, is the runner detectJsTestRunner already found
 * for this exact target BEFORE Claude is invoked — passing it through here
 * turns "check package.json and existing test files to see which one it
 * uses" (a Read/Grep round trip Claude would otherwise spend on every single
 * test generation) into a plain fact, since CODEMD already knows the answer
 * deterministically. Falls back to the old self-discovery wording whenever
 * detection didn't confidently resolve to one of JS_RUNNER_DISPLAY_NAMES.
 */
function testFrameworkNounFor(language: TestLanguage, jsRunnerHint?: string): string {
  if (language === 'python') { return 'pytest'; }
  if (language === 'java') { return 'JUnit 5 (Jupiter) — use `org.junit.jupiter.api.Test`/`Assertions`, not JUnit 4\'s `org.junit.Test`'; }
  if (language === 'csharp') { return "this project's actual .NET test framework (xUnit's `[Fact]`, NUnit's `[Test]`, or MSTest's `[TestMethod]` — check the .csproj's PackageReference entries and existing test files to see which one it uses; default to xUnit with `[Fact]`/`Assert` if none is apparent)"; }
  if (language === 'go') { return 'go test — a `func TestXxx(t *testing.T)` function, using table-driven subtests (`t.Run`) for multiple cases rather than separate top-level functions, matching Go\'s own convention'; }
  if (language === 'rust') { return 'cargo test — a `#[test] fn` function (inside a `#[cfg(test)] mod tests` block if this project follows that convention, check existing tests)'; }
  if (language === 'kotlin') { return 'JUnit 5 (Jupiter) — use `org.junit.jupiter.api.Test`/`Assertions`, exactly as for this project\'s Java code'; }
  const jsRunnerLabel = jsRunnerHint ? JS_RUNNER_DISPLAY_NAMES[jsRunnerHint] : undefined;
  if (jsRunnerLabel) {
    return `${jsRunnerLabel} — this project already uses it (confirmed from package.json/config), so match its real import style and assertion syntax, consistent with any existing test files`;
  }
  return "this project's actual JS/TS test framework (Jest, Vitest, or Mocha — check package.json and existing test files to see which one it uses)";
}

/**
 * Java's filename MUST equal its public class name exactly (a compiler
 * requirement, not a convention), and Maven Surefire/Gradle's default test
 * discovery only picks up `**Test.java` — so unlike Python/JS's loose
 * `test_<kind>_<name>.ext` naming, a Java generated test needs a real,
 * unique, valid identifier ending in "Test". Keeps the kind word (chain/
 * newfunc/callpath/broadcoverage) as its own leading PascalCase segment so
 * generatorKindFromTestFileName below can still recognize it.
 */
function javaTestClassNameFor(namePrefix: string): string {
  const parts = namePrefix.split(/[^A-Za-z0-9]+/).filter(Boolean);
  const pascal = parts.map((p) => p.charAt(0).toUpperCase() + p.slice(1)).join('') || 'Generated';
  const validStart = /^[A-Za-z]/.test(pascal) ? pascal : `T${pascal}`;
  // "Codemd" leads every generated class name (CodemdCallpathFooTest, not
  // CallpathFooTest) so provenance is legible from the class/filename alone —
  // matches the "codemd_" prefix generatedTestFileNameFor gives Python/JS —
  // without it, a Java test copied out of .codemd/generated_tests/ or shown
  // in a bare test-runner listing gave no hint CODEMD wrote it.
  return validStart.endsWith('Test') ? `Codemd${validStart}` : `Codemd${validStart}Test`;
}

/**
 * A generation prompt line, Java-only (empty string otherwise, harmless in a
 * `.join(' ')`'d prompt array): tells Claude the EXACT class name it must
 * use, pre-computed here rather than left to Claude's own choice, since
 * writeGeneratedTest/generatedTestFileNameFor will save the file under this
 * exact name and Java requires the public class name to match the filename.
 */
function javaClassNameInstruction(language: TestLanguage, namePrefix: string): string {
  if (language !== 'java') { return ''; }
  const className = javaTestClassNameFor(namePrefix);
  return `The public test class MUST be named exactly \`${className}\` (it will be saved as ${className}.java, and Java requires the class name to match the filename exactly) — do not choose a different name.`;
}

/**
 * Repo-relative save filename for a generated test, one convention per
 * language: Python/JS's loose `test_<prefix>.<ext>` (their runners discover
 * by prefix/directory, not exact name), or Java's compiler-mandated
 * class-name-equals-filename via javaTestClassNameFor above. Centralized so
 * writeGeneratedTest and generateCallPathTestForResult's own inline save
 * logic can't drift into two different Java naming schemes.
 */
function generatedTestFileNameFor(namePrefix: string, language: TestLanguage | null, ext: string): string {
  if (language === 'java') {
    return `${javaTestClassNameFor(namePrefix)}${ext}`;
  }
  const safeName = namePrefix.replace(/[^A-Za-z0-9_]/g, '_');
  // "codemd_" makes provenance legible from the filename alone (e.g. moved
  // out of .codemd/generated_tests/, or listed by a bare test-runner report)
  // instead of relying entirely on directory location — generatorKindFromTestFileName
  // below still recognizes the older un-prefixed form for tests already on disk.
  return `test_codemd_${safeName}${ext}`;
}

/**
 * Repo-relative path a generated test WOULD land at if Claude itself wrote
 * it via writeGeneratedTest — used only by Prompt-Only Mode, which never
 * calls writeGeneratedTest (nothing comes back to CODEMD to write), so
 * Claude needs to be told this exact path up front in order to save the
 * file where CODEMD's Run/Fix buttons and test-status cache will actually
 * look for it. Callers pass the identical namePrefix/language/ext they'd
 * otherwise pass to writeGeneratedTest, so the two can never drift apart.
 */
function expectedGeneratedTestPath(folder: vscode.WorkspaceFolder, namePrefix: string, language: TestLanguage | null, ext: string): string {
  const outName = generatedTestFileNameFor(namePrefix, language, ext);
  return `${ARTIFACT_OUTPUT_DIR}/generated_tests/${outName}`;
}

/** Which of the five generators produced a test file, inferred from the
 * `test_<kind>_...` filename prefix (Python/JS) or the leading PascalCase
...
function generatorKindFromTestFileName(testFile: string): string {
  ...
  // Accepts both the current 'test_codemd_<kind>_'/'codemd<Kind>' naming
  // (generatedTestFileNameFor/javaTestClassNameFor) and the older
  // 'test_<kind>_'/'<kind>' form with no "codemd" segment, so tests already
  // on disk from before that prefix was added still get recognized.
  if (base.startsWith('test_codemd_callpath_') || base.startsWith('codemdcallpath')
    || base.startsWith('test_callpath_') || base.startsWith('callpath')) { return 'call_path'; }
  if (base.startsWith('test_codemd_newfunc_') || base.startsWith('codemdnewfunc')
    || base.startsWith('test_newfunc_') || base.startsWith('newfunc')) { return 'new_function'; }
  if (base.startsWith('test_codemd_chain_') || base.startsWith('codemdchain')
    || base.startsWith('test_chain_') || base.startsWith('chain')) { return 'call_chain'; }
  if (base.startsWith('test_codemd_contract_') || base.startsWith('codemdcontract')
    || base.startsWith('test_contract_') || base.startsWith('contract')) { return 'contract'; }
  if (base.startsWith('test_codemd_broadcoverage_') || base.startsWith('codemdbroadcoverage')
    || base.startsWith('test_broadcoverage_') || base.startsWith('broadcoverage')) { return 'broad_coverage'; }
  return '';
}

/** Friendly category label per generator kind, for grouping the "tests
 * found" list by test type instead of by raw file path — a bare
 * `.codemd/generated_tests/test_broadcoverage_foo.py` path means nothing to
 * a user skimming for what to look at first. */
const TEST_KIND_LABELS: Record<string, string> = {
  contract: 'Contract test — proven-broken caller',
  call_chain: 'Call-chain test — changed functions calling each other',
  call_path: 'Call-path test — changed functions calling each other',
  new_function: 'New-function test',
  broad_coverage: 'Multi-caller test — covers distinct calling patterns',
};

/** Display order for the labels above — proven-broken-call-site evidence
 * (contract/call-chain/call-path) ranks above generic coverage (new-function,
 * then broad-coverage across many callers), so the most concrete signal of
 * real breakage shows first. An unrecognized/hand-written test (kind '')
 * sorts last — least is known about how it relates to this specific change. */
const TEST_KIND_ORDER: Record<string, number> = {
  contract: 0,
  call_chain: 1,
  call_path: 1,
  new_function: 2,
...
function testKindRank(kind: string): number {
  return kind in TEST_KIND_ORDER ? TEST_KIND_ORDER[kind] : 4;
}

/** Output filename extension for a generated test, matching the source
 * file's own extension convention for JS/TS so it's picked up by that
 * project's test runner globs (e.g. `*.test.ts`). */
function generatedTestExtFor(file: string, language: TestLanguage): string {
  if (language === 'python') { return '.py'; }
  if (language === 'java') { return '.java'; }
  if (language === 'csharp') { return '.cs'; }
  if (language === 'go') { return '_test.go'; }
  if (language === 'rust') { return '.rs'; }
  if (language === 'kotlin') { return '.kt'; }
  const lower = file.toLowerCase();
  if (lower.endsWith('.tsx')) { return '.test.tsx'; }
  if (lower.endsWith('.ts') || lower.endsWith('.mts')) { return '.test.ts'; }
  if (lower.endsWith('.jsx')) { return '.test.jsx'; }
  return '.test.js';
}
 ...
 * claude -p response.
 */
function stripStrayClosingTag(text: string): string {
  let result = text;
  for (let i = 0; i < 5; i++) {
...
/**
 * Looks up exact call-site locations for a function's confirmed callers from
 * the cached callgraph artifact for its language (see callgraphArtifactFor —
 * a `caller`/`callee`/`file`/`line`/`call_text` record per real call edge,
 * however that language's pipeline stores it). Used to hand
 * generateCallPathTestForResult's Claude prompt precise "read this file at
 * this line" locations instead of bare symbol names — without this, Claude
 * has to Grep/Glob the whole repo to even find where a caller lives before
 * it can read how the function is actually invoked, which is exactly the
 * exploration that was burning turns and wall-clock. Returns an empty map
 * (never throws) if the artifact is missing or unreadable — callers fall
 * back to the old bare-name prompt.
 */
async function resolveCallSites(
  folder: vscode.WorkspaceFolder,
  ...
  const { relPath, key } = callgraphArtifactFor(language);
  const cgPath = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, relPath);
  let data: any;
  try {
  ...
  const orderedCalls: any[] = Array.isArray(data?.[key]) ? data[key] : [];
...
/**
 * Bounded, deterministic code around one call site — read directly here so
 * generateBroadCoverageTestForResult's prompt can hand Claude real caller
 * context up front instead of making it spend a Read tool call (a turn, and
 * real wall-clock time) fetching each caller itself. Same motivation as
 * resolveCallSites' own doc comment above, one step further: that function
 * stopped Claude from having to Grep/Glob to even find a caller; this stops
 * it from having to Read the caller once it knows where one is. Never
 * throws — an unreadable file (moved/deleted since the callgraph was built)
 * just yields no snippet, and the caller of this function skips it.
 */
function readCallSiteSnippet(folder: vscode.WorkspaceFolder, relFile: string, line: number, contextLines = 12): string {
  try {
    const absPath = path.join(folder.uri.fsPath, relFile);
    const lines = fs.readFileSync(absPath, 'utf8').split(/\r?\n/);
    if (line < 1 || line > lines.length) { return ''; }
    const startIdx = Math.max(0, line - 1 - contextLines);
    const endIdx = Math.min(lines.length, line + contextLines);
    return lines.slice(startIdx, endIdx).map((text, i) => `${startIdx + i + 1}: ${text}`).join('\n');
  } catch {
    return '';
  }
}

interface CallerSite extends CallSite {
  caller: string;
}

/**
 * All real callers of a symbol, discovered from scratch from the current
 * callgraph's ordered call list — unlike resolveCallSites above, which only
 * verifies a pre-known candidate list. Needed for a function that has no
 * pre-change impact data yet (e.g. one newly added in the current diff):
 * its real callers, if any, only exist in the just-regenerated callgraph.
 */
export async function findCallersOf(
  folder: vscode.WorkspaceFolder,
  language: TestLanguage,
  calleeSymbol: string,
  limit = 8,
 ...
 * per language actually present in `results` over its ordered call list, not
 * O(pairs), so this stays cheap even for a diff touching dozens of functions
 * across languages.
 */
async function findSameCommitCallChains(
  ...
  const languagesPresent = new Set<TestLanguage>();
  for (const r of results) {
    if (r.graphSymbol) {
      bySymbol.set(compactSymbolName(r.graphSymbol), r);
    }
    const language = r.file ? testLanguageFor(r.file) : null;
    if (language) { languagesPresent.add(language); }
  }
  const chains: SameCommitCallChain[] = [];
  const seenPairs = new Set<string>();
  for (const language of languagesPresent) {
    const { relPath, key } = callgraphArtifactFor(language);
    const cgPath = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, relPath);
    let data: any;
    try {
    ...
    for (const call of orderedCalls) {
      ...
      const callerModule = callerResult.file ? topLevelModuleFor(callerResult.file, language) : '';
      const calleeModule = calleeResult.file ? topLevelModuleFor(calleeResult.file, language) : '';
...
/**
 * Parses coverlet's Cobertura XML report (`--collect:"XPlat Code Coverage"`'s
 * default format) — no XML library in this project's dependencies, so this
 * targets the specific, well-documented Cobertura shape directly rather than
 * pulling one in: `<class filename="...">` blocks each containing
 * `<line number="N" hits="H" .../>` entries. Attributes are matched
 * independently within each tag (not by fixed order) since emitters aren't
 * guaranteed to write `number` before `hits`.
 */
function parseCoberturaCoverage(xmlText: string, targetPathPosix: string, rangeStart: number, rangeEnd: number): { executedInRange: number; trackedInRange: number } {
  let executedInRange = 0;
  let trackedInRange = 0;
  const classRe = /<class\b([^>]*)>([\s\S]*?)<\/class>/g;
  let classMatch: RegExpExecArray | null;
  while ((classMatch = classRe.exec(xmlText))) {
    const filenameMatch = classMatch[1].match(/\bfilename="([^"]*)"/);
    const filename = filenameMatch ? filenameMatch[1].replace(/\\/g, '/') : '';
    if (!filename || (filename !== targetPathPosix && !filename.endsWith(`/${targetPathPosix}`))) {
      continue;
    }
    const lineRe = /<line\b([^>]*)\/?>/g;
    let lineMatch: RegExpExecArray | null;
    while ((lineMatch = lineRe.exec(classMatch[2]))) {
      const numberMatch = lineMatch[1].match(/\bnumber="(\d+)"/);
      const hitsMatch = lineMatch[1].match(/\bhits="(\d+)"/);
      if (!numberMatch || !hitsMatch) { continue; }
      const lineNo = Number(numberMatch[1]);
      if (lineNo < rangeStart || lineNo > rangeEnd) { continue; }
      trackedInRange++;
      if (Number(hitsMatch[1]) > 0) { executedInRange++; }
    }
  }
  return { executedInRange, trackedInRange };
}
...
/**
 * Parses JaCoCo's own native XML report format (its `report` CLI command's
 * `--xml` output — a different schema from Cobertura, not shared with
 * coverlet). Structure: `<package name="com/example">` containing
 * `<sourcefile name="Foo.java">` containing `<line nr="N" ci="COVERED_INSTR"
 * .../>` — `ci` (covered instructions) > 0 means the line executed.
 * `sourcefile`'s name is bare (no path), so the real relative path is the
 * enclosing package's `/`-separated name plus the filename — matched as a
 * suffix of the full target path (e.g. "com/example/Foo.java" against
 * "src/main/java/com/example/Foo.java"), same boundary discipline as the
 * Go/Cobertura matchers.
 */
function parseJacocoXmlReport(xmlText: string, targetPathPosix: string, rangeStart: number, rangeEnd: number): { executedInRange: number; trackedInRange: number } {
  let executedInRange = 0;
  let trackedInRange = 0;
  const packageRe = /<package\b([^>]*)>([\s\S]*?)<\/package>/g;
  let packageMatch: RegExpExecArray | null;
  while ((packageMatch = packageRe.exec(xmlText))) {
    const packageNameMatch = packageMatch[1].match(/\bname="([^"]*)"/);
    const packagePath = packageNameMatch ? packageNameMatch[1] : '';
    const sourcefileRe = /<sourcefile\b([^>]*)>([\s\S]*?)<\/sourcefile>/g;
    let sfMatch: RegExpExecArray | null;
    while ((sfMatch = sourcefileRe.exec(packageMatch[2]))) {
      const nameMatch = sfMatch[1].match(/\bname="([^"]*)"/);
      const sourceFileName = nameMatch ? nameMatch[1] : '';
      if (!sourceFileName) { continue; }
      const combinedPath = packagePath ? `${packagePath}/${sourceFileName}` : sourceFileName;
      if (!(targetPathPosix === combinedPath || targetPathPosix.endsWith(`/${combinedPath}`))) {
        continue;
      }
      const lineRe = /<line\b([^>]*)\/?>/g;
      let lineMatch: RegExpExecArray | null;
      while ((lineMatch = lineRe.exec(sfMatch[2]))) {
        const nrMatch = lineMatch[1].match(/\bnr="(\d+)"/);
        if (!nrMatch) { continue; }
        const lineNo = Number(nrMatch[1]);
        if (lineNo < rangeStart || lineNo > rangeEnd) { continue; }
        const ciMatch = lineMatch[1].match(/\bci="(\d+)"/);
        trackedInRange++;
        if (Number(ciMatch?.[1] || 0) > 0) { executedInRange++; }
      }
    }
  }
  return { executedInRange, trackedInRange };
...
// coverlet's XPlat collector writes its report to an unpredictable
// `<resultsDir>/<run-guid>/coverage.cobertura.xml` path (a fresh GUID
// subfolder per run), so the caller can't just construct the path directly —
// needs an actual (small, bounded) recursive search under the results dir.
...
/** Same upward walk as findCargoRootDir, but for whichever Java build tool
 * this repo actually uses — needed because a CODEMD-generated test lives
 * under .codemd/generated_tests/, not inside src/(main|test)/java, so
 * "which build root does this belong to" can't be inferred from the target
 * source file's own path the way Cargo.toml lookup can. */
function findJavaBuildRootDir(folder: vscode.WorkspaceFolder): { root: string; tool: 'maven' | 'gradle' } | null {
  const root = folder.uri.fsPath;
  if (fs.existsSync(path.join(root, 'pom.xml'))) { return { root, tool: 'maven' }; }
  if (fs.existsSync(path.join(root, 'build.gradle')) || fs.existsSync(path.join(root, 'build.gradle.kts'))) {
    return { root, tool: 'gradle' };
  }
  return null;
}

/** Walks upward from a C# file's directory to find the nearest .csproj —
 * needed as the ProjectReference target for the throwaway test project
 * runCsharpTestFileForResult builds (see its doc comment for why C# gets a
 * genuinely different, simpler strategy than Java's). */
function findCsprojFor(absFilePath: string): string | null {
  let dir = path.dirname(absFilePath);
  for (let i = 0; i < 20; i++) {
    if (fs.existsSync(dir)) {
      const csproj = fs.readdirSync(dir).find((f) => f.toLowerCase().endsWith('.csproj'));
...
/** First `<TargetFramework>` (or `<TargetFrameworks>`, taking the first of
 * the semicolon list) declared in a .csproj — the throwaway test project
 * must target something compatible to reference it at all. Falls back to a
 * reasonably current default rather than failing outright when the csproj
 * uses an SDK feature (Directory.Build.props, MSBuild conditionals) this
 * simple regex read can't follow. */
function targetFrameworkFor(csprojPath: string): string {
  try {
    const text = fs.readFileSync(csprojPath, 'utf8');
    const single = text.match(/<TargetFramework>([^<]+)<\/TargetFramework>/);
    if (single) { return single[1].trim(); }
    const multi = text.match(/<TargetFrameworks>([^<]+)<\/TargetFrameworks>/);
...
/**
 * Cheap, filesystem-only check for whether a Java repo has ANY test
 * framework declared at all — the same job detectJsTestRunner does for
 * JS/TS. A real XML/Groovy/Kotlin-DSL parse is overkill for a yes/no
 * presence check, so this just looks for the coordinate text in the build
 * file — false positives (a commented-out dependency) are harmless here,
 * they just mean "Set up test runner" won't offer to add a second one.
 */
function detectJavaTestRunner(folder: vscode.WorkspaceFolder): { found: boolean; runner?: string } {
  const buildRoot = findJavaBuildRootDir(folder);
  if (!buildRoot) { return { found: false }; }
  const buildFile = buildRoot.tool === 'maven'
    ? 'pom.xml'
    : (fs.existsSync(path.join(buildRoot.root, 'build.gradle.kts')) ? 'build.gradle.kts' : 'build.gradle');
  let text = '';
  try {
    text = fs.readFileSync(path.join(buildRoot.root, buildFile), 'utf8');
  } catch {
    return { found: false };
  }
  if (/junit-jupiter|org\.junit\.jupiter/.test(text)) { return { found: true, runner: 'JUnit 5' }; }
  if (/<artifactId>\s*junit\s*<\/artifactId>|['"]junit:junit['"]/.test(text)) { return { found: true, runner: 'JUnit 4' }; }
  if (/testng/i.test(text)) { return { found: true, runner: 'TestNG' }; }
  return { found: false };
}

/**
 * Inserts a `<dependency>` block for a single, fixed JUnit 5 coordinate into
 * pom.xml's `<dependencies>` (creating that element if the pom has none at
 * all — rare, but a bare-bones pom.xml is valid). Deliberately a plain text
 * insertion, not a real XML parse/serialize round-trip — that would risk
 * reformatting the whole file (reordering attributes, changing quote style,
 * collapsing whitespace) for a one-element change. Returns null only when
 * neither anchor (`</dependencies>` or `</project>`) is found at all, which
 * means the file is malformed enough that guessing at an edit would be
 * worse than reporting "couldn't automatically edit pom.xml".
 */
function insertMavenJUnitDependency(pomText: string): string | null {
  const snippet = [
    '    <dependency>',
  ...
  if (pomText.includes('</dependencies>')) {
    return pomText.replace('</dependencies>', `${snippet}</dependencies>`);
  }
  if (pomText.includes('</project>')) {
    const block = `  <dependencies>\n${snippet}</dependencies>\n`;
    return pomText.replace('</project>', `${block}</project>`);
  }
  return null;
}

/**
 * Same job as insertMavenJUnitDependency, for build.gradle(.kts). Gradle,
 * unlike Maven Surefire, does NOT auto-detect JUnit 5 from the dependency
 * alone — `test { useJUnitPlatform() }` is required or Gradle silently uses
 * the legacy JUnit 4 runner and never discovers Jupiter tests at all, so
 * this adds both, each only if not already present.
 */
export function insertGradleJUnitDependency(buildText: string): string {
  let result = buildText;
  if (!/junit-jupiter|org\.junit\.jupiter/.test(result)) {
    const depLine = "    testImplementation 'org.junit.jupiter:junit-jupiter:5.10.2'\n";
    const depBlock = result.match(/dependencies\s*\{/);
    if (depBlock && typeof depBlock.index === 'number') {
      const at = depBlock.index + depBlock[0].length;
      result = `${result.slice(0, at)}\n${depLine}${result.slice(at)}`;
    } else {
      result += `\ndependencies {\n${depLine}}\n`;
    }
  }
  if (!/test\s*\{[^}]*useJUnitPlatform/s.test(result)) {
    const testBlock = result.match(/\btest\s*\{/);
    if (testBlock && typeof testBlock.index === 'number') {
      const at = testBlock.index + testBlock[0].length;
      result = `${result.slice(0, at)}\n    useJUnitPlatform()\n${result.slice(at)}`;
    } else {
      result += '\ntest {\n    useJUnitPlatform()\n}\n';
    }
...
async function resolveCommitParentRef(hash: string, cwd: string): Promise<string> {
  const result = await execGitAsync(['rev-parse', '--verify', `${hash}^`], cwd);
  return result.status === 0 ? (result.stdout || '').trim() : EMPTY_TREE_SHA;
}

async function computeGitStateHash(workspaceRoot: string): Promise<string | null> {
...
interface NormalizedSearchResult {
  ...
  impactScore?: number;
  changeTime?: number;
  // When set, the webview renders each entry as its own clickable file row
  // instead of a single whole-item click — used by "Latest Commits" so a
  // multi-file commit lets you pick which file to open instead of always
  // jumping to an arbitrary first one.
  fileList?: string[];
  fileListMore?: number;
  // Set on a "Latest Commits" row itself (not its per-file rows) — clicking
  // the row runs the same impact/callgraph analysis "Check Uncommitted
  // Edits" does, just diffing this commit against its parent instead of the
  // working tree against HEAD.
  commitHash?: string;
  // Explicit ref pair a "View diff"/graph click should diff, overriding the
  // default HEAD-vs-working-tree behavior — set on results produced by a
  // commit analysis so the diff shown matches that commit, not live edits.
  diffBase?: string;
  diffTarget?: string;
  // Disk-backed test-status cache facts for this symbol (see
  // validCachedGeneratedKinds/hasValidCachedPassingRun) — lets the webview
  // hide a generator chip whose test already exists even on a fresh
  // session, instead of relying only on generatedTestsBySymbol (in-memory,
  // reset on every reload).
  testStatus?: { generatedKinds: string[]; anyValidPassingRun: boolean };
}

interface ChangeCard {
  kind: 'modified' | 'removed' | 'added' | 'files' | 'blastRadius';
  title: string;
  symbol?: string;
  change: string;
  risk: string;
  ...
  evidence: string[];
  checks: string[];
  signatureDetails?: { oldSignature?: string; newSignature?: string };
  // Confirmed-impact node names (compact) — the "N Confirmed impact" metric's
  // backing list. Kept out of Evidence/Recommended checks (which stay
  // action-oriented) and rendered collapsed behind a "Show N impacted
  // functions" toggle instead, so a wide fan-out doesn't dump 30 names
  // in front of the user by default.
  impactedFunctions?: string[];
  // Real, literal direct callers only (level 1 in the impact radius, high
  // confidence) — unlike impactedFunctions' full multi-hop transitive
  // radius, these actually have a call site to resolve, so anything that
  // claims to read/generate from "confirmed caller(s)" (the no-tests-found
  // note, Multi-Caller/contract test generation) should use this instead.
  directCallers?: string[];
  actions: string[];
  startCollapsed?: boolean;
  // True only for confirmed-cosmetic modified functions (see
  // isCosmeticOnlyChange) — lets the webview route these into their own
  // "Cosmetic-only Changes" group instead of the generic low-impact one,
  // even though both currently share riskLevel 'low'.
  cosmeticOnly?: boolean;
  // These two flag the cards that are actually worth reading individually —
  // everything else on a 'modified' card is impact-score noise (fan-in
  // based, not a proven finding) and gets folded into a collapsed group by
...
function uncommittedFilePath(file: any): string {
...
function uncommittedFileLabel(file: any): string {
  if (!file || typeof file !== 'object') {
    return String(file || '');
  }
...
function uncommittedFileStatus(file: any): string {
  if (file && typeof file === 'object') {
...
function deletedFolderSummaries(files: any[]): string[] {
...
function parseEvidenceFileLabel(label: string): { file: string; status: string } {
...
function fileRiskSignals(file: any): string[] {
...
function assessFileChangeRisk(files: any[]): { label: string; level: ChangeCard['riskLevel']; score: number; signals: string[] } {
  let score = 0;
...
function nodeConfidenceFromChange(change: any): Record<string, string> {
  const raw = change?.node_confidence;
  return raw && typeof raw === 'object' ? raw : {};
}

function confirmedAndInferredNodes(
  impactedNodes: string[],
  nodeConfidence: Record<string, string>,
): { confirmed: string[]; inferred: string[] } {
...
function directCallersFromLevels(change: any): number {
...
function directCallerNamesFromChange(change: any, nodeConfidence: Record<string, string>): string[] {
  const levels = change?.levels || {};
  return Object.keys(levels).filter((node) => Number(levels[node]) === 1 && nodeConfidence[node] === 'high');
}

function impactScoreForModifiedChange(change: any): number {
...
function impactScoreForDeletedChange(change: any): number {
...
function impactScoreForAddedChange(_change: any): number {
  return 1;
}
...
function pickHighestImpactGraphable(results: NormalizedSearchResult[]): NormalizedSearchResult | undefined {
  return [...results]
...
function isCosmeticOnlyChange(change: any): boolean {
...
interface SignatureDiff {
  ...
  removed: string[];
  starArgsChanged: boolean;
  starKwargsChanged: boolean;
  oldSignatureText: string;
  newSignatureText: string;
}

function signatureDiffFromChange(change: any): SignatureDiff {
  ...
  const toStrings = (v: unknown) => (Array.isArray(v) ? v.map((item) => String(item || '')).filter(Boolean) : []);
  return {
...
function callSiteIssuesFromChange(change: any): CallSiteIssue[] {
  ...
  return raw
    ...
    .filter((issue: CallSiteIssue) => issue.caller && issue.reason);
}
...
function confirmedEvidenceLines(impactedNodes: string[], nodeConfidence: Record<string, string>): string[] {
  ...
  if (inferred.length) {
    lines.push(
      `(+ ${inferred.length} more possible/unconfirmed — heuristic or name-matched edges, e.g. dynamic dispatch; not a traced call site)`,
    );
  }
  return lines;
}
...
function signatureDiffLine(diff: SignatureDiff): string | null {
  if (!diff.changed) {
    return null;
  }
  const parts: string[] = [];
  if (diff.addedRequired.length) { parts.push(`added required parameter${diff.addedRequired.length === 1 ? '' : 's'}: ${diff.addedRequired.join(', ')}`); }
  if (diff.addedOptional.length) { parts.push(`added optional parameter${diff.addedOptional.length === 1 ? '' : 's'}: ${diff.addedOptional.join(', ')}`); }
  if (diff.removed.length) { parts.push(`removed parameter${diff.removed.length === 1 ? '' : 's'}: ${diff.removed.join(', ')}`); }
  if (diff.starArgsChanged) { parts.push('*args changed'); }
  if (diff.starKwargsChanged) { parts.push('**kwargs changed'); }
...
function signatureTextLines(diff: SignatureDiff): string[] {
  if (!diff.changed) {
    return [];
  }
  const lines: string[] = [];
...
function signatureDetails(diff: SignatureDiff): { oldSignature?: string; newSignature?: string } | undefined {
  if (!diff.changed) {
    return undefined;
  }
  const details = {
...
function buildModifiedChangeCard(change: any): ChangeCard {
  ...
  return {
    ...
    evidence: [
      ...callSiteIssues.map((issue) =>
        `Broken call: ${compactSymbolName(issue.caller)} (${issue.file}${issue.line ? ':' + issue.line : ''}) — ${issue.reason}`),
...
function buildAddedChangeCard(change: any): ChangeCard {
  const symbol = String(change?.symbol || '');
  return {
    kind: 'added',
    title: 'New function',
...
function buildOtherFilesCard(files: any[]): ChangeCard {
...
function buildChangesAnswer(report: any): string {
  ...
  // These two counts are statically PROVEN facts (from python_function_
  ...
  // buried under the raw "118 functions changed" count. Pure counts (functions
  // touched, new functions, impacted files, uncommitted files, ...) are
  // surfaced as summary tiles instead (see buildChangesSummaryTiles) — this
  // text is reserved for what a tile can't hold: which specific functions.
  const breaking = modified.filter((m: any) => callSiteIssuesFromChange(m).length > 0);
  const unprovenSignatureChanges = modified.filter(
    (m: any) => !isCosmeticOnlyChange(m) && callSiteIssuesFromChange(m).length === 0 && signatureDiffFromChange(m).changed,
  );

  const namesFor = (list: any[]) => {
    const names = list.slice(0, 5).map((m: any) => compactSymbolName(String(m?.symbol || '')));
...
/**
 * Numeric facts from the same report/results/callChains buildChangesAnswer
 * and buildTestPlanSummaryLine already compute, reshaped into "at a glance"
 * tiles instead of prose — call-path opportunities lead, matching the
 * priority order in buildTestPlanSummaryLine's own doc comment.
 */
function buildChangesSummaryTiles(
  report: any,
  results: NormalizedSearchResult[],
  callChains: SameCommitCallChain[],
): { label: string; value: number; tone: 'high' | 'primary' | 'info' | 'neutral' }[] {
  const modified = Array.isArray(report?.modified) ? report.modified : [];
  const deleted = Array.isArray(report?.deleted) ? report.deleted : [];
  const unsupported = Array.isArray(report?.unsupported_files) ? report.unsupported_files : [];
  ...
  const modifiedTestCount = results.filter((r) => r.changeCard?.kind === 'modified').length;

  const tiles: { label: string; value: number; tone: 'high' | 'primary' | 'info' | 'neutral' }[] = [];
  if (breaking.length) {
    tiles.push({ label: 'Breaking changes', value: breaking.length, tone: 'high' });
  }
  if (callChains.length) {
    tiles.push({ label: 'Call-path opportunities', value: callChains.length, tone: 'primary' });
  }
  if (newFunctionCount) {
    tiles.push({ label: 'New functions', value: newFunctionCount, tone: 'info' });
  }
  if (modifiedTestCount) {
    tiles.push({ label: 'Modified functions', value: modifiedTestCount, tone: 'info' });
  }
  tiles.push({ label: 'Functions touched', value: modified.length + deleted.length, tone: 'neutral' });
  if (impactedFileSet.size) {
    tiles.push({ label: 'Impacted files', value: impactedFileSet.size, tone: 'neutral' });
  }
  if (unsupported.length) {
    tiles.push({ label: 'Other changed files', value: unsupported.length, tone: 'neutral' });
  }
  if (uncommittedFiles.length) {
    tiles.push({ label: 'Uncommitted files', value: uncommittedFiles.length, tone: 'neutral' });
  }
  return tiles;
}
...
/**
 * The chain-test generator (generateCallChainTestForResult -> writeGeneratedTest)
 * always names its output test_chain_<callerTail>_<calleeTail><ext> with no
 * timestamp/uniqueness suffix, so a prior generation for this exact pair is
 * cheap to detect by just recomputing that same deterministic path — no need
 * to search file contents. Returns '' when the pair hasn't been generated
 * (or spans two languages, which the generator itself refuses).
 */
function existingCallChainTestFile(folder: vscode.WorkspaceFolder, chain: SameCommitCallChain): string {
  const callerFile = chain.callerResult.file;
  const calleeFile = chain.calleeResult.file;
  const language = testLanguageFor(callerFile);
  if (!language || language !== testLanguageFor(calleeFile)) {
    return '';
  }
  const callerSymbol = chain.callerResult.graphSymbol || '';
  const calleeSymbol = chain.calleeResult.graphSymbol || '';
  const callerTail = callerSymbol.split('.').pop() || callerSymbol;
  const calleeTail = calleeSymbol.split('.').pop() || calleeSymbol;
  const ext = generatedTestExtFor(callerFile, language);
  const fileName = generatedTestFileNameFor(`chain_${callerTail}_${calleeTail}`, language, ext);
  const outPath = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'generated_tests', fileName);
  return fs.existsSync(outPath) ? path.relative(folder.uri.fsPath, outPath).replace(/\\/g, '/') : '';
}

function serializeCallChains(chains: SameCommitCallChain[], folder: vscode.WorkspaceFolder): any[] {
  return chains.map((c) => ({
    ...
    existingTestPath: existingCallChainTestFile(folder, c),
  }));
}

function buildChangesAnswerLinks(report: any, diffRefs?: { base: string; target: string }): AnswerFunctionLink[] {
  const modified = Array.isArray(report?.modified) ? report.modified : [];
  const signatureChanges = modified.filter(
    (m: any) => !isCosmeticOnlyChange(m) && signatureDiffFromChange(m).changed,
  );
  return signatureChanges.map((change: any) => {
    const symbol = String(change?.symbol || '');
...
export function mcpSetupHelpText(folder: vscode.WorkspaceFolder, context: vscode.ExtensionContext, targets: McpSetupTarget[] = ['claude', 'codex']): string {
  ...
  lines.push(
    ...
    '',
    // Deliberately a suggestion to copy, not an automatic edit — CODEMD never
    // writes into a file the user owns/maintains without an explicit action
    // naming that file specifically (unlike .mcp.json above, which this same
    // button already writes). A CLAUDE.md/AGENTS.md this terse is safe to
    // paste into an existing file without clobbering whatever is already
    // there, so leaving it manual keeps that choice with the user.
    'Optional: for CODEMD\'s testing tools specifically, consider adding this short pointer to your project\'s CLAUDE.md/AGENTS.md',
    '(after Regenerate has run at least once, so .codemd/agent_policy/testing.md exists):',
    '"When working with CODEMD testing, use Claude for software-engineering judgment and CODEMD for deterministic testing machinery. '
      + 'CODEMD normally handles change detection, callgraph traversal, test discovery/selection, known test commands, execution, '
      + 'result parsing, and caching. Use Claude to design meaningful tests, reason about caller contracts and assertions, diagnose '
      + 'failures, distinguish code/test/environment problems, and decide what evidence is needed to resolve ambiguous results. '
      + 'When CODEMD provides task-specific guidance through MCP or .codemd/agent_policy/testing.md, follow it and keep the '
      + 'investigation focused on the selected testing problem."',
  );
  return lines.join('\n');
}
...
class GraphsViewProvider implements vscode.WebviewViewProvider {
  ...
  private lastFeatureCatalog: any = null;
  private lastFeatureCatalogRepoStats: any = null;
  ...
  private activeFixCancelers = new Map<string, () => void>();
  // Same idea as activeFixCancelers above, but for the write-only test
  // generators (call-path, new-function, call-chain, contract,
  // broad-coverage/multi-caller) — a function with many confirmed callers
  // (broad-coverage reads several caller snippets, call-path can end up
  // scanning a large/high-fan-in file) can legitimately run for minutes with
  // no way to stop it short of reloading the webview, which the Fix flow
  // already solved for its own long-running Claude calls.
  private activeTestGenCancelers = new Map<string, () => void>();
  ...
  private pendingAutoAwaits = new Map<string, (msg: any) => void>();
  ...
  private sessionUsageTotals: ClaudeUsageTotals = { tokens: 0, ms: 0 };
  private autoModeRunning = false;
  private autoModeCancelRequested = false;
  private autoModeRequestSeq = 0;
  ...
  private autoModeCompletedItemKeys = new Set<string>();
  private initialGraphPosted = false;
  private initialGraphPostWaiters: Array<() => void> = [];
  ...
  private openGraphPanel(currentGraphUrl?: string): void {
    ...
    const url =
      this.normalizeCurrentServerGraphUrl(currentGraphUrl) ||
  ...
  private normalizeCurrentServerGraphUrl(url: unknown): string {
  ...
  private async refreshMcpUsage(folder = vscode.workspace.workspaceFolders?.[0]): Promise<void> {
    if (!folder) {
  ...
  private ensureLocalGraphLoaded(folder?: vscode.WorkspaceFolder): void {
    ...
    outputChannel?.appendLine(
      `[ensureLocalGraphLoaded] candidate=${candidate.fsPath} exists=${candidateExists} `
  ...
  private markInitialGraphPosted(): void {
  ...
  private async waitForInitialGraphPost(timeoutMs = 2000): Promise<void> {
    if (this.initialGraphPosted) {
      return;
    }
  ...
  async runChangesCheckAfterInitialGraphPost(options: { focusHighestImpact?: boolean; quietIfBusy?: boolean } = {}): Promise<void> {
  ...
  private scimFunctionSuggestions(folder = vscode.workspace.workspaceFolders?.[0]): string[] {
    if (!folder) {
      return [];
    }
  ...
  private postSearchSuggestions(folder = vscode.workspace.workspaceFolders?.[0]): void {
   ...
   */
  private postFeatureCatalog(analyzeResult: any): void {
  ...
  private accumulateClaudeUsage(message: any): void {
    if (!message || !HOST_USAGE_MESSAGE_TYPES.has(message.type)) { return; }
    ...
    if (!tokens && !ms) { return; }
    const totals = this.context.globalState.get<ClaudeUsageTotals>(CLAUDE_USAGE_TOTALS_KEY) || { tokens: 0, ms: 0 };
    const updated: ClaudeUsageTotals = { tokens: totals.tokens + tokens, ms: totals.ms + ms };
  ...
  private postClaudeUsageTotals(totals?: ClaudeUsageTotals): void {
    const value = totals || this.context.globalState.get<ClaudeUsageTotals>(CLAUDE_USAGE_TOTALS_KEY) || { tokens: 0, ms: 0 };
    this.post({
  ...
  private resetClaudeUsageTotals(): void {
    const zero: ClaudeUsageTotals = { tokens: 0, ms: 0 };
    this.context.globalState.update(CLAUDE_USAGE_TOTALS_KEY, zero);
    this.postClaudeUsageTotals(zero);
  }

  private awaitPostedResult(requestId: string, timeoutMs = 300000): Promise<any> {
    return new Promise((resolve) => {
  ...
  private reportsNoticeShown = false;
  private reportWriteQueue: Promise<void> = Promise.resolve();

  // requestId -> start time, set when a test-run request is first dispatched
  // (runTestFileForResult / runTestViaClaudeForResult) and consumed once in
  // postTestRunResult to compute how long the run actually took, without
  // threading a timer through every one of the ~50 postTestRunResult call
  // sites across all the per-language runners.
  private testRunStartTimes = new Map<string, number>();

  private maybeAppendReportEntry(message: any): void {
  ...
  private async writeReportEntry(folder: vscode.WorkspaceFolder, markdown: string): Promise<void> {
    ...
    if (!fs.existsSync(reportPath)) {
      const header = '# CODEMD Activity Report\n\n'
        ...
        + 'The "$X.XX API-equivalent usage" figures below are not real charges — nothing is billed per call on a Claude Pro/Max subscription. '
        + 'They\'re just an estimate of what each call would have cost under Claude API / Claude Console pay-as-you-go pricing, for anyone who wants a comparison number.\n\n'
        + '---\n\n';
    ...
    if (!this.reportsNoticeShown) {
      ...
      vscode.window.showInformationMessage(
        'CODEMD is logging test runs, generated tests, and fixes to .codemd/reports/activity.md.',
        'Open Report',
      ).then((choice) => {
  ...
  private isReadOnlyMode(): boolean {
    return vscode.workspace.getConfiguration('codemdGraphs').get<boolean>('readOnlyMode', false) === true;
  }
   ...
   */
  private maxCostPerActionUsd(): number {
    const configured = vscode.workspace.getConfiguration('codemdGraphs').get<number>('maxCostPerActionUsd', 1);
   ...
   */
  private postModesConfig(): void {
    const config = vscode.workspace.getConfiguration('codemdGraphs');
    const maxAutoWritesPerBatch = config.get<number>('maxAutoWritesPerBatch', 3);
    this.post({
      type: 'modesConfig',
      autoMode: config.get<string>('autoMode', 'manual') === 'localOnly' ? 'localOnly' : 'manual',
      ...
      claudeAgentConsent: config.get<boolean>('claudeAgentConsent', false) === true,
      claudeAgentMode: this.getClaudeAgentMode(),
    });
  }

  private async setAutoMode(value: any): Promise<void> {
    const config = vscode.workspace.getConfiguration('codemdGraphs');
  ...
  /**
   * Tri-state read behind every Claude-powered action: 'manual' (nothing
   * runs), 'promptOnly' (CODEMD types the prompt into the user's own
   * `claude` terminal instead of spawning a subprocess — see
   * sendPromptToUsersClaudeTerminal), or 'agent' (CODEMD spawns `claude`
   * itself, as before this mode existed). Falls back to the pre-existing
   * boolean codemdGraphs.claudeAgentConsent ONLY when claudeAgentMode has
   * never been explicitly set, so a user who already turned on "Use Coding
   * Agent" before this mode shipped keeps behaving as 'agent' on upgrade
   * instead of silently reverting to 'manual'.
   */
  private getClaudeAgentMode(): ClaudeAgentMode {
    const config = vscode.workspace.getConfiguration('codemdGraphs');
    const inspected = config.inspect<string>('claudeAgentMode');
    const explicitlySet = Boolean(inspected && (
      inspected.workspaceValue !== undefined
      || inspected.globalValue !== undefined
      || inspected.workspaceFolderValue !== undefined
    ));
    if (explicitlySet) {
      const value = config.get<string>('claudeAgentMode', 'manual');
      if (value === 'promptOnly' || value === 'agent' || value === 'manual') {
        return value;
      }
    }
    return config.get<boolean>('claudeAgentConsent', false) === true ? 'agent' : 'manual';
  }

  /**
   * Direct setter behind the unified Modes popup's Manual / Prompt-Only /
   * Use Coding Agent rows — the popup itself is now the confirmation UI
   * (icon + title + description per row, like Claude Code's own mode
   * picker), so unlike the old claudeConsentBtn pill this no longer needs to
   * open a second, native QuickPick just to ask the same question again.
   * Keeps the legacy claudeAgentConsent boolean in sync (true only for
   * 'agent') in case anything else still reads it directly.
   */
  private async setClaudeAgentMode(value: any): Promise<void> {
    const mode: ClaudeAgentMode = value === 'promptOnly' ? 'promptOnly' : value === 'agent' ? 'agent' : 'manual';
    const config = vscode.workspace.getConfiguration('codemdGraphs');
    await config.update('claudeAgentMode', mode, vscode.ConfigurationTarget.Workspace);
    await config.update('claudeAgentConsent', mode === 'agent', vscode.ConfigurationTarget.Workspace);
    this.postModesConfig();
  }

  /**
   * Gate in front of every Claude SUBPROCESS invocation (test generation,
   * run-command discovery, Fix) — NOT called at all in 'promptOnly' mode,
   * since that mode never spawns a subprocess to consent to (see each call
   * site's own mode branch). The first time an 'agent'-eligible action is
   * actually about to run in 'manual' mode, this shows a one-time popup —
   * mirroring pickMcpSetupTargets's QuickPick — instead of spending a Claude
   * call on implicit consent. Picking "Use Coding Agent" persists approval
   * (both claudeAgentConsent and claudeAgentMode) so every future Claude
   * action proceeds without asking again; picking Manual (or dismissing)
   * only skips this one click and deliberately does NOT persist a refusal,
   * so the next Claude-powered click asks again rather than silently
   * blocking the feature forever — same "don't remember no" choice
   * requestClaudeMcpApproval already makes for MCP approval.
   */
  private async ensureClaudeAgentConsent(): Promise<boolean> {
    if (CLAUDE_AGENT_MODE_DISABLED) {
      return false;
    }
    const config = vscode.workspace.getConfiguration('codemdGraphs');
    if (config.get<boolean>('claudeAgentConsent', false) === true) {
      return true;
    }
    const items: Array<vscode.QuickPickItem & { allow: boolean }> = [
      {
        label: '✋ Manual',
    ...
    const picked = await vscode.window.showQuickPick(items, {
      ...
      placeHolder: 'For a CODEMD UI, Manual is the safer default — it clearly contrasts with Use Coding Agent (Claude). Prompt-Only Mode is available from the Modes menu if you want Claude\'s help without CODEMD spending any of your usage.',
      ignoreFocusOut: true,
    });
    if (!picked?.allow) {
      return false;
    ...
    await config.update('claudeAgentMode', 'agent', vscode.ConfigurationTarget.Workspace);
    this.postModesConfig();
    return true;
  }
   ...
   * copy somewhere outside the webview. `message` (the original request this
   * result answers) is optional; used to record a defect (recordDefectIfGenuine)
   * and to enrich the .codemd/reports/activity.md entry (formatReportEntryMarkdown)
   * with the test file/target/duration, none of which the per-language runners
   * put on `payload` themselves.
   */
  private postTestRunResult(payload: any, message?: any): void {
    ...
    const startedAt = requestId ? this.testRunStartTimes.get(requestId) : undefined;
    if (requestId) { this.testRunStartTimes.delete(requestId); }
    const durationMs = typeof payload?.durationMs === 'number' ? payload.durationMs
      : typeof startedAt === 'number' ? Date.now() - startedAt : undefined;
    const defectRecorded = this.recordDefectIfGenuine(payload, message);
    const testFile = String(payload?.file || message?.file || '');
    const targetPath = String(payload?.targetPath || message?.targetPath || '');
    const targetSymbol = String(payload?.targetSymbol || message?.targetSymbol || '');
    // Every run path (Python/Go/Rust/Java/C#/Kotlin direct, Claude-assisted,
    // Auto Mode, existing-tests check, a manual Run click) posts through
    // here, so this is the one place to record into the disk-backed
    // test-status cache — see hasValidCachedPassingRun/recordTestRunInCache
    // above — instead of threading a write call through every call site.
    if (payload?.ok !== false && testFile && targetPath && targetSymbol) {
      const folder = vscode.workspace.workspaceFolders?.[0];
      if (folder) {
        const kind = generatorKindFromTestFileName(testFile) || 'existing';
        recordTestRunInCache(folder, targetPath, targetSymbol, kind, testFile, {
    ...
    this.post({
      ...
      ...payload,
      durationMs,
      file: payload?.file || message?.file || undefined,
      targetSymbol: payload?.targetSymbol || message?.targetSymbol || undefined,
      targetPath: payload?.targetPath || message?.targetPath || undefined,
      defectRecorded: defectRecorded || undefined,
    });
  }

  private defectStatusFilePath(folder: vscode.WorkspaceFolder): string {
  ...
  private defectKey(targetPath: string, targetSymbol: string): string {
    return `${targetPath.replace(/\\/g, '/')}::${targetSymbol}`;
  }

  private readDefectStatusIndex(folder: vscode.WorkspaceFolder): Record<string, { status: 'active' | 'fixed'; kind?: 'code' | 'test'; fixedAt?: string; lastSeenAt?: string }> {
    try {
  ...
  private writeDefectStatusIndex(folder: vscode.WorkspaceFolder, data: Record<string, unknown>): void {
    try {
   ...
   */
  private recordDefectIfGenuine(payload: any, message: any): string | null {
    if (!message || payload?.ok !== true || payload?.passed !== false || payload?.environmentIssue) {
      return null;
    }
    ...
    try {
      ...
      const safeSymbol = targetSymbol.replace(/[^A-Za-z0-9_]/g, '_').slice(0, 80) || 'symbol';
      const outPath = path.join(defectsDir, `${timestamp.replace(/[:.]/g, '-')}_${safeSymbol}.json`);
      const record = {
        timestamp,
        generator,
        targetSymbol,
        targetPath,
        targetStartLine: Number(message?.targetStartLine || 0) || undefined,
        testFile,
   ...
   * assumption in the generated test itself — each row carries its `kind`
   * ('code' | 'test', undefined while still active and unclassified) so the
   * UI can label it. A group with no status entry yet defaults to 'active'.
   */
  private postDefects(): void {
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder) {
    ...
    try {
      ...
      fs.readdirSync(defectsDir)
        ...
        .forEach((name) => {
          try {
            ...
            if (!targetSymbol || !targetPath) { return; }
            const key = this.defectKey(targetPath, targetSymbol);
      ...
      let activeCount = 0;
      let fixedCount = 0;
      let fixedCodeCount = 0;
      let fixedTestCount = 0;
      const items: DefectGroup[] = [];
      const countFixed = (kind?: string) => {
        fixedCount++;
        if (kind === 'code') { fixedCodeCount++; } else if (kind === 'test') { fixedTestCount++; }
      };
      const seenKeys = new Set<string>();
      groups.forEach((group, key) => {
        seenKeys.add(key);
        const entry = statusIndex[key];
        if (entry?.status === 'fixed') {
          countFixed(entry?.kind);
          group.status = 'fixed';
          group.kind = entry.kind;
          group.timestamp = entry.fixedAt || group.timestamp;
          items.push(group);
        } else {
          activeCount++;
          group.kind = entry?.kind;
      ...
      // resolveDefectIfApplicable/"Mark fixed" can mark a (targetPath,
      // targetSymbol) fixed in _status.json without recordDefectIfGenuine
      // ever having written a raw record for it first (the original failure
      // may not have gone through that narrow guard, or was resolved from a
      // live test row rather than this panel). Those entries have no group
      // above and used to be dropped from both counts (and the list)
      // silently — a real fix that never shows up anywhere. Count them into
      // `fixed` and synthesize a minimal row from the status index alone
      // (targetSymbol/targetPath/fixedAt) so the fix is actually visible,
      // even with no failure output or test file to show.
      Object.keys(statusIndex).forEach((key) => {
        if (seenKeys.has(key) || statusIndex[key]?.status !== 'fixed') { return; }
        const entry = statusIndex[key];
        countFixed(entry?.kind);
        const sepIdx = key.lastIndexOf('::');
        const targetPath = sepIdx >= 0 ? key.slice(0, sepIdx) : key;
        const targetSymbol = sepIdx >= 0 ? key.slice(sepIdx + 2) : '';
        ...
        items.push({
          ...
          generator: '',
          output: '',
          timestamp: entry.fixedAt || '',
          occurrences: 1,
          status: 'fixed',
          kind: entry.kind,
        });
      });
      // Active bugs (still need a click) sort to the top; fixed bugs (code
      // and test alike) trail below as a record of what CODEMD's generated
      // tests actually caught. Each group sorts most-recent-first within
      // itself.
      items.sort((a, b) => {
      ...
      this.post({
        type: 'defectsResult', ok: true, items,
        counts: { active: activeCount, fixed: fixedCount, fixedCode: fixedCodeCount, fixedTest: fixedTestCount },
      });
    } catch (err: any) {
  ...
  private handleMessageInner(message: any, source: 'view' | 'side' = 'view'): void {
    if (message.type === 'generate') {
    ...
    } else if (message.type === 'setClaudeAgentMode') {
      this.setClaudeAgentMode(message.value);
    ...
    } else if (message.type === 'setUpTestRunner') {
      const folder = vscode.workspace.workspaceFolders?.[0];
      const requestId = String(message.requestId || '');
      if (!folder) {
        this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: 'No workspace folder open.' });
      } else {
        const language = testLanguageFor(String(message.file || ''));
        if (language === 'java' || language === 'kotlin') {
          // Kotlin reuses Java's harness-install entirely unchanged — same
          // pom.xml/build.gradle files, same JUnit 5 coordinate, same
          // detectJavaTestRunner/insertMavenJUnitDependency functions. See
          // runKotlinTestFileForResult's doc comment for why no separate
          // setUpKotlinTestRunnerForResult exists.
          this.setUpJavaTestRunnerForResult(message, folder);
        } else {
          this.setUpJsTestRunnerForResult(message, folder);
        }
      }
    } else if (message.type === 'runTestViaClaude') {
      const folder = vscode.workspace.workspaceFolders?.[0];
      if (!folder) {
        this.postTestRunResult({ requestId: String(message.requestId || ''), ok: false, error: 'No workspace folder open.' }, message);
      } else {
      ...
      cancel?.();
    } else if (message.type === 'cancelTestGeneration') {
      const requestId = String(message.requestId || '');
      const cancel = this.activeTestGenCancelers.get(requestId);
      cancel?.();
      ...
      logDebug(
        `[webview error] ${message.message || ''} (${message.source || ''}:${message.line || ''})\n${message.stack || ''}`,
      );
  ...
  private markWebviewReadyIfStillCurrent(source: 'view' | 'side', webview: vscode.WebviewView | vscode.WebviewPanel | undefined): void {
    if (source === 'side') {
  ...
  private markWebviewReady(source: 'view' | 'side'): void {
    if (source === 'side') {
  ...
  private async viewDiff(file: string, refs?: { base: string; target: string }, status?: string): Promise<void> {
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder || !file) {
      return;
    }
  ...
  async runGenerateAfterInitialGraphShown(options: { quiet: boolean }): Promise<void> {
    await this.waitForInitialGraphPost();
  ...
  async runGenerate(options: { quiet: boolean }): Promise<void> {
    const { quiet } = options;
    // Claimed synchronously as the very first statement, before any
    // `await` — so two calls fired close together (rapid "Generate"
    // clicks, or overlapping background triggers like startup analysis and
    // a file-watcher refresh landing around the same time) can never both
    // observe this.busy as false and each go on to spawn their own
    // local-analyze.py process. Everything below this point — including
    // the quiet fast-paths, which do their own `await`s before deciding
    // whether real analysis is even needed — runs under the claim, and the
    // `finally` guarantees it's released on every exit path, including the
    // early returns.
    if (this.busy) {
      if (!quiet) {
        await this.reveal();
  ...
  private async generateCallPathTestForResult(result: any, requestId: string): Promise<void> {
    ...
    if (this.isReadOnlyMode()) {
      this.post({ type: 'callPathTestResult', requestId, ok: false, error: 'Read-only mode is on — CODEMD will not invoke Claude to write tests. Turn off codemdGraphs.readOnlyMode in Settings to enable this.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const claudeAgentMode = this.getClaudeAgentMode();
    if (claudeAgentMode !== 'promptOnly' && !(await this.ensureClaudeAgentConsent())) {
      this.post({ type: 'callPathTestResult', requestId, ok: false, error: 'Claude wasn\'t allowed to run — click again to be asked.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const language = testLanguageFor(file);
    if (!language) {
      this.post({ type: 'callPathTestResult', requestId, ok: false, error: 'Generating a test with Claude currently only supports Python, JavaScript/TypeScript, Java, C#, Go, Rust, and Kotlin.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const jsRunnerHint = language === 'javascript' ? detectJsTestRunner(folder, file).runner : undefined;
    const tail = symbol.split('.').pop() || symbol;
    const callers: string[] = Array.isArray(result?.changeCard?.directCallers)
      ? result.changeCard.directCallers.slice(0, 8)
      : [];
    const callSites = callers.length ? await resolveCallSites(folder, language, symbol, callers) : new Map<string, CallSite>();
    // "removed" change cards point at a symbol that no longer exists in
    // `file` — telling Claude to "Read the function X defined in file Y"
    // below would send it hunting for code that was just deleted. The real
    // ask here is different in kind, not just in wording: there is nothing
    // to test directly, so this instead points Claude at the (still-real)
    // former callers and asks it to confirm THEIR behavior is still correct
    // now that the call is gone.
    const isRemoved = result?.changeCard?.kind === 'removed';
    let callerLine: string;
    if (!callers.length) {
      callerLine = isRemoved
        ? 'No confirmed callers were found for this removed function in the current callgraph — since nothing in the codebase appears to call it, there is no call path to regression-test.'
        : 'No confirmed callers were found in the current callgraph — write a direct test of the function using realistic inputs based on its own signature and body.';
    } else {
      ...
      callerLine = isRemoved
        ? `Its confirmed former callers in this codebase (before the removal): ${described.join(', ')}.`
          + (callSites.size
            ? ' Read ONE of them with an exact file:line given above.'
            : ' Read at least one of these to see how it used to invoke this function.')
        : `Its confirmed real callers in this codebase: ${described.join(', ')}.`
          + (callSites.size
    ...
    const removedContext = String(result?.changeCard?.change || `The function "${symbol}" was removed from the analyzed source.`);
    const introLine = isRemoved
      ? `The function "${symbol}" that used to live in "${file}"${targetLocation} was just removed — ${removedContext} It no longer exists, so do not try to Read or test it directly; do not import it.`
      : `Read the function "${symbol}" defined in "${file}"${targetLocation} (${importHintFor(language, file, folder, tail)}).`;
    const testTargetLine = isRemoved
      ? `Write ONE ${testFrameworkNounFor(language, jsRunnerHint)} test file (as plain text, not written to disk by you) that exercises the caller you read the way it is really`
        + ' used now — realistic constructed inputs, not placeholders — so the test would catch that caller regressing now that this function is gone,'
        + ' e.g. a leftover reference to the removed function, a broken fallback, or a crash.'
      : `Write ONE ${testFrameworkNounFor(language, jsRunnerHint)} test file (as plain text, not written to disk by you) that exercises this function the way it is really`
    ...
    const importLine = isRemoved
      ? '(including its own imports — it will be saved standalone and run exactly as you wrote it); do not import the removed function.'
      : `(including its own imports — it will be saved standalone and run exactly as you wrote it), importing ${tail} exactly as described above.`;
    const prompt = [
      introLine,
      callerLine,
      ...
      isRemoved
        ? 'Do not read or trace any other helper functions the caller calls internally — reason about them only from their names and how'
      ...
      javaClassNameInstruction(language, `callpath_${tail}`),
      'Respond only in the given JSON schema: `testSource` must be the complete, valid, self-contained test file source',
      importLine,
      // One narrow, named exception to "do not grep or explore the rest of
      // the codebase" above — a single specific file, not general
      // exploration. See runClaudeTestGeneration for why this points at the
      // file instead of restating its content here.
      'One exception to the above: if ".codemd/agent_policy/testing.md" exists in this repo, read it for CODEMD\'s testing',
      'responsibility policy and follow it alongside the specific instructions above.',
    ].join(' ');
    if (claudeAgentMode === 'promptOnly') {
      const expectedPath = expectedGeneratedTestPath(folder, `callpath_${tail}`, language, generatedTestExtFor(file, language));
      const interactivePrompt = `${prompt} You are running interactively in your own terminal via CODEMD's Prompt-Only Mode — ignore the`
        + ' instruction above about responding in a JSON schema; instead write the test file directly to'
        + ` "${expectedPath}" yourself (creating the .codemd/generated_tests/ directory if it doesn't exist yet), then briefly summarize`
        + ' what you wrote in chat.';
      const sent = await sendPromptToUsersClaudeTerminal(folder, interactivePrompt);
      this.post({
        type: 'callPathTestResult', requestId, ok: sent.ok, sentToTerminal: true,
        path: sent.ok ? expectedPath : undefined, error: sent.ok ? undefined : sent.error,
        targetPath: file, targetStartLine: startLine, targetSymbol: symbol,
      });
      return;
    }
    const schema = JSON.stringify({
      type: 'object',
      properties: {
        testSource: { type: 'string', description: 'Complete, self-contained test file source, including imports, in this project\'s actual test framework.' },
    ...
    // Shown immediately, before Claude has even started — a user asked why
    // the button just said "Generating…" with nothing else, and specifically
    ...
    try {
      ...
      const handleChunk = (text: string, stream: 'stdout' | 'stderr') => {
        if (stream === 'stderr') {
          if (text.trim()) { this.post({ type: 'testGenProgress', requestId, chunk: text }); }
          return;
        }
        ...
        for (const line of lines) {
          ...
          if (formatted) { this.post({ type: 'testGenProgress', requestId, chunk: formatted + '\n' }); }
        }
      };
      // Same canceledBeforeStart/liveCancel pattern as Fix's
      // activeFixCancelers and the shared runClaudeTestGeneration above —
      // this generator deliberately duplicates the CLI-invocation code
      // instead of routing through that shared helper (see the comment on
      // runClaudeTestGeneration), so it needs its own copy of the same
      // cancellation wiring.
      let canceledBeforeStart = false;
      let liveCancel: (() => void) | null = null;
      const cancelGen = () => { canceledBeforeStart = true; liveCancel?.(); };
      this.activeTestGenCancelers.set(requestId, cancelGen);
      type ClaudeCliRunResult = { status: number | null; stdout: string; stderr: string; timedOut: boolean; canceled: boolean; spawnError?: string };
      const genPromise = runClaudeCliExclusive<ClaudeCliRunResult>(() => {
        if (canceledBeforeStart) {
          return Promise.resolve<ClaudeCliRunResult>({ status: null, stdout: '', stderr: '', timedOut: false, canceled: true });
        }
        ...
        const spawned = execCommandCancelableAsync(
          claudeCommand,
          [
            '-p', prompt,
            ...
            // stream-json (JSONL, one event per line), not the single-blob json
            // every other field on this response still assumes — see
            // extractClaudeResultEnvelope below, which handles both shapes.
            // --verbose is REQUIRED by the CLI for -p + stream-json.
      ...
      let run: Awaited<typeof genPromise>;
      try {
        run = await genPromise;
      } finally {
      ...
      if (run.spawnError) {
        this.post({ type: 'callPathTestResult', requestId, ok: false, error: `Couldn't launch Claude Code CLI: ${run.spawnError}`, targetPath: file, targetSymbol: symbol });
        return;
      }
      if (run.timedOut) {
      ...
      if (run.status !== 0) {
        ...
        if (typeof errorEnvelope?.duration_ms === 'number') { generationMs = errorEnvelope.duration_ms; }
      ...
      if (!envelope) {
        this.post({ type: 'callPathTestResult', requestId, ok: false, error: 'Could not parse a response from Claude Code CLI.', generationMs, output: rawOutput, targetPath: file, targetSymbol: symbol });
        return;
      }
      if (typeof envelope.duration_ms === 'number') { generationMs = envelope.duration_ms; }
      const structured = envelope?.structured_output;
      const testSource = typeof structured?.testSource === 'string' ? stripStrayClosingTag(structured.testSource) : '';
      if (!testSource.trim()) {
        this.post({ type: 'callPathTestResult', requestId, ok: false, error: 'Claude did not return any test source.', generationMs, output: rawOutput, targetPath: file, targetSymbol: symbol });
        return;
      }
      const generationCostUsd = typeof envelope?.total_cost_usd === 'number' ? envelope.total_cost_usd : undefined;
      const generationTokens = tokensFromEnvelope(envelope);
      const outDir = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'generated_tests');
      await fs.promises.mkdir(outDir, { recursive: true });
      const outName = generatedTestFileNameFor(`callpath_${tail}`, language, generatedTestExtFor(file, language));
      const outPath = path.join(outDir, outName);
      await fs.promises.writeFile(outPath, testSource, 'utf8');
      const relPath = path.relative(folder.uri.fsPath, outPath).replace(/\\/g, '/');
      recordGeneratedTestInCache(folder, file, symbol, 'call_path', relPath);
      ...
      this.post({ type: 'callPathTestResult', requestId, ok: false, error: err?.message || String(err), targetPath: file, targetSymbol: symbol });
    }
  }
  ...
  private async runClaudeTestGeneration(
    ...
    | { ok: true; testSource: string; notes: string; plainSummary: string; generationMs: number; generationCostUsd?: number; generationTokens?: number; output: string }
    | { ok: false; error: string; generationMs?: number; output?: string }
    | { ok: true; sentToTerminal: true; path: string }
    | { ok: false; sentToTerminal: true; error: string }
  > {
    if (this.isReadOnlyMode()) {
    ...
    if (this.getClaudeAgentMode() === 'promptOnly') {
      const interactivePrompt = `${prompt} You are running interactively in your own terminal via CODEMD's Prompt-Only Mode — ignore the`
        + ' instruction above about responding in a JSON schema; instead write the test file directly to'
        + ` "${expectedPath}" yourself (creating the .codemd/generated_tests/ directory if it doesn't exist yet), then briefly summarize`
        + ' what you wrote in chat.';
      const sent = await sendPromptToUsersClaudeTerminal(folder, interactivePrompt);
      return sent.ok ? { ok: true, sentToTerminal: true, path: expectedPath } : { ok: false, sentToTerminal: true, error: sent.error };
    }
    ...
    const costCapUsd = this.maxCostPerActionUsd();
    // Appended once here rather than duplicated into each of the 4 prompts
    // that reach this shared function — Read is already an allowed tool, so
    // this points Claude at CODEMD's one generated source of truth for
    // testing responsibility instead of restating any of it inline (see
    // AGENT_POLICY_TESTING_MD in backend/main.py, which writes this file).
    const promptWithPolicy = `${prompt} If ".codemd/agent_policy/testing.md" exists in this repo, read it for CODEMD's testing`
      + ' responsibility policy and follow it alongside the specific instructions above.';
    // Same live-transcript treatment as generateCallPathTestForResult and the
    // Fix flow: the prompt itself lands in the transcript immediately (so a
    ...
    try {
      ...
      // Registered before the CLI call is even queued (see
      // runClaudeCliExclusive) so Stop still works while this generation is
      // waiting its turn behind another in-flight Claude call, not just once
      // it has actually spawned — canceledBeforeStart lets the queued task
      // below skip spawning entirely rather than starting a process just to
      // immediately kill it. Same pattern as Fix's activeFixCancelers.
      let canceledBeforeStart = false;
      let liveCancel: (() => void) | null = null;
      const cancel = () => { canceledBeforeStart = true; liveCancel?.(); };
      this.activeTestGenCancelers.set(requestId, cancel);
      type ClaudeCliRunResult = { status: number | null; stdout: string; stderr: string; timedOut: boolean; canceled: boolean; spawnError?: string };
      const genPromise = runClaudeCliExclusive<ClaudeCliRunResult>(() => {
        if (canceledBeforeStart) {
          return Promise.resolve<ClaudeCliRunResult>({ status: null, stdout: '', stderr: '', timedOut: false, canceled: true });
        ...
        const spawned = execCommandCancelableAsync(
          claudeCommand,
          ['-p', promptWithPolicy, '--max-turns', '16', '--allowedTools', 'Read,Grep,Glob', '--output-format', 'stream-json', '--verbose', '--json-schema', schema, '--max-budget-usd', String(costCapUsd)],
          folder.uri.fsPath,
          { ...process.env },
          TIMEOUT_MS,
          handleChunk,
      ...
      const generationCostUsd = typeof envelope?.total_cost_usd === 'number' ? envelope.total_cost_usd : undefined;
      const generationTokens = tokensFromEnvelope(envelope);
      // run.stdout is the CLI's --output-format json envelope, printed as one
      // compact line — testSource alone can run several KB once its own
      // newlines are escaped, so the naive `.slice(-4000)` tail-truncation
      // used for the error paths above routinely cuts into the middle of
      // this JSON instead of the (usually irrelevant) start. Pretty-print it
      // instead so "View Claude output" shows something readable, and keep
      // any stderr (CLI warnings, e.g. workspace-trust prompts) as a
      // separately labeled block rather than glued onto the same string.
      let prettyOutput: string;
      try {
        prettyOutput = JSON.stringify(envelope, null, 2);
      } catch {
        prettyOutput = run.stdout;
      }
      if (prettyOutput.length > 8000) { prettyOutput = prettyOutput.slice(0, 8000) + '\n… (truncated)'; }
      if (run.stderr.trim()) { prettyOutput += `\n\n--- stderr ---\n${run.stderr.trim().slice(-2000)}`; }
      return {
        ok: true,
        testSource,
  ...
  private async writeGeneratedTest(folder: vscode.WorkspaceFolder, namePrefix: string, testSource: string, ext = '.py', language: TestLanguage | null = null): Promise<string> {
    const outDir = path.join(folder.uri.fsPath, ARTIFACT_OUTPUT_DIR, 'generated_tests');
    await fs.promises.mkdir(outDir, { recursive: true });
    const outPath = path.join(outDir, generatedTestFileNameFor(namePrefix, language, ext));
    await fs.promises.writeFile(outPath, testSource, 'utf8');
  ...
  private async generateNewFunctionTestForResult(result: any, requestId: string): Promise<void> {
    ...
    const language = testLanguageFor(file);
    if (!language) {
      this.post({ type: 'newFunctionTestResult', requestId, ok: false, error: 'Generating a test for a new function currently only supports Python, JavaScript/TypeScript, Java, C#, Go, Rust, and Kotlin.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const jsRunnerHint = language === 'javascript' ? detectJsTestRunner(folder, file).runner : undefined;
    const tail = symbol.split('.').pop() || symbol;
    const callers = await findCallersOf(folder, language, symbol, 5);
    ...
    const targetLocation = startLine ? ` at line ${startLine}` : '';
    const prompt = [
      `A new function "${symbol}" was just added in "${file}"${targetLocation} (${importHintFor(language, file, folder, tail)}), as part of the current uncommitted change.`,
      callerLine,
      'Do not read or trace any other helper functions this one calls internally — reason about them only from their names and how',
      ...
      'Do not modify any files. Do not run any commands. Do not fix anything you find wrong — only write the test.',
      javaClassNameInstruction(language, `newfunc_${tail}`),
      'Respond only in the given JSON schema: `testSource` must be the complete, valid, self-contained test file source',
      `(including its own imports — it will be saved standalone and run exactly as you wrote it), importing ${tail} exactly as described above.`,
    ].join(' ');
    const gen = await this.runClaudeTestGeneration(folder, prompt, requestId, expectedGeneratedTestPath(folder, `newfunc_${tail}`, language, generatedTestExtFor(file, language)));
    if ('sentToTerminal' in gen) {
      this.post({ type: 'newFunctionTestResult', requestId, ok: gen.ok, sentToTerminal: true, path: gen.ok ? gen.path : undefined, error: gen.ok ? undefined : gen.error, targetPath: file, targetStartLine: startLine, targetSymbol: symbol });
      return;
    }
    if (!gen.ok) {
      this.post({ type: 'newFunctionTestResult', requestId, ok: false, error: gen.error, generationMs: gen.generationMs, output: gen.output, targetPath: file, targetSymbol: symbol });
      return;
    }
    const relPath = await this.writeGeneratedTest(folder, `newfunc_${tail}`, gen.testSource, generatedTestExtFor(file, language), language);
    recordGeneratedTestInCache(folder, file, symbol, 'new_function', relPath);
    this.post({
      ...
      generationTokens: gen.generationTokens,
      output: gen.output,
    });
  }
  ...
  private async generateCallChainTestForResult(chain: any, requestId: string): Promise<void> {
    ...
    const callerLanguage = testLanguageFor(callerFile);
    const calleeLanguage = testLanguageFor(calleeFile);
    if (!callerLanguage || callerLanguage !== calleeLanguage) {
      this.post({ type: 'callChainTestResult', requestId, ok: false, error: 'Call-chain tests currently only support Python, JavaScript/TypeScript, Java, C#, Go, Rust, and Kotlin, and both functions must be in the same language.' });
      return;
    }
    const language = callerLanguage;
    const jsRunnerHint = language === 'javascript' ? detectJsTestRunner(folder, callerFile).runner : undefined;
    const callerTail = callerSymbol.split('.').pop() || callerSymbol;
    const calleeTail = calleeSymbol.split('.').pop() || calleeSymbol;
    const callerModule = topLevelModuleFor(callerFile, language);
    const calleeModule = topLevelModuleFor(calleeFile, language);
    const prompt = [
      `Two functions changed together in the current diff and are directly connected: "${callerSymbol}" (in "${callerFile}",`,
      `${importHintFor(language, callerFile, folder, callerTail)}) calls "${calleeSymbol}" (in "${calleeFile}", ${importHintFor(language, calleeFile, folder, calleeTail)})`,
      ...
      `Write ONE ${testFrameworkNounFor(language, jsRunnerHint)} test file (as plain text, not written to disk by you) that exercises the two together as a real call chain —`,
      ...
      'not just a syntax error. If relevant to what you read, also consider: a return value from either function that changed shape',
      `(e.g. became optional/nullable) but whose caller still assumes the old shape; an exception "${calleeTail}" could now raise that`,
      `"${callerTail}" does not handle; and boundary/null/empty inputs at the point where "${callerTail}" calls "${calleeTail}".`,
      'Do not modify any files. Do not run any commands. Do not fix anything you find wrong — only write the test.',
      javaClassNameInstruction(language, `chain_${callerTail}_${calleeTail}`),
      'Respond only in the given JSON schema: `testSource` must be the complete, valid, self-contained test file source',
      `(including its own imports — it will be saved standalone and run exactly as you wrote it), importing ${callerTail} exactly as described above.`,
    ].join(' ');
    const gen = await this.runClaudeTestGeneration(folder, prompt, requestId, expectedGeneratedTestPath(folder, `chain_${callerTail}_${calleeTail}`, language, generatedTestExtFor(callerFile, language)));
    if ('sentToTerminal' in gen) {
      this.post({ type: 'callChainTestResult', requestId, ok: gen.ok, sentToTerminal: true, path: gen.ok ? gen.path : undefined, error: gen.ok ? undefined : gen.error, targetPath: callerFile, targetStartLine: Number(chain?.callerLine || 0) || 0, targetSymbol: callerSymbol });
      return;
    }
    if (!gen.ok) {
    ...
    const relPath = await this.writeGeneratedTest(folder, `chain_${callerTail}_${calleeTail}`, gen.testSource, generatedTestExtFor(callerFile, language), language);
    recordGeneratedTestInCache(folder, callerFile, callerSymbol, 'call_chain', relPath);
    this.post({
   ...
   *
   * Python-only, and NOT extendable to JS/TS the same "free" way the other
   * four generators were: check_call_sites' proof comes from
   * python_find_calls_in_function/python_check_call_compatibility
   * (backend/features/core/helpers.py), which parse the caller's Python AST
   * and hard-filter to `.py` callers — there is no JS/TS equivalent static
   * analysis yet, so callSiteIssues is always empty for JS/TS and this gate
   * reflects a real capability gap, not just an unlifted check.
   */
  private async generateContractTestForResult(result: any, requestId: string): Promise<void> {
    ...
    if (!file.toLowerCase().endsWith('.py')) {
      this.post({ type: 'contractTestResult', requestId, ok: false, error: 'Contract tests currently only support Python.', targetPath: file, targetSymbol: symbol });
      return;
    }
    if (!issues.length) {
      this.post({ type: 'contractTestResult', requestId, ok: false, error: 'No broken call sites found for this function.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const tail = symbol.split('.').pop() || symbol;
    ...
    const prompt = [
      ...
      `If relevant to what you read, also cover: a boundary/null/empty value for any new or changed parameter, and — if "${tail}" can now`,
      'raise an exception it could not before, or its return value changed shape (e.g. became optional/nullable) — a test asserting the',
      'corrected caller actually handles that.',
    ...
    const gen = await this.runClaudeTestGeneration(folder, prompt, requestId, expectedGeneratedTestPath(folder, `contract_${tail}`, null, '.py'));
    if ('sentToTerminal' in gen) {
      this.post({ type: 'contractTestResult', requestId, ok: gen.ok, sentToTerminal: true, path: gen.ok ? gen.path : undefined, error: gen.ok ? undefined : gen.error, targetPath: file, targetStartLine: startLine, targetSymbol: symbol });
      return;
    }
    if (!gen.ok) {
      this.post({ type: 'contractTestResult', requestId, ok: false, error: gen.error, generationMs: gen.generationMs, output: gen.output, targetPath: file, targetSymbol: symbol });
      return;
    }
    const relPath = await this.writeGeneratedTest(folder, `contract_${tail}`, gen.testSource);
    recordGeneratedTestInCache(folder, file, symbol, 'contract', relPath);
    this.post({
  ...
  private async generateBroadCoverageTestForResult(result: any, requestId: string): Promise<void> {
    ...
    const language = testLanguageFor(file);
    if (!language) {
      this.post({ type: 'broadCoverageTestResult', requestId, ok: false, error: 'Input variation tests currently only support Python, JavaScript/TypeScript, Java, C#, Go, Rust, and Kotlin.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const jsRunnerHint = language === 'javascript' ? detectJsTestRunner(folder, file).runner : undefined;
    // A caller-count-only cap (the old flat slice(0, 8)) bounds how many
    // callers get considered but not how much reading each one takes — a
    // handful of huge/complex callers can blow the prompt up just as easily
    // as many small ones, and either way Claude previously had to spend a
    // Read tool call (a turn, plus real wall-clock time) per caller just to
    // see it. Both are addressed together below: candidatePool is a wider
    // pool to choose from, but the actual selection greedily accepts callers
    // in order until EITHER the count cap or the combined snippet-size
    // budget is hit, and the snippet itself (real code around the call
    // site) is read here and embedded directly in the prompt — so Claude
    ...
    const candidatePool: string[] = Array.isArray(result?.changeCard?.directCallers) ? result.changeCard.directCallers.slice(0, 20) : [];
    ...
    const tail = symbol.split('.').pop() || symbol;
    const callSites = await resolveCallSites(folder, language, symbol, candidatePool);
    const selected: { name: string; site: CallSite; snippet: string }[] = [];
    let totalSnippetLines = 0;
    for (const name of candidatePool) {
      if (selected.length >= MAX_BROAD_COVERAGE_CALLERS) { break; }
      const site = callSites.get(name);
      if (!site) { continue; }
      const snippet = readCallSiteSnippet(folder, site.file, site.line);
      if (!snippet) { continue; }
      const snippetLines = snippet.split('\n').length;
      // Always take the first 2 regardless of size (the floor this generator
      // already required) — after that, stop growing once the combined
      // snippet budget would be exceeded rather than silently truncating a
      // caller's context mid-snippet.
      if (selected.length >= 2 && totalSnippetLines + snippetLines > MAX_BROAD_COVERAGE_SNIPPET_LINES) { break; }
      selected.push({ name, site, snippet });
      totalSnippetLines += snippetLines;
    }
    if (selected.length < 2) {
      this.post({ type: 'broadCoverageTestResult', requestId, ok: false, error: 'Fewer than 2 confirmed callers with readable source — a single Call Path Test already covers this well enough.', targetPath: file, targetSymbol: symbol });
      return;
    }
    const callerBlocks = selected.map(({ name, site, snippet }) =>
      `--- Caller: ${name} (${site.file}, around line ${site.line}) ---\n${snippet}`).join('\n\n');
    const targetLocation = startLine ? ` at line ${startLine}` : '';
    const prompt = [
      `The function "${symbol}" in "${file}"${targetLocation} (${importHintFor(language, file, folder, tail)}) was changed and has ${selected.length} confirmed`,
      "real callers in this codebase — a single test on one caller wouldn't cover how differently each of them actually uses it.",
      'Here is the real code around each caller\'s call site, already read for you (exact file and line given per block) — do not re-read',
      'these same files:',
      '',
      callerBlocks,
      '',
      'From this, identify genuinely DIFFERENT calling patterns — different argument shapes, edge cases, or usage contexts — not',
      'near-duplicates of each other.',
      'Only use Read/Grep/Glob if one of the snippets above alone is not enough to understand a call (e.g. a variable defined outside the',
      'shown range) — do not read or trace any other helper functions this one calls internally, and do not grep or explore the rest of the',
      'codebase beyond understanding these specific callers.',
      `Write ONE ${testFrameworkNounFor(language, jsRunnerHint)} test file (as plain text, not written to disk by you) with one test function per genuinely distinct calling pattern you found`,
      ...
      javaClassNameInstruction(language, `broadcoverage_${tail}`),
      'Respond only in the given JSON schema: `testSource` must be the complete, valid, self-contained test file source',
      `(including its own imports — it will be saved standalone and run exactly as you wrote it), importing ${tail} exactly as described above.`,
    ].join('\n');
    const gen = await this.runClaudeTestGeneration(folder, prompt, requestId, expectedGeneratedTestPath(folder, `broadcoverage_${tail}`, language, generatedTestExtFor(file, language)));
    if ('sentToTerminal' in gen) {
      this.post({ type: 'broadCoverageTestResult', requestId, ok: gen.ok, sentToTerminal: true, path: gen.ok ? gen.path : undefined, error: gen.ok ? undefined : gen.error, targetPath: file, targetStartLine: startLine, targetSymbol: symbol });
      return;
    }
    if (!gen.ok) {
      this.post({ type: 'broadCoverageTestResult', requestId, ok: false, error: gen.error, generationMs: gen.generationMs, output: gen.output, targetPath: file, targetSymbol: symbol });
      return;
    }
    const relPath = await this.writeGeneratedTest(folder, `broadcoverage_${tail}`, gen.testSource, generatedTestExtFor(file, language), language);
    recordGeneratedTestInCache(folder, file, symbol, 'broad_coverage', relPath);
    this.post({
  ...
  private async runFindTestsForResult(result: any, requestId: string): Promise<void> {
    ...
    try {
      const { status, stdout, timedOut } = await execNodeCliAsync(
        scriptPath,
        ['--workspace', folder.uri.fsPath, '--cli', 'find_tests', '--query', query, '--limit', '5'],
        folder.uri.fsPath,
      );
      if (timedOut) {
      ...
      const knownFiles = new Set(confirmedMatches.map((m: any) => String(m.file || '')));
      ...
      this.post({
        ...
        matches: [...confirmedMatches, ...generatedMatches]
          ...
          // Sort by importance/impact before truncating to 5, so a
          // low-priority broad-coverage match can't bump a proven-broken
          // contract/call-chain match out of the visible list.
          .sort((a: { kind: string }, b: { kind: string }) => testKindRank(a.kind) - testKindRank(b.kind))
          .slice(0, 5),
      });
    } catch (err: any) {
  ...
  /**
   * Runs the existing test file(s) codemd_find_tests already discovers for a
   * changed symbol, and reports real pass/fail + coverage-confirmation per
   * match — the "regression-test selection" half of verification that static
   * discovery alone doesn't cover. No Claude call (pure local search + pytest),
   * so unlike the four Claude generators this is meant to be triggered
   * automatically by the webview, not gated behind a button click.
   */
  private async runExistingTestsForResult(result: any, requestId: string): Promise<void> {
    const query = String(result?.graphSymbol || result?.fullName || result?.symbol || result?.name || '').trim();
    const file = String(result?.file || '');
    const startLine = Number(result?.line || 0) || 0;
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!query || !folder) {
      this.post({ type: 'existingTestsResult', requestId, ok: false, error: 'No symbol to check.' });
      return;
    }
    const lowerFile = file.toLowerCase();
    const supportedExts = ['.py', '.go', '.cs', '.java', '.kt', '.kts'];
    if (!supportedExts.some((ext) => lowerFile.endsWith(ext))) {
      this.post({ type: 'existingTestsResult', requestId, ok: false, error: 'Running existing tests currently only supports Python, Go, C#, Java, and Kotlin.' });
      return;
    }
    ...
    if (cachedPass) {
      this.post({
        ...
        matches: [{ file: cachedPass.testFile, ok: true, passed: true, coverageConfirmed: true, executed: true }],
      });
      return;
    }
    const scriptPath = vscode.Uri.joinPath(this.context.extensionUri, 'scripts', 'codemd-mcp-server.js').fsPath;
    if (!fs.existsSync(scriptPath)) {
      this.post({ type: 'existingTestsResult', requestId, ok: false, error: 'Test lookup script not found in this build.' });
      return;
    }
    try {
      const { status, stdout, timedOut } = await execNodeCliAsync(
      ...
      if (timedOut || status !== 0) {
        this.post({ type: 'existingTestsResult', requestId, ok: false, error: 'Test lookup timed out or failed.' });
        return;
      }
      const data = JSON.parse(stdout);
      ...
      const knownFiles = new Set(rawMatches.map((m: any) => String(m.file || '')));
      ...
      const matches = await Promise.all(combinedMatches.map(async (m: any) => {
        const matchFile = String(m.file || '');
        ...
        // Match on the FOUND test file's own extension, not the target's —
        // find_tests could in principle return a match in a different
        // language than the changed function; running it with the wrong
        // language's runner would misreport rather than just skip it.
        const runArgs = { file: matchFile, nodeId, targetPath: file, targetStartLine: startLine, targetSymbol: query };
        const matchLower = matchFile.toLowerCase();
        const payload = matchLower.endsWith('.go') ? await this.computeGoTestRunPayload(runArgs, folder)
          : matchLower.endsWith('.cs') ? await this.computeCsharpTestRunPayload(runArgs, folder)
          : matchLower.endsWith('.java') ? await this.computeJavaTestRunPayload(runArgs, folder)
          : (matchLower.endsWith('.kt') || matchLower.endsWith('.kts')) ? await this.computeKotlinTestRunPayload(runArgs, folder)
          : await this.computePythonTestRunPayload(runArgs, folder);
        return {
          file: matchFile,
          ok: !!payload.ok,
          error: payload.error,
          ...
          coverageConfirmed: !!payload.coverageConfirmed,
          executed: !!payload.executed,
          executedInRange: payload.executedInRange,
          trackedInRange: payload.trackedInRange,
          output: payload.output,
        };
      }));
      this.post({ type: 'existingTestsResult', requestId, ok: true, symbol: query, matches });
    } catch (err: any) {
      this.post({ type: 'existingTestsResult', requestId, ok: false, error: err?.message || String(err) });
    }
  }
  ...
  private async runTestFileForResult(message: any): Promise<void> {
    const requestId = String(message?.requestId || '');
    if (requestId) { this.testRunStartTimes.set(requestId, Date.now()); }
    const file = String(message?.file || '');
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder || !file) {
      this.postTestRunResult({ requestId, ok: false, error: 'No test file to run.' }, message);
      return;
    }
    const lower = file.toLowerCase();
    ...
    if (lower.endsWith('.java')) {
      return this.runJavaTestFileForResult(message, folder);
    }
    if (lower.endsWith('.cs')) {
      return this.runCsharpTestFileForResult(message, folder);
    }
    if (lower.endsWith('.kt') || lower.endsWith('.kts')) {
      return this.runKotlinTestFileForResult(message, folder);
    }
    this.postTestRunResult({ requestId, ok: false, error: 'Running tests for this file type isn\'t supported directly yet — use "Ask Claude to run" instead.' }, message);
  }
  ...
  private async runPythonTestFileForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    ...
    this.postTestRunResult({ requestId, ...payload }, message);
  }
  ...
  private async installTestDependencyAndRetry(message: any): Promise<void> {
    ...
    if (!folder || !pythonPath || !packageName) {
      this.postTestRunResult({ requestId, ok: false, error: 'Missing install target — nothing to install.' }, message);
      return;
    }
    ...
    if (install.spawnError) {
      this.postTestRunResult({ requestId, ok: false, error: `Couldn't launch "${pythonPath}" to install ${packageName}: ${install.spawnError}` }, message);
      return;
    }
  ...
  /**
   * The opt-in "⚙ Set up test runner" action for a JS/TS repo that
   * detectJsTestRunner found has none — the only place in this whole
   * feature allowed to add a JS/TS test framework, and only on this exact,
   * explicit click (never automatically, never as a side effect of Run or
   * Fix — see the comments on those). Deliberately does NOT call Claude:
   * selectJsTestRunner picks the framework and writes its config from a
   * fixed template, so the outcome for a given repo shape is always the
   * same and the diff stays small and predictable. `npm install --save-dev`
   * (not --no-save, unlike the Fix flow's transient installs) so the
   * addition is real, visible in package.json, and survives past this run.
   */
  private async setUpJsTestRunnerForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    const requestId = String(message?.requestId || '');
    if (this.isReadOnlyMode()) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: 'Read-only mode is on — CODEMD will not modify your repo. Turn off codemdGraphs.readOnlyMode in Settings to enable this.' });
      return;
    }
    const existing = detectJsTestRunner(folder, String(message?.file || ''));
    if (existing.found) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: true, alreadyPresent: true, runner: existing.runner || '', summary: `${existing.runner || 'A test runner'} is already set up in this repo — nothing to install.` });
      return;
    }
    const plan = selectJsTestRunner(folder);
    this.post({ type: 'status', text: `Setting up ${plan.runner} — this repo has no JS/TS test runner yet…` });
    const install = await execShellLineAsync(`npm install --save-dev ${plan.packages.join(' ')}`, folder.uri.fsPath, { ...process.env }, 180000);
    const installOutput = `${install.stdout}\n${install.stderr}`.trim();
    if (install.spawnError) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: `Couldn't launch npm: ${install.spawnError}`, output: installOutput.slice(-4000) });
      return;
    }
    if (install.timedOut) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: 'npm install did not finish within 180s.', output: installOutput.slice(-4000) });
      return;
    }
    if (install.status !== 0) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: `npm install exited with code ${install.status}. See output.`, output: installOutput.slice(-4000) });
      return;
    }
    const configPath = path.join(folder.uri.fsPath, plan.configFile);
    if (!fs.existsSync(configPath)) {
      await fs.promises.writeFile(configPath, plan.configContents, 'utf8');
    }
    // Only the "test" script is ours to add here — devDependencies/lockfile
    // are npm's own job above, already done and already using npm's exact
    // formatting; hand-editing those too would just risk drifting from it.
    let scriptAdded = false;
    try {
      const pkgPath = path.join(folder.uri.fsPath, 'package.json');
      const pkg = JSON.parse(await fs.promises.readFile(pkgPath, 'utf8'));
      const currentScript = String(pkg.scripts?.test || '');
      if (!currentScript || /no test specified/i.test(currentScript)) {
        pkg.scripts = { ...(pkg.scripts || {}), test: plan.testScript };
        await fs.promises.writeFile(pkgPath, `${JSON.stringify(pkg, null, 2)}\n`, 'utf8');
        scriptAdded = true;
      }
    } catch {
      // package.json unreadable/malformed after npm's own write — the
      // install itself already succeeded, so still report ok:true; the
      // config file alone is enough to run tests with an explicit command.
    }
    this.post({
      type: 'setUpTestRunnerResult',
      requestId,
      ok: true,
      runner: plan.runner,
      packagesInstalled: plan.packages,
      configFile: plan.configFile,
      scriptAdded,
      summary: `Installed ${plan.packages.join(', ')} and wrote ${plan.configFile}${scriptAdded ? ' plus a "test" script in package.json' : ''}. ${plan.reason}`,
      output: installOutput.slice(-4000),
    });
  }

  /**
   * Java's version of setUpJsTestRunnerForResult, same rule: no Claude call,
   * one fixed coordinate (JUnit 5 — the uncontested default, no Jest-vs-
   * Vitest-style choice to make here), a real edit to the project's own
   * build file so it's visible and permanent, never a silent side effect of
   * Run or Fix. Text-insertion, not a real XML/Groovy parse — see
   * insertMavenJUnitDependency's doc comment for why. `mvn dependency:resolve`
   * afterward is the verification step (equivalent to npm's install actually
   * downloading something): if the coordinate were ever wrong, this is
   * where it would fail, not silently deferred to the next test run.
   */
  private async setUpJavaTestRunnerForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    const requestId = String(message?.requestId || '');
    if (this.isReadOnlyMode()) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: 'Read-only mode is on — CODEMD will not modify your repo. Turn off codemdGraphs.readOnlyMode in Settings to enable this.' });
      return;
    ...
    const buildRoot = findJavaBuildRootDir(folder);
    if (!buildRoot) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: 'No pom.xml or build.gradle found at the workspace root.' });
      return;
    }
    const existing = detectJavaTestRunner(folder);
    if (existing.found) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: true, alreadyPresent: true, runner: existing.runner || '', summary: `${existing.runner || 'A test framework'} is already declared in this project — nothing to install.` });
      return;
    }
    const buildFileName = buildRoot.tool === 'maven'
      ? 'pom.xml'
      : (fs.existsSync(path.join(buildRoot.root, 'build.gradle.kts')) ? 'build.gradle.kts' : 'build.gradle');
    const buildFilePath = path.join(buildRoot.root, buildFileName);
    let originalText: string;
    try {
      originalText = await fs.promises.readFile(buildFilePath, 'utf8');
    } catch (err: any) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: `Could not read ${buildFileName}: ${err?.message || err}` });
      return;
    }
    const updatedText = buildRoot.tool === 'maven' ? insertMavenJUnitDependency(originalText) : insertGradleJUnitDependency(originalText);
    if (!updatedText) {
      this.post({ type: 'setUpTestRunnerResult', requestId, ok: false, error: `Could not automatically edit ${buildFileName} (no <dependencies>/</project> anchor found) — add this dependency yourself: org.junit.jupiter:junit-jupiter:5.10.2 (test scope).` });
      return;
    }
    this.post({ type: 'status', text: `Setting up JUnit 5 — this project has no test framework declared yet…` });
    await fs.promises.writeFile(buildFilePath, updatedText, 'utf8');
    // Verification step — resolves the dependency for real, same role
    // npm install's actual download plays for the JS/TS path. On failure,
    // roll the edit back rather than leaving a bad, uncommitted change in
    // the user's build file for something that turned out not to resolve.
    const verify = buildRoot.tool === 'maven'
      ? await execCommandAsync('mvn', ['-q', '-B', 'dependency:resolve'], buildRoot.root, { ...process.env }, 120000)
      : await execCommandAsync('gradle', ['-q', 'dependencies', '--configuration', 'testCompileClasspath'], buildRoot.root, { ...process.env }, 120000);
    const verifyOutput = `${verify.stdout}\n${verify.stderr}`.trim();
    if (verify.spawnError || verify.status !== 0) {
      await fs.promises.writeFile(buildFilePath, originalText, 'utf8');
      this.post({
        type: 'setUpTestRunnerResult',
        requestId,
        ok: false,
        error: verify.spawnError
          ? `Wrote the dependency, but couldn't launch "${buildRoot.tool === 'maven' ? 'mvn' : 'gradle'}" to verify it resolves — reverted ${buildFileName}: ${verify.spawnError}`
          : `The added dependency did not resolve — reverted ${buildFileName}. See output.`,
        output: verifyOutput.slice(-4000),
      });
      return;
    }
    this.post({
      ...
      ok: true,
      runner: 'JUnit 5',
      packagesInstalled: ['org.junit.jupiter:junit-jupiter:5.10.2'],
      configFile: buildFileName,
      summary: `Added org.junit.jupiter:junit-jupiter:5.10.2 (test scope) to ${buildFileName}${buildRoot.tool === 'gradle' ? ' and enabled useJUnitPlatform()' : ''}, verified it resolves.`,
      output: verifyOutput.slice(-4000),
    });
  }
   ...
   */
  private async describePytestFixtureSetupError(
    pythonPath: string,
    output: string,
    folder: vscode.WorkspaceFolder,
  ): Promise<{ error: string; missingPackage?: string } | null> {
    if (!/ERROR at setup of /i.test(output)) { return null; }
   ...
   */
  private async findDeclaredPackageForFixture(folder: vscode.WorkspaceFolder, fixtureName: string): Promise<string | undefined> {
   ...
   * with "No module named ..." even though the target file genuinely exists,
   * the usual cause is a self-contained subfolder (this extension's own
   * bundled backend/ is exactly this shape) whose own internal imports are
   * absolute relative to ITS OWN directory, not the workspace root — e.g.
   * backend/main.py does `from features.core import helpers`, which only
   * resolves once backend/ itself, not the repo root, is on sys.path.
   * Confirmed empirically (see the bug this fixes) that adding the right
   * ancestor directory to PYTHONPATH makes both "import backend.main" and a
   * bare "import main" resolve, since the missing sibling package becomes
   * reachable either way — so this reuses the test's existing dotted import
   * name unchanged rather than needing to know which style was written.
   * Probes ancestors between the target file and the workspace root,
   * shallowest first (matches the common "self-contained sub-app root" case
   * in one probe); returns the absolute directory to add to PYTHONPATH, or
   * null if no candidate fixes it.
   */
  private async findPythonImportRootFix(
    pythonPath: string,
    folder: vscode.WorkspaceFolder,
    targetPath: string,
    env: NodeJS.ProcessEnv,
  ): Promise<string | null> {
    const dotted = pythonModuleNameFor(targetPath);
    const relDir = path.dirname(targetPath.replace(/\\/g, '/'));
    if (!relDir || relDir === '.') { return null; }
    let candidateRel = '';
    for (const segment of relDir.split('/').filter(Boolean)) {
      candidateRel = candidateRel ? `${candidateRel}/${segment}` : segment;
      const candidateAbs = path.join(folder.uri.fsPath, candidateRel);
      const probeEnv = { ...env, PYTHONPATH: candidateAbs + (env.PYTHONPATH ? path.delimiter + env.PYTHONPATH : '') };
      const probe = await execCommandAsync(pythonPath, ['-c', `import ${dotted}`], folder.uri.fsPath, probeEnv, 15000);
      if (probe.status === 0) {
        return candidateAbs;
      }
    }
  ...
  /**
   * Runs a pytest-family command and, only if it fails to collect at all
   * with a "No module named" error, retries once with an extra PYTHONPATH
   * entry from findPythonImportRootFix — see that method's doc comment.
   * Cheap in the common case: the retry probe only runs after a genuine
   * collection failure, never on an already-working test.
   */
  private async execPythonTestCommand(
    pythonPath: string,
    args: string[],
    folder: vscode.WorkspaceFolder,
    env: NodeJS.ProcessEnv,
    ...
    const run = await execCommandAsync(pythonPath, args, folder.uri.fsPath, env);
    if (run.spawnError || run.timedOut || run.status === 0 || run.status === 1 || !targetPath) {
      return run;
    }
    const combinedOutput = `${run.stdout}\n${run.stderr}`;
    if (!/no module named ['"]?[\w.]+['"]?/i.test(combinedOutput)) {
      return run;
    }
    const extraRoot = await this.findPythonImportRootFix(pythonPath, folder, targetPath, env);
    if (!extraRoot) {
      return run;
    }
    const retryEnv = { ...env, PYTHONPATH: extraRoot + (env.PYTHONPATH ? path.delimiter + env.PYTHONPATH : '') };
    return execCommandAsync(pythonPath, args, folder.uri.fsPath, retryEnv);
  }
  ...
  private async computePythonTestRunPayload(message: any, folder: vscode.WorkspaceFolder): Promise<any> {
    // Queues rather than firing unboundedly — see acquirePythonTestRunSlot's
    // doc comment for why this exists (runExistingTestsForResult's auto-fire
    // has no cap of its own and can otherwise flood the OS with processes).
    await acquirePythonTestRunSlot();
    try {
      return await this.computePythonTestRunPayloadImpl(message, folder);
    } finally {
      releasePythonTestRunSlot();
    }
  }

  private async computePythonTestRunPayloadImpl(message: any, folder: vscode.WorkspaceFolder): Promise<any> {
    const file = String(message?.file || '');
    ...
    let output = '';
    try {
      const covRun = await this.execPythonTestCommand(
        pythonPath,
      ...
      if (noCoverage) {
        ...
        const plainRun = await this.execPythonTestCommand(pythonPath, ['-m', 'pytest', target, '-v'], folder, env, targetPath);
      ...
      const jsonRun = await execCommandAsync(
        ...
        ['-m', 'coverage', 'json', `--data-file=${dataFile}`, '-o', jsonFile, `--include=${targetPath}`],
        folder.uri.fsPath,
        env,
        30000,
      );
  ...
  private async runGoTestFileForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    const requestId = String(message?.requestId || '');
    const payload = await this.computeGoTestRunPayload(message, folder);
    this.postTestRunResult({ requestId, ...payload }, message);
  }

  // Split out from runGoTestFileForResult (same reasoning as Python's
  // computePythonTestRunPayload split) so runExistingTestsForResult's
  // auto-fire "does an existing test actually cover this?" check can reuse
  // this exact run-and-confirm logic for Go instead of being Python-only.
  private async computeGoTestRunPayload(message: any, folder: vscode.WorkspaceFolder): Promise<any> {
    ...
    try {
      ...
      output = `${run.stdout}\n${run.stderr}`.trim();
      if (run.spawnError) {
        return { ok: false, error: `Couldn't launch "go": ${run.spawnError}. Is the Go toolchain installed and on PATH?` };
      }
      if (run.timedOut) {
        return { ok: false, error: 'Test run timed out after 90s.', output: output.slice(-4000) };
      }
      ...
      if (couldNotRun) {
        return { ok: false, error: 'go test could not build/collect this test — see output.', output: output.slice(-4000) };
      }
  ...
  private async runRustTestFileForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    ...
    try {
      ...
      output = `${run.stdout}\n${run.stderr}`.trim();
      if (run.spawnError) {
        this.postTestRunResult({ requestId, ok: false, error: `Couldn't launch "cargo": ${run.spawnError}. Is the Rust toolchain installed and on PATH?` }, message);
        return;
      }
      if (run.timedOut) {
        this.postTestRunResult({ requestId, ok: false, error: 'Test run timed out after 150s.', output: output.slice(-4000) }, message);
        return;
      }
      const combined = output.toLowerCase();
      ...
      if (couldNotRun) {
        this.postTestRunResult({ requestId, ok: false, error: 'cargo could not build/collect this test — see output.', output: output.slice(-4000) }, message);
        return;
      }
      const passed = run.status === 0;
      ...
      this.postTestRunResult({ requestId, ok: false, error: err?.message || String(err), output: output.slice(-4000) }, message);
    } finally {
  ...
  /**
   * Java's fourth direct-run tier alongside Python/Go/Rust. Unlike those
   * three, a CODEMD-generated Java test can't just be handed to `mvn test` —
   * it's saved flatly under .codemd/generated_tests/ (see
   * generatedTestFileNameFor), not inside src/test/java where Maven's own
   * test phase looks by convention, and teaching "Set up test runner" to
   * rewrite pom.xml's compiled test-source roots felt like a bigger, riskier
   * edit than this problem needs. Instead: compile the one generated file
   * directly with javac against the project's real dependency classpath
   * (from `mvn dependency:build-classpath`) plus its already-compiled
   * main/test classes, then execute it with the JUnit Platform Console
   * Launcher (fetched once via `mvn dependency:get` into the normal local
   * ~/.m2 cache, same as any other Maven dependency) — the standard,
   * documented way to run one ad-hoc JUnit 5 class outside a full build.
   * Maven only for now (see findJavaBuildRootDir) — Gradle's classpath
   * isn't a one-liner to extract the same way and is left as a clear,
   * honest "not yet supported" rather than a fragile guess.
   * NOTE: like the Rust runner above, this could not be empirically
   * verified against a real Maven project in this session (no JDK/Maven
   * available in this dev environment) — the invocation follows the
   * standard, documented `dependency:build-classpath` / Console Launcher
   * `execute` usage, but should be checked against a real project before
   * relying on it in production.
   */
  // Fetches (or reuses the already ~/.m2-cached) Maven artifact by exact GAV
  // coordinate — shared by the JUnit Console Launcher, JaCoCo agent, and
  // JaCoCo CLI jar fetches (Java and Kotlin both), all of which follow the
  // identical "check ~/.m2 first, else `mvn dependency:get`" pattern. Only
  // used for the two NEW JaCoCo jars below — the pre-existing Console
  // Launcher fetch is left as its own inline block rather than migrated,
  // to keep this change scoped to what coverage actually needs.
  private async ensureMavenArtifactCached(
    groupId: string, artifactId: string, version: string, classifier: string | null,
    buildRootDir: string, env: NodeJS.ProcessEnv,
  ): Promise<{ jarPath: string | null; appended: string }> {
    const groupPath = groupId.split('.').join(path.sep);
    const fileSuffix = classifier ? `-${classifier}` : '';
    const jarPath = path.join(os.homedir(), '.m2', 'repository', groupPath, artifactId, version, `${artifactId}-${version}${fileSuffix}.jar`);
    if (fs.existsSync(jarPath)) { return { jarPath, appended: '' }; }
    const coord = classifier ? `${groupId}:${artifactId}:${version}:jar:${classifier}` : `${groupId}:${artifactId}:${version}`;
    const getStep = await execCommandAsync('mvn', ['-q', '-B', 'dependency:get', `-Dartifact=${coord}`], buildRootDir, env, 120000);
    const appended = `\n${getStep.stdout}\n${getStep.stderr}`.trim();
    return { jarPath: fs.existsSync(jarPath) ? jarPath : null, appended };
  }

  // Shared by Java and Kotlin's run paths (both use the same JVM/Maven/JUnit
  // Console Launcher machinery): fetches the JaCoCo agent + CLI jars
  // (degrading gracefully, same "best effort" tier as Go/Rust when their own
  // coverage tool isn't available, if either fetch fails), re-runs the
  // Console Launcher with the agent attached to produce a raw .exec dump,
  // then invokes the CLI's own `report` command to turn that into XML and
  // checks whether the target function's lines were covered.
  private async runJvmCoverageConfirmedLauncher(
    consoleJar: string, buildRootDir: string, runClasspath: string, fqcn: string,
    classDirs: string[], targetPath: string, targetStartLine: number, absTargetFile: string,
    env: NodeJS.ProcessEnv,
  ): Promise<{ run: { status: number | null; stdout: string; stderr: string; timedOut: boolean; spawnError?: string }; appended: string; coverageConfirmed: boolean; executed: boolean; executedInRange?: number; trackedInRange?: number; reason?: string }> {
    const jacocoVersion = '0.8.13';
    let appended = '';
    const agentFetch = await this.ensureMavenArtifactCached('org.jacoco', 'org.jacoco.agent', jacocoVersion, 'runtime', buildRootDir, env);
    appended += agentFetch.appended;
    const cliFetch = await this.ensureMavenArtifactCached('org.jacoco', 'org.jacoco.cli', jacocoVersion, 'nodeps', buildRootDir, env);
    appended += cliFetch.appended;
    const execFile = path.join(os.tmpdir(), `codemd-jacoco-${crypto.randomBytes(6).toString('hex')}.exec`);
    const javaArgs: string[] = [];
    if (agentFetch.jarPath) {
      javaArgs.push(`-javaagent:${agentFetch.jarPath}=destfile=${execFile}`);
    }
    javaArgs.push('-jar', consoleJar, 'execute', '--class-path', runClasspath, '--select-class', fqcn, '--details=tree', '--fail-if-no-tests');
    const run = await execCommandAsync('java', javaArgs, buildRootDir, env, 90000);
    appended += `\n${run.stdout}\n${run.stderr}`.trim();
    if (run.spawnError || run.timedOut || !agentFetch.jarPath || !cliFetch.jarPath || !fs.existsSync(execFile) || !targetPath || !targetStartLine) {
      return {
        run, appended, coverageConfirmed: false, executed: false,
        reason: !agentFetch.jarPath || !cliFetch.jarPath
          ? 'Could not fetch JaCoCo from Maven — no coverage tool available for this run.'
          : (!targetPath || !targetStartLine ? 'No target line range available to check.' : undefined),
      };
    }
    try {
      const reportXmlPath = path.join(os.tmpdir(), `codemd-jacoco-${crypto.randomBytes(6).toString('hex')}.xml`);
      const reportArgs = ['-jar', cliFetch.jarPath, 'report', execFile];
      for (const dir of classDirs) { reportArgs.push('--classfiles', dir); }
      reportArgs.push('--xml', reportXmlPath);
      const reportStep = await execCommandAsync('java', reportArgs, buildRootDir, env, 60000);
      appended += `\n${reportStep.stdout}\n${reportStep.stderr}`.trim();
      if (reportStep.status !== 0 || !fs.existsSync(reportXmlPath)) {
        return { run, appended, coverageConfirmed: false, executed: false, reason: 'JaCoCo could not generate a coverage report — see output.' };
      }
      const xmlText = await fs.promises.readFile(reportXmlPath, 'utf8');
      await fs.promises.unlink(reportXmlPath).catch(() => {});
      const rangeEnd = estimateBraceBlockEndLine(absTargetFile, targetStartLine) || targetStartLine;
      const { executedInRange, trackedInRange } = parseJacocoXmlReport(xmlText, targetPath.replace(/\\/g, '/'), targetStartLine, rangeEnd);
      return {
        run, appended,
        coverageConfirmed: trackedInRange > 0,
        executed: executedInRange > 0,
        executedInRange, trackedInRange,
        reason: trackedInRange === 0 ? 'Coverage report had no data for this line range.' : undefined,
      };
    } finally {
      fs.promises.unlink(execFile).catch(() => {});
    }
  }

  private async runJavaTestFileForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    const requestId = String(message?.requestId || '');
    const payload = await this.computeJavaTestRunPayload(message, folder);
    this.postTestRunResult({ requestId, ...payload }, message);
  }

  // Split out from runJavaTestFileForResult (same reasoning as Go/Python's
  // compute/post split) so runExistingTestsForResult's auto-fire "does an
  // existing test actually cover this?" check can reuse this for Java too.
  private async computeJavaTestRunPayload(message: any, folder: vscode.WorkspaceFolder): Promise<any> {
    const file = String(message?.file || '');
    const targetPath = String(message?.targetPath || '').replace(/\\/g, '/');
    const targetStartLine = Number(message?.targetStartLine || 0);
    const buildRoot = findJavaBuildRootDir(folder);
    if (!buildRoot) {
      return { ok: false, error: 'No pom.xml or build.gradle found at the workspace root — CODEMD needs a Maven or Gradle project to run a Java test.' };
    }
    if (buildRoot.tool === 'gradle') {
      return { ok: false, error: 'Running a generated test directly isn\'t implemented yet for Gradle projects (Maven is supported). Open the file and run it from your IDE/Gradle for now.' };
    }
    // The generated test's own `import org.junit.jupiter.api.*` needs a real
    // junit-jupiter jar on the compile classpath step 4 below builds from
    // `mvn dependency:build-classpath` — if this project has never declared
    // JUnit at all, that classpath won't have it and javac will fail with a
    // confusing "package org.junit.jupiter.api does not exist". Check first,
    // cheaply, same as the JS/TS path does before ever spending real time.
    if (!detectJavaTestRunner(folder).found) {
      return {
        ok: false,
        noTestRunner: true,
        error: 'No JUnit (or TestNG) dependency declared in pom.xml. CODEMD will not add one on its own — click "Set up test runner" or add one yourself, then Run again.',
      };
    }
    const env = { ...process.env };
    const absTestFile = path.join(folder.uri.fsPath, file);
    const source = await fs.promises.readFile(absTestFile, 'utf8').catch(() => '');
    const packageMatch = source.match(/^\s*package\s+([\w.]+)\s*;/m);
    const className = path.basename(file, '.java');
    const fqcn = packageMatch ? `${packageMatch[1]}.${className}` : className;
    const classpathFile = path.join(os.tmpdir(), `codemd-javacp-${crypto.randomBytes(6).toString('hex')}.txt`);
    const compileOutDir = path.join(os.tmpdir(), `codemd-javaout-${crypto.randomBytes(6).toString('hex')}`);
    let output = '';
    try {
      // 1) Ensure main + existing test classes are actually compiled and up
      // to date — the generated test may reference either.
      const compileStep = await execCommandAsync('mvn', ['-q', '-B', 'test-compile'], buildRoot.root, env, 180000);
      output += `${compileStep.stdout}\n${compileStep.stderr}`.trim();
      if (compileStep.spawnError) {
        return { ok: false, error: `Couldn't launch "mvn": ${compileStep.spawnError}. Is Maven installed and on PATH?`, output: output.slice(-4000) };
      }
      if (compileStep.status !== 0) {
        return { ok: false, error: 'This project itself does not currently compile (`mvn test-compile` failed) — not something CODEMD generated. See output.', output: output.slice(-4000) };
      }
      // 2) Resolve this project's real dependency classpath.
      const cpStep = await execCommandAsync('mvn', ['-q', '-B', 'dependency:build-classpath', `-Dmdep.outputFile=${classpathFile}`], buildRoot.root, env, 120000);
      output += `\n${cpStep.stdout}\n${cpStep.stderr}`.trim();
      if (cpStep.status !== 0 || !fs.existsSync(classpathFile)) {
        return { ok: false, error: 'Could not resolve this project\'s Maven dependency classpath (`mvn dependency:build-classpath` failed). See output.', output: output.slice(-4000) };
      }
      const depClasspath = (await fs.promises.readFile(classpathFile, 'utf8')).trim();
      const projectClasspath = [
        depClasspath,
        path.join(buildRoot.root, 'target', 'classes'),
        path.join(buildRoot.root, 'target', 'test-classes'),
      ].filter(Boolean).join(path.delimiter);
      // 3) Fetch (or reuse the already-cached) JUnit Console Launcher — a
      // normal Maven dependency resolve, so it lands in the user's own
      // local ~/.m2 cache exactly like any other artifact this project uses.
      const consoleVersion = '1.10.2';
      const consoleCoord = `org.junit.platform:junit-platform-console-standalone:${consoleVersion}`;
      const consoleJar = path.join(os.homedir(), '.m2', 'repository', 'org', 'junit', 'platform', 'junit-platform-console-standalone', consoleVersion, `junit-platform-console-standalone-${consoleVersion}.jar`);
      if (!fs.existsSync(consoleJar)) {
        const getStep = await execCommandAsync('mvn', ['-q', '-B', 'dependency:get', `-Dartifact=${consoleCoord}`], buildRoot.root, env, 120000);
        output += `\n${getStep.stdout}\n${getStep.stderr}`.trim();
        if (getStep.status !== 0 || !fs.existsSync(consoleJar)) {
          return { ok: false, error: `Could not fetch the JUnit Platform Console Launcher (${consoleCoord}) via Maven. See output.`, output: output.slice(-4000) };
        }
      }
      // 4) Compile just the one generated file against that classpath.
      await fs.promises.mkdir(compileOutDir, { recursive: true });
      const javac = await execCommandAsync('javac', ['-cp', projectClasspath, '-d', compileOutDir, absTestFile], buildRoot.root, env, 60000);
      output += `\n${javac.stdout}\n${javac.stderr}`.trim();
      if (javac.spawnError) {
        return { ok: false, error: `Couldn't launch "javac": ${javac.spawnError}. Is a JDK installed and on PATH?`, output: output.slice(-4000) };
      }
      if (javac.status !== 0) {
        return { ok: false, error: 'The generated test does not compile — see output.', output: output.slice(-4000) };
      }
      // 5) Execute it with the Console Launcher (wrapped with a JaCoCo
      // javaagent so pass/fail comes with real coverage confirmation, same
      // tier Python/Go already have) and read the real exit code.
      const runClasspath = [projectClasspath, compileOutDir].join(path.delimiter);
      const absTargetFile = path.join(folder.uri.fsPath, targetPath || file);
      const classDirs = [path.join(buildRoot.root, 'target', 'classes'), path.join(buildRoot.root, 'target', 'test-classes'), compileOutDir];
      const launcherResult = await this.runJvmCoverageConfirmedLauncher(
        consoleJar, buildRoot.root, runClasspath, fqcn, classDirs, targetPath, targetStartLine, absTargetFile, env,
      );
      const run = launcherResult.run;
      output += launcherResult.appended;
      if (run.spawnError) {
        return { ok: false, error: `Couldn't launch "java": ${run.spawnError}. Is a JDK installed and on PATH?`, output: output.slice(-4000) };
      }
      if (run.timedOut) {
        return { ok: false, error: 'Test run timed out after 90s.', output: output.slice(-4000) };
      }
      const combined = output.toLowerCase();
      const couldNotRun = combined.includes('no tests found') || combined.includes('0 tests found');
      if (couldNotRun) {
        return { ok: false, error: `The Console Launcher found no tests in "${fqcn}" — see output.`, output: output.slice(-4000) };
      }
      return {
        ok: true,
        passed: run.status === 0,
        coverageConfirmed: launcherResult.coverageConfirmed,
        executed: launcherResult.executed,
        executedInRange: launcherResult.executedInRange,
        trackedInRange: launcherResult.trackedInRange,
      ...
      fs.promises.unlink(classpathFile).catch(() => {});
      fs.promises.rm(compileOutDir, { recursive: true, force: true }).catch(() => {});
    }
  }

  /**
   * Kotlin's direct-run tier — a template copy of runJavaTestFileForResult
   * above, not a new design: same Maven/Gradle root-finding
   * (findJavaBuildRootDir is already language-agnostic despite its name —
   * it just checks for pom.xml/build.gradle, which a Kotlin/JVM project
   * uses identically to a Java one), same detectJavaTestRunner reuse for
   * the "no test framework declared" check, same JUnit Platform Console
   * Launcher execution. The one real difference: step 4 compiles the
   * generated file with `kotlinc`, not `javac` — everything downstream
   * (the compiled .class files, the classpath, the launcher invocation)
   * doesn't care which compiler produced them, since JUnit runs JVM
   * bytecode either way. This is also why Kotlin needed no
   * setUpKotlinTestRunnerForResult of its own — see the 'kotlin' branch in
   * the extension's setUpTestRunner message dispatch, which just routes
   * straight to setUpJavaTestRunnerForResult unchanged.
   * NOTE: like the Rust/Java/C# runners, this could not be empirically
   * verified against a real Kotlin project — no Kotlin compiler available
   * in this dev environment. Should be checked against a real project
   * before relying on it in production.
   */
  private async runKotlinTestFileForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    const requestId = String(message?.requestId || '');
    const payload = await this.computeKotlinTestRunPayload(message, folder);
    this.postTestRunResult({ requestId, ...payload }, message);
  }

  // Split out from runKotlinTestFileForResult (same reasoning as Go/Python's
  // compute/post split) so runExistingTestsForResult's auto-fire "does an
  // existing test actually cover this?" check can reuse this for Kotlin too.
  private async computeKotlinTestRunPayload(message: any, folder: vscode.WorkspaceFolder): Promise<any> {
    const file = String(message?.file || '');
    const targetPath = String(message?.targetPath || '').replace(/\\/g, '/');
    const targetStartLine = Number(message?.targetStartLine || 0);
    const buildRoot = findJavaBuildRootDir(folder);
    if (!buildRoot) {
      return { ok: false, error: 'No pom.xml or build.gradle found at the workspace root — CODEMD needs a Maven or Gradle project to run a Kotlin test.' };
    }
    if (buildRoot.tool === 'gradle') {
      return { ok: false, error: 'Running a generated test directly isn\'t implemented yet for Gradle projects (Maven is supported). Open the file and run it from your IDE/Gradle for now.' };
    }
    ...
    const absTestFile = path.join(folder.uri.fsPath, file);
    const source = await fs.promises.readFile(absTestFile, 'utf8').catch(() => '');
    const packageMatch = source.match(/^\s*package\s+([\w.]+)/m);
    const className = path.basename(file, '.kt');
    const fqcn = packageMatch ? `${packageMatch[1]}.${className}` : className;
    const classpathFile = path.join(os.tmpdir(), `codemd-kotlincp-${crypto.randomBytes(6).toString('hex')}.txt`);
    const compileOutDir = path.join(os.tmpdir(), `codemd-kotlinout-${crypto.randomBytes(6).toString('hex')}`);
    let output = '';
    try {
      // 1) Ensure main + existing test classes are actually compiled and up
      // to date — relies on this project's own pom.xml already wiring the
      // kotlin-maven-plugin into the standard compile/test-compile phases,
      // same as any other Maven project's own build.
      const compileStep = await execCommandAsync('mvn', ['-q', '-B', 'test-compile'], buildRoot.root, env, 180000);
      output += `${compileStep.stdout}\n${compileStep.stderr}`.trim();
      if (compileStep.spawnError) {
        return { ok: false, error: `Couldn't launch "mvn": ${compileStep.spawnError}. Is Maven installed and on PATH?`, output: output.slice(-4000) };
      ...
      // 3) Fetch (or reuse the already-cached) JUnit Console Launcher.
      const consoleVersion = '1.10.2';
      const consoleCoord = `org.junit.platform:junit-platform-console-standalone:${consoleVersion}`;
      const consoleJar = path.join(os.homedir(), '.m2', 'repository', 'org', 'junit', 'platform', 'junit-platform-console-standalone', consoleVersion, `junit-platform-console-standalone-${consoleVersion}.jar`);
      if (!fs.existsSync(consoleJar)) {
      ...
      // 4) Compile just the one generated file against that classpath —
      // kotlinc, not javac, is the one real difference from Java's version.
      await fs.promises.mkdir(compileOutDir, { recursive: true });
      const kotlinc = await execCommandAsync('kotlinc', ['-cp', projectClasspath, '-d', compileOutDir, absTestFile], buildRoot.root, env, 120000);
      output += `\n${kotlinc.stdout}\n${kotlinc.stderr}`.trim();
      if (kotlinc.spawnError) {
        return { ok: false, error: `Couldn't launch "kotlinc": ${kotlinc.spawnError}. Is the Kotlin compiler installed and on PATH? (It's a separate install from the JDK.)`, output: output.slice(-4000) };
      }
      if (kotlinc.status !== 0) {
        return { ok: false, error: 'The generated test does not compile — see output.', output: output.slice(-4000) };
      }
      // 5) Execute it with the Console Launcher (wrapped with a JaCoCo
      // javaagent so pass/fail comes with real coverage confirmation, same
      // tier Java/Python/Go already have) and read the real exit code.
      const runClasspath = [projectClasspath, compileOutDir].join(path.delimiter);
      const absTargetFile = path.join(folder.uri.fsPath, targetPath || file);
      const classDirs = [path.join(buildRoot.root, 'target', 'classes'), path.join(buildRoot.root, 'target', 'test-classes'), compileOutDir];
      const launcherResult = await this.runJvmCoverageConfirmedLauncher(
  ...
  /**
   * C#'s direct-run tier — a genuinely different, simpler strategy than
   * Java's javac+Console-Launcher approach, not the same trick ported over.
   * Rather than resolving the TARGET project's own classpath and hoping it
   * already has a test framework on it (Java's approach, which is why Java
   * needed detectJavaTestRunner/setUpJavaTestRunnerForResult as a
   * prerequisite), this builds one throwaway SDK-style project per run in a
   * temp directory: a fresh `<PackageReference>` on xUnit (always added,
   * regardless of what the target project has), plus a `<ProjectReference>`
   * to the target project so the code under test is visible, plus a copy of
   * the one generated test file (the only .cs file in that directory, so
   * `dotnet test` unambiguously runs just it — no --filter/class-name
   * guessing needed). This is why C# has no setUpCsharpTestRunnerForResult:
   * the ad-hoc run path is self-sufficient and never depends on the user's
   * own project already having a test framework declared. (A *permanent*,
   * visible test project added to the user's own solution — the C#
   * equivalent of Java's harness-install philosophy — remains a worthwhile
   * but separate follow-up; see TODO.md.)
   * NOTE: like the Rust and Java runners, this could not be empirically
   * verified against a real .NET project in this session (no .NET SDK
   * available in this dev environment) — the ephemeral-project structure
   * follows standard, documented `dotnet test`/SDK-style-project conventions,
   * but should be checked against a real project before relying on it in
   * production.
   */
  private async runCsharpTestFileForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    const requestId = String(message?.requestId || '');
    const payload = await this.computeCsharpTestRunPayload(message, folder);
    this.postTestRunResult({ requestId, ...payload }, message);
  }

  // Split out from runCsharpTestFileForResult (same reasoning as Go/Python's
  // compute/post split) so runExistingTestsForResult's auto-fire "does an
  // existing test actually cover this?" check can reuse this for C# too.
  private async computeCsharpTestRunPayload(message: any, folder: vscode.WorkspaceFolder): Promise<any> {
    const file = String(message?.file || '');
    const targetPath = String(message?.targetPath || '');
    const targetStartLine = Number(message?.targetStartLine || 0);
    const absTestFile = path.join(folder.uri.fsPath, file);
    const absTargetFile = path.join(folder.uri.fsPath, targetPath || file);
    const targetCsproj = findCsprojFor(absTargetFile);
    if (!targetCsproj) {
      return { ok: false, error: 'No .csproj found for the target file — CODEMD needs the code under test to belong to a real C# project to reference it.' };
    }
    const tfm = targetFrameworkFor(targetCsproj);
    const workDir = path.join(os.tmpdir(), `codemd-csharp-${crypto.randomBytes(6).toString('hex')}`);
    const resultsDir = path.join(workDir, 'TestResults');
    const env = { ...process.env };
    let output = '';
    try {
      await fs.promises.mkdir(workDir, { recursive: true });
      await fs.promises.copyFile(absTestFile, path.join(workDir, path.basename(absTestFile)));
      const csprojContent = [
        ...
        '  </ItemGroup>',
        '  <ItemGroup>',
        `    <ProjectReference Include="${targetCsproj}" />`,
        '  </ItemGroup>',
        '</Project>',
        '',
      ].join('\n');
      await fs.promises.writeFile(path.join(workDir, 'CodemdGeneratedTest.csproj'), csprojContent, 'utf8');
      const run = await execCommandAsync(
        'dotnet',
        ['test', '-v', 'minimal', '--collect:XPlat Code Coverage', '--results-directory', resultsDir],
        workDir, env, 180000,
      );
      output = `${run.stdout}\n${run.stderr}`.trim();
      if (run.spawnError) {
        return { ok: false, error: `Couldn't launch "dotnet": ${run.spawnError}. Is the .NET SDK installed and on PATH?`, output: output.slice(-4000) };
      }
      if (run.timedOut) {
        return { ok: false, error: 'Test run timed out after 180s.', output: output.slice(-4000) };
      }
      const combined = output.toLowerCase();
      const couldNotRun = combined.includes('build failed')
        || combined.includes('error cs')
        || combined.includes('no test is available');
      if (couldNotRun) {
        return { ok: false, error: 'The generated test project did not build or collect any tests — see output.', output: output.slice(-4000) };
      }
      const passed = run.status === 0;
      const coberturaPath = findFileRecursive(resultsDir, 'coverage.cobertura.xml');
      if (!targetPath || !targetStartLine || !coberturaPath) {
        return {
          ok: true, passed, coverageConfirmed: false,
          reason: !coberturaPath ? 'No coverage report was produced (coverlet.collector may have failed to run).' : 'No target line range available to check.',
          output: output.slice(-4000),
        };
      }
      const xmlText = await fs.promises.readFile(coberturaPath, 'utf8');
      const rangeEnd = estimateBraceBlockEndLine(absTargetFile, targetStartLine) || targetStartLine;
      const { executedInRange, trackedInRange } = parseCoberturaCoverage(xmlText, targetPath.replace(/\\/g, '/'), targetStartLine, rangeEnd);
      return {
        ok: true,
        passed,
        coverageConfirmed: trackedInRange > 0,
  ...
  /**
   * For JS/TS test files, split into two phases so Claude is only ever
   * trusted for the one thing that genuinely needs project-specific
   * judgment, never for the result itself:
   *   1. Determine the exact run command. For a known runner with a
   *      well-understood single-file invocation (vitest/jest/mocha/ava —
   *      see buildDeterministicJsRunCommand), this is built directly from
   *      detectJsTestRunner's own findings with NO Claude call at all —
   *      CODEMD already knows the runner and the file, so there is nothing
   *      left for Claude to determine. Only a genuinely ambiguous case (a
   *      generic package.json "test" script, or an uncommon runner) falls
   *      back to asking Claude (read-only — Read/Grep/Glob, no Bash) to work
   *      out the invocation by inspecting package.json/Makefile/build files.
   ...
   * Step 2 wraps the command with `c8` (works against Jest/Vitest/mocha/
   * `node --test` alike, no per-framework special-casing needed) to get real
   * Istanbul coverage data, so pass/fail and coverage are both measured
   * directly rather than asserted by an LLM reading its own output. The
   * `viaClaude` flag on the result reflects only whether phase 1 actually
   * fell back to Claude — the UI uses it to decide whether "Run again" would
   * cost a real Claude call.
   */
  private async runTestViaClaudeForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    ...
    const targetPath = String(message?.targetPath || '');
    const targetStartLine = Number(message?.targetStartLine || 0);
    // A repo with zero JS/TS test tooling at all is the common case CODEMD
    // exists for, not an edge case — check for that up front (no Claude
    // call, just fs reads) instead of spending 90-120s asking Claude to
    // "discover" a command that can only ever fail to exist. CODEMD never
    // installs a test framework on your behalf (see detectJsTestRunner) —
    // this only reports what's already there.
    let detected: { found: boolean; runner?: string; dir?: string } = { found: false };
    if (testLanguageFor(file) === 'javascript') {
      // The generated test itself always lives under a fixed, workspace-root
      // .codemd/generated_tests/ directory regardless of which package it's
      // testing — scoping the search to `file` would walk up from THAT
      // location and never reach the real package (e.g. apps/storefront) at
      // all. targetPath (the actual source file under test) is what's really
      // inside the right package directory in a monorepo; only fall back to
      // `file` when there's no target (e.g. a hand-authored test).
      detected = detectJsTestRunner(folder, targetPath || file);
      if (!detected.found) {
        this.postTestRunResult({
          requestId,
          ok: false,
          // Deliberately NOT environmentIssue — that flag offers a "Fix
          // environment issue with Claude" button, and the whole point here
          // is that Claude should not freelance a test-framework install as
          // a "fix" (see fixTestFailureForResult's environment-issue prompt).
          // noTestRunner suppresses the Fix button entirely instead.
          noTestRunner: true,
          error: 'No JS/TS test runner detected in this repo (checked package.json dependencies/scripts and jest/vitest/mocha/ava config files). '
            + 'CODEMD will not install a test framework on its own — add one yourself (e.g. `npm install --save-dev vitest` plus a "test" script) and click Run again.',
        }, message);
        return;
      }
    }
    ...
    const costCapUsd = this.maxCostPerActionUsd();
    // Skip Claude entirely for the common case: a known runner (vitest/jest/
    // mocha/ava) with a well-understood single-file invocation. Claude adds
    // no value reading package.json and telling us what we already just
    // determined ourselves — see buildDeterministicJsRunCommand.
    const deterministic = buildDeterministicJsRunCommand(folder, detected, file);
    let usedClaudeDiscovery = false;
    if (deterministic) {
      command = deterministic.command;
      cwdRel = deterministic.cwdRel;
    } else if (this.getClaudeAgentMode() === 'promptOnly') {
      // Nothing for CODEMD to parse back in this mode (no captured stdout),
      // so there is no point splitting discovery from execution the way the
      // 'agent' branch below does — just ask the user's own Claude to do
      // both and report the result in chat.
      const interactivePrompt = `Determine the exact single command needed to run ${testDescription} using this project's own test`
        + ' tooling (check package.json scripts, a Makefile, or other build files as needed to find the right test runner and'
        + ' invocation syntax, and how to run just this one test if the tooling supports it), then run it yourself directly in'
        + ' this terminal and tell me whether it passed.';
      const sent = await sendPromptToUsersClaudeTerminal(folder, interactivePrompt);
      this.postTestRunResult({ requestId, ok: sent.ok, sentToTerminal: true, error: sent.ok ? undefined : sent.error }, message);
      return;
    } else {
      ...
      const claudeCommand = await resolveCommand('claude');
      if (!claudeCommand) {
        this.postTestRunResult({ requestId, ok: false, error: 'Claude Code CLI was not found on PATH or bundled with the Claude Code extension.' }, message);
        return;
      }
      usedClaudeDiscovery = true;
      try {
        const discover = await runClaudeCliExclusive(() => execCommandAsync(
          claudeCommand,
          [
            '-p', discoveryPrompt,
            '--max-turns', '6',
        ...
        if (discover.spawnError) {
          this.postTestRunResult({ requestId, ok: false, error: `Couldn't launch "claude": ${discover.spawnError}. Is Claude Code CLI installed and on PATH?` }, message);
          return;
        }
        if (discover.timedOut) {
          this.postTestRunResult({ requestId, ok: false, error: 'Claude did not finish finding a test command within 90s.', output: discoveryOutput }, message);
          return;
        }
        if (discover.status !== 0) {
          const budgetError = parseClaudeBudgetError(discover.stdout, costCapUsd);
          const turnsError = parseClaudeMaxTurnsError(discover.stdout, 6);
          this.postTestRunResult({ requestId, ok: false, error: budgetError || turnsError || 'Claude Code CLI exited with an error — see output. It may not be logged in (run `claude` and sign in, then try again).', output: discoveryOutput }, message);
          return;
        }
        let envelope: any;
        try {
          ...
          this.postTestRunResult({ requestId, ok: false, error: 'Could not parse a response from Claude Code CLI.', output: discoveryOutput }, message);
          return;
        }
        const structured = envelope?.structured_output;
        ...
        if (!command) {
          this.postTestRunResult({ requestId, ok: false, error: 'Claude could not determine a test command for this project.', output: discoveryOutput }, message);
          return;
        }
      } catch (err: any) {
        this.postTestRunResult({ requestId, ok: false, error: err?.message || String(err) }, message);
        return;
      }
    }
    ...
    // NO_COLOR/FORCE_COLOR=0: this command runs through a real shell
    // (execShellLineAsync), and on Windows several color-detection libraries
    // (chalk, picocolors, supports-color — which vitest/vite pull in) assume
    // ANSI support from the platform alone rather than checking whether
    // stdout is actually a TTY, so the raw escape codes were landing
    // verbatim in the captured output shown by "View output." Both env vars
    // are the same standard toggle every one of those libraries already
    // honors; setting both covers whichever one a given tool checks.
    ...
    const claudeNote = usedClaudeDiscovery ? ` (command found by Claude: \`${command}\`)` : '';
    try {
      ...
      const rawOutput = `${run.stdout}\n${run.stderr}`.trim().slice(-4000);
      if (run.spawnError) {
        this.postTestRunResult({ requestId, ok: false, error: `Couldn't launch the test command: ${run.spawnError}`, output: rawOutput + claudeNote }, message);
        return;
      }
      if (run.timedOut) {
        this.postTestRunResult({ requestId, ok: false, error: 'Test run timed out after 120s.', output: rawOutput + claudeNote }, message);
        return;
      }
      const passed = run.status === 0;
      if (!usedCoverageWrap || !targetPath || !targetStartLine) {
        this.postTestRunResult({
          requestId, ok: true, passed, viaClaude: usedClaudeDiscovery, coverageConfirmed: false,
  ...
  private async fixTestFailureForResult(message: any, folder: vscode.WorkspaceFolder): Promise<void> {
    ...
    const kind: 'environment' | 'bug' = isEnvironmentIssue ? 'environment' : 'bug';
    const testDescription = nodeId ? `the test "${nodeId}" in "${file}"` : `the test file "${file}"`;
    ...
    const resolveDefectIfApplicable = (filesChangedList: string[], passedAfterFix: boolean | null): ('code' | 'test' | undefined) => {
      if (isEnvironmentIssue || !targetPath || !targetSymbol || passedAfterFix === false) { return undefined; }
      ...
      let resolvedKind: 'code' | 'test' | undefined;
      if (touchedSource) { resolvedKind = 'code'; }
      else if (touchedTest) { resolvedKind = 'test'; }
      else if (passedAfterFix === true) { resolvedKind = 'test'; }
      if (resolvedKind) {
    ...
    const prompt = isEnvironmentIssue
      ? [
        'Your shell is already running with its working directory set to this project\'s root — run commands directly from here,',
        'do not `cd` into it first, and do not hunt for or switch to a different Python/Node virtual environment than what\'s',
        'already on PATH unless the project\'s own docs specifically say to use one.',
        ...
        `${targetDescription}'s own behavior or with the test's assertions. Fix ONLY the environment: install a dependency that is`,
        "ALREADY declared in this project's package.json/requirements/etc. but just not installed yet, correct an import path or",
        'sys.path/package setup, or fix existing test/build configuration. Do NOT install a new test framework or runner that is not',
        'already part of this project (e.g. do not add Jest/Vitest/pytest to a project that has none) — if no test runner is configured',
        'for this project at all, that is not something to fix here; say so honestly in your summary and stop instead of installing one.',
        'Only touch the test file itself if the actual problem is in its import/setup lines (e.g. a wrong import path) — never change its',
        "assertions or add mocking to work around the real issue. Do not fix unrelated code, do not refactor, do not add features.",
        'After fixing, rerun the same test yourself to verify it now runs (pass or fail is fine — the goal here is just that it RUNS)',
        'before reporting back. If you cannot make it run, say so honestly rather than reporting success.',
        'If ".codemd/agent_policy/testing.md" exists in this repo, read it for CODEMD\'s testing responsibility policy and follow it',
        'alongside the specific instructions above.',
      ...
      : [
        ...
        // Confirmed against a real run: without this, a failure that is
        // actually structural (a monorepo with no test runner wired up for
        // THIS specific package, code that calls a live backend/database
        // that is not running here, missing secrets/network access) sent
        // Claude down a 220s, ~24-turn spiral of `pnpm install`, hunting
        // through unrelated packages' jest configs, and guessing at
        // commands — then exiting with a generic "may not be logged in"
        // error that had nothing to do with what actually happened. This
        // mirrors the same honest-bailout permission the environment-kind
        // prompt above already has for a missing test runner.
        'If, after investigating, the real reason is that the code under test depends on infrastructure genuinely unavailable in this',
        'environment — a backend/database server that is not running, external network access, unset secrets/env vars — and none of',
        'that can reasonably be started or provided as part of this task, do NOT keep guessing at commands or installing things to work',
        'around it. Stop, say so plainly in your summary (name the specific missing piece), and report passedAfterFix as false — this is',
        'not a bug in the code and should not be reported as one.',
        'Do not fix unrelated code, do not refactor, do not add features.',
        'Your goal is a DEFENSIBLE RESOLUTION, not the fewest possible commands: passed, a real code bug (fixed), a bad generated test',
        '(fixed), or a genuinely unresolved environment problem (reported honestly, see above) — not just "stopped after N tries". Stay',
        'on the single test identified above (never the full test suite, never other test files) UNLESS a specific result is genuinely',
        'ambiguous — e.g. it passed once and failed once with no code change in between — in which case running the broader suite once to',
        'see whether the same odd behavior is isolated to this test or systemic is legitimate evidence-gathering, not waste. A Vite/Vitest',
        'error reading exactly "Cannot read properties of undefined (reading \'config\')" is a known cold-start race, not a real bug — if',
        'you see exactly that, a rerun with no changes is expected to resolve it. More generally: rerun as many times as it actually takes',
        'to tell apart a real regression from test/environment noise, but each rerun should be answering a specific question you don\'t yet',
        'know the answer to — once two consecutive runs under the same code agree, stop investigating further and report that result.',
        'Do NOT clear node_modules, Vite/Vitest/Jest/Turbo/pytest caches, lockfiles, or reinstall dependencies as a debugging step — that',
        'never distinguishes real-vs-transient and only burns turns; if you suspect it might genuinely be needed, that IS an environment',
        'problem, so stop and report it as one instead of trying it. After any fix, rerun to confirm before reporting back. If you cannot',
        'reach a passing result, say so honestly (with your best determination of test-bug vs. code-bug vs. environment) rather than',
        'reporting success or leaving it ambiguous.',
        'If ".codemd/agent_policy/testing.md" exists in this repo, read it for CODEMD\'s testing responsibility policy and follow it',
        'alongside the specific instructions above.',
        'Respond only in the given JSON schema.',
      ].join(' ');
    ...
    if (this.getClaudeAgentMode() === 'promptOnly') {
      const interactivePrompt = `${prompt} You are running interactively in your own terminal via CODEMD's Prompt-Only Mode — ignore the`
        + ' instruction above about responding in a JSON schema; instead make the edits directly yourself, rerun to confirm as instructed'
        + ' above, then briefly summarize in chat what you changed and whether it now passes.';
      const sent = await sendPromptToUsersClaudeTerminal(folder, interactivePrompt);
      this.post({ type: 'fixTestResult', requestId, kind, targetPath, targetSymbol, ok: sent.ok, sentToTerminal: true, error: sent.ok ? undefined : sent.error });
      return;
    }
    if (!(await this.ensureClaudeAgentConsent())) {
    ...
    try {
      ...
      const handleChunk = (text: string, stream: 'stdout' | 'stderr') => {
        if (stream === 'stderr') {
          if (text.trim()) { this.post({ type: 'fixTestProgress', requestId, chunk: text }); }
          return;
        }
        stdoutLineBuffer += text;
        const lines = stdoutLineBuffer.split(/\r?\n/);
      ...
      // Registered before the CLI call is even queued (see
      // runClaudeCliExclusive) so Cancel still works while this fix is
      // waiting its turn behind another in-flight Claude call, not just once
      // it has actually spawned — canceledBeforeStart lets the queued task
      // below skip spawning entirely rather than starting a process just to
      // immediately kill it.
      ...
      type ClaudeCliRunResult = { status: number | null; stdout: string; stderr: string; timedOut: boolean; canceled: boolean; spawnError?: string };
      const promise = runClaudeCliExclusive<ClaudeCliRunResult>(() => {
        if (canceledBeforeStart) {
          return Promise.resolve<ClaudeCliRunResult>({ status: null, stdout: '', stderr: '', timedOut: false, canceled: true });
        }
        const spawned = execCommandCancelableAsync(
          ...
          [
            ...
            // "Fix with Claude just fails for no reason". Same story for a
            // venv-relative interpreter (./.venv/Scripts/python.exe) and a
            // PowerShell equivalent of the same command — a compound
            // `cd X && Y` still won't match any of these prefix patterns
            // (the prompt above tells Claude not to prepend cd at all, since
            // its cwd is already the project root), but the bare command by
            // itself now will. This is a headless process with no human to
            // answer a permission prompt, so anything unlisted here silently
            // burns a turn instead of ever getting approved — see the prompt
            // above and TODO.md for the compound-command gap that remains.
            '--allowedTools', 'Read,Grep,Glob,Edit,'
              + 'Bash(cd *),Bash(python*),Bash(pip*),Bash(pytest*),Bash(./.venv/*),Bash(.venv/*),Bash(./venv/*),Bash(venv/*),'
              + 'Bash(go test*),Bash(go get*),Bash(npm *),Bash(npx *),Bash(yarn *),Bash(pnpm *),Bash(node *),'
              + 'Bash(mvn *),Bash(gradle *),Bash(gradlew*),Bash(./gradlew*),Bash(cargo test*),'
              + 'PowerShell(python*),PowerShell(pytest*),PowerShell(./.venv/*),PowerShell(.venv/*),'
              + 'PowerShell(go test*),PowerShell(npm *),PowerShell(npx *),PowerShell(yarn *),PowerShell(pnpm *),PowerShell(node *),'
              + 'PowerShell(mvn *),PowerShell(gradle *),PowerShell(cargo test*)',
            '--output-format', 'stream-json',
            '--verbose',
            '--json-schema', schema,
            '--max-budget-usd', String(costCapUsd),
      ...
      let run: Awaited<typeof promise>;
      try {
        run = await promise;
      } finally {
      ...
      const envelope = extractClaudeResultEnvelope(run.stdout);
      if (run.status !== 0) {
        // Claude can finish the actual fix — edits landed on disk, its own
        // rerun already confirmed pass/fail, structured_output already
        // written — and the `claude` process itself still exits non-zero
        // afterward (most commonly --max-turns/--max-budget-usd catching it
        // right after that final turn, before it winds down cleanly).
        // Confirmed against a real run: this fell through to the generic
        // "not logged in" message below despite Claude having just spent
        // 210s successfully installing deps, fixing an import, and getting
        // the test to 5/5 passing — recover the real result from the
        // envelope instead of reporting a hard failure that would make the
        // user re-run this (expensive) fix from scratch for no reason.
        const structured = envelope?.structured_output;
        if (structured && typeof structured.summary === 'string') {
          ...
          const defectResolvedKind = resolveDefectIfApplicable(filesChanged, typeof structured.passedAfterFix === 'boolean' ? structured.passedAfterFix : null);
          this.post({
            type: 'fixTestResult',
            requestId,
            kind,
            ...
            defectResolvedKind,
            summary: stripStrayClosingTag(structured.summary) + (budgetNote ? ` (Claude reported this right as it hit its usage cap.)` : ''),
            costUsd: typeof envelope?.total_cost_usd === 'number' ? envelope.total_cost_usd : undefined,
            tokensUsed: tokensFromEnvelope(envelope),
            // Claude's own reported duration_ms in preference to our wall-clock
            // measurement — excludes time this run spent queued behind another
            // in-flight Claude call, so it's the real number for the
            // session-usage tiles.
            durationMs: typeof envelope?.duration_ms === 'number' ? envelope.duration_ms : durationMs(),
            output: rawOutput,
          });
          return;
        }
   ...
   */
  private async runDeletionReportScript(
    folder: vscode.WorkspaceFolder,
    onlyFiles?: string[],
   ...
   * the callgraph.
   */
  private async mostRecentlyChangedFile(folder: vscode.WorkspaceFolder): Promise<string | null> {
    const diff = await execGitAsync(
      ['diff', '--name-only', '-M', 'HEAD', '--', '.', `:!${ARTIFACT_OUTPUT_DIR}/**`, ':!out/**', ':!dist/**', ':!build/**', ':!output*/**', ':!*.vsix', ':!*.pyc', ':!*.pyo'],
      folder.uri.fsPath,
    );
    if (diff.status !== 0) {
      return null;
    ...
    const untracked = await execGitAsync(
      ['ls-files', '--others', '--exclude-standard', '--', '.', `:!${ARTIFACT_OUTPUT_DIR}/**`, ':!out/**', ':!dist/**', ':!build/**', ':!output*/**', ':!*.vsix', ':!*.pyc', ':!*.pyo'],
      folder.uri.fsPath,
    );
  ...
  private async postLatestChangeSketch(folder: vscode.WorkspaceFolder): Promise<void> {
    try {
   ...
   */
  private async postMostRecentChangePreview(folder: vscode.WorkspaceFolder): Promise<void> {
    try {
      const file = await this.mostRecentlyChangedFile(folder);
      if (!file) {
        return;
   ...
   * changed files.
   */
  private buildChangeResults(
    report: any,
    folder: vscode.WorkspaceFolder,
    options?: { changeTimeOverride?: number; diffRefs?: { base: string; target: string } },
    ...
    const changeTimeFor = (file: string) => options?.changeTimeOverride ?? this.changeTimeForFile(folder, file);
    ...
    const uncommittedFiles: any[] = Array.isArray(report.uncommitted_files) ? report.uncommitted_files : [];
    const otherFileRecords = uncommittedFiles.length
      ? uncommittedFiles.filter((file: any) => {
        const filePath = uncommittedFilePath(file);
        return filePath && (uncommittedFileStatus(file) === 'deleted' || !representedFiles.has(filePath));
      })
      : (report.unsupported_files || []).map((file: unknown) => ({ status: 'changed', path: String(file || '') }));
    ...
    if (otherFileRecords.length) {
      ...
      results.push({
        ...
        changeTime: options?.changeTimeOverride ?? Math.max(0, ...otherFilePaths.map((file: string) => this.changeTimeForFile(folder, file))),
        diffBase,
        diffTarget,
      });
    }
    ...
    // One cache read for the whole batch, then a cheap per-symbol lookup —
    // attaches testStatus (see the field's own doc comment) so the webview
    // can hide a generator chip whose test survives from an earlier VS Code
    // session, not just this one.
    const testStatusCache = readTestStatusCache(folder);
    for (const res of results) {
      const sym = res.fullName || res.graphSymbol;
      if (!sym || !res.file || !res.changeCard || (res.changeCard.kind !== 'modified' && res.changeCard.kind !== 'added')) { continue; }
      const generatedKinds = Object.keys(validCachedGeneratedKinds(folder, testStatusCache, res.file, sym));
      const anyValidPassingRun = !!hasValidCachedPassingRun(folder, testStatusCache, res.file, sym);
      if (generatedKinds.length || anyValidPassingRun) {
        res.testStatus = { generatedKinds, anyValidPassingRun };
      }
    }
    return results;
  }

  async runChangesCheck(options: { focusHighestImpact?: boolean; quietIfBusy?: boolean } = {}): Promise<void> {
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder) {
      return;
    }
    ...
    try {
      ...
      // Covers Python, JavaScript/TypeScript, Java, C#, Go, Rust, and Kotlin (findSameCommitCallChains
      // reads whichever languages' callgraphs the changed results touch) —
      // a no-op for any other language, so this never fails elsewhere.
      ...
      const summaryTiles = buildChangesSummaryTiles(report, results, callChains);
      ...
      const resultMessage = {
        ...
        defaultSort: 'impact',
        results,
        callChains: serializeCallChains(callChains, folder),
      };
      this.rememberSearchResult(resultMessage);
      this.post(resultMessage);
  ...
  /**
   * ⚡ Auto Mode — one click processes every medium/high/removed-priority
   * uncommitted change SERIALLY: check existing tests (local, free, any
   * language with a direct runner) → generate EVERY applicable test, not
   * just one (new-function for an added symbol; contract for a Python
   * function with proven-broken callers; multi-caller for one with 2+
   * confirmed callers; call-path as the catch-all otherwise — contract can
   * stack with multi-caller/call-path on the same function, since it's an
   * independent signal) → run each → if a run genuinely fails, ask Claude
   * to fix it, then re-run once → move on. Every step reuses the exact same
   * server method a manual button click would call (via awaitPostedResult,
   * see post() above), so behavior — including defect recording via
   * recordDefectIfGenuine/markDefectStatus inside postTestRunResult/
   * fixTestFailureForResult, and activity.md logging via
   * maybeAppendReportEntry inside post() itself — is identical to doing it
   * by hand, just sequenced instead of requiring N clicks per function.
   *
   * "Low priority" (excluded) = cosmeticOnly changes and file/blastRadius
   * summary rows. Among the rest, blast-radius-qualifying changes (see
   * blastRadiusEntriesFromReport) are processed first.
   *
   * Deliberately un-capped: codemdGraphs.maxAutoWritesPerBatch no longer
   * bounds this run (comprehensive coverage was explicitly requested over
   * an artificially short batch) — the only remaining spending guardrails
   * are the per-call codemdGraphs.maxCostPerActionUsd (enforced deeper
   * inside each Claude CLI invocation, same as every other Claude action)
   * and the user's own Stop button, fed by the running claudeCallsUsed
   * count included in every progress post so they can see spend build up
   * live and stop whenever they've seen enough.
   */
  private async runAutoMode(): Promise<void> {
    ...
    // untended prompts into the user's terminal (or falling through to the
    // generic Manual/Use Coding Agent popup, which would look like a
    // non-sequitur after they deliberately picked Prompt-Only), this is
    // refused with an explanation naming the actual conflict.
    if (this.getClaudeAgentMode() === 'promptOnly') {
      this.post({ type: 'autoModeProgress', done: true, error: 'Auto Mode runs an unattended serial pipeline and can\'t pause for you to review each prompt. Switch to "Use Coding Agent" in the Modes menu to run Auto Mode, or use the individual Claude-powered buttons while in Prompt-Only Mode.' });
      return;
    }
    if (!(await this.ensureClaudeAgentConsent())) {
    ...
    let claudeCallsUsed = 0;
    const DIRECT_TEST_EXTENSIONS = ['.py', '.go', '.rs', '.java', '.cs', '.kt', '.kts'];
    const GEN_KIND_LABELS: Record<string, string> = {
      new: 'New-Function Test',
      contract: 'Contract Test',
      broad: 'Multi-Caller Test',
      callpath: 'Call-Path Test',
    };
    // Auto Mode's short genType codes vs. the cache's kind names (matching
    // generatorKindFromTestFileName's output, shared with the manual UI).
    const GEN_TYPE_TO_CACHE_KIND: Record<string, string> = {
      new: 'new_function', contract: 'contract', broad: 'broad_coverage', callpath: 'call_path',
    };
    const summary: Array<{ symbol: string; file: string; status: string; testKind?: string; testFile?: string; steps: string[] }> = [];
    try {
      ...
      const queue = allResults
        .filter((r: any) => r.changeCard && (r.changeCard.kind === 'modified' || r.changeCard.kind === 'added' || r.changeCard.kind === 'removed') && !r.changeCard.cosmeticOnly && !isLikelyTestFilePath(String(r.file || '')))
        .sort((a: any, b: any) => {
      ...
      for (let i = 0; i < queue.length; i++) {
        ...
        const progress = (text: string, activeRequestId?: string, activeHasOutput?: boolean) => this.post({
          ...
          activeHasOutput: !!activeHasOutput, claudeCallsUsed,
        });

        // Step 1 — existing tests, any language with a direct local runner
        // (Python/Go/Rust/Java/C#/Kotlin — the same set Step 3 below treats
        // as "direct"). Free, no Claude call, always runs. A language
        // without a direct runner (e.g. JS/TS) still gets its matches
        // listed for visibility, just not auto-run — verifying it would
        // itself cost a Claude call, defeating the point of a free check.
        // Whole-file, not node-scoped like the old Python-only version
        // (runExistingTestsForResult) — coarser, so an unrelated failing
        // test in the same file can cause a false "not covered" here, but
        // never a false "covered": worst case this generates a test that
        // turns out to be unnecessary, never skips one that was needed.
        //
        // Before any of that: check the disk-backed test-status cache first
        // — if this exact function (by content hash) already has a
        // confirmed-passing, coverage-confirmed test on record from an
        // earlier run (this Auto Mode run or any prior one), skip straight
        // to "covered" without spawning anything at all. This is what makes
        // re-running Auto Mode on an unchanged function nearly free instead
        // of redoing the same discovery+run work every time.
        let covered = false;
        const coveredEntry: { symbol: string; file: string; status: string; steps: string[] } = { symbol, file, status: 'unknown', steps: [] };
        const cache = readTestStatusCache(folder);
        const cachedPass = hasValidCachedPassingRun(folder, cache, file, symbol);
        if (cachedPass) {
          covered = true;
          coveredEntry.status = 'already-covered';
          coveredEntry.steps.push(`Cached: ${cachedPass.testFile} already passed with coverage confirmed on ${new Date(cachedPass.ranAt).toLocaleString()}, and nothing has changed since — skipped without running anything.`);
        }
        if (!covered) {
          const existingReqId = 'automode_existing_' + (this.autoModeRequestSeq++) + '_' + Date.now();
          progress(`Checking existing tests for ${symbol}…`, existingReqId, false);
          const scriptPath = vscode.Uri.joinPath(this.context.extensionUri, 'scripts', 'codemd-mcp-server.js').fsPath;
          if (symbol && fs.existsSync(scriptPath)) {
            try {
              const { status, stdout, timedOut } = await execNodeCliAsync(
                scriptPath,
                ['--workspace', folder.uri.fsPath, '--cli', 'find_tests', '--query', symbol, '--limit', '5'],
                folder.uri.fsPath,
              );
              if (!timedOut && status === 0) {
                const data = JSON.parse(stdout);
                ...
                const rawMatches: any[] = data.method === 'callgraph' && Array.isArray(data.matches) ? data.matches.slice(0, 3) : [];
                ...
                if (skippedMatches.length && !directMatches.length) {
                  coveredEntry.steps.push(`Found ${skippedMatches.length} existing test file(s) (${skippedMatches.map((m) => m.file).join(', ')}) but this language has no local runner to auto-verify them — generating anyway.`);
                }
                for (const m of directMatches) {
                  const matchFile = String(m.file || '');
                  const runReqId = 'automode_existingrun_' + (this.autoModeRequestSeq++) + '_' + Date.now();
                  void this.runTestFileForResult({ requestId: runReqId, file: matchFile, targetPath: file, targetStartLine: startLine, targetSymbol: symbol });
                  const runResult = await this.awaitPostedResult(runReqId, 120000);
                  if (runResult?.ok && runResult.passed && runResult.coverageConfirmed && runResult.executed) {
                    covered = true;
                    coveredEntry.status = 'already-covered';
                    coveredEntry.steps.push(`Existing test ${matchFile} already passes and confirms coverage — nothing to do.`);
                    break;
                  }
                }
              }
            } catch {
              // Discovery hiccup — fall through and generate rather than
              // stalling the whole run on one function's lookup failure.
            }
          }
        }
        if (covered) {
        ...
        // first match. Contract is an independent signal (a proven-broken
        // caller) and can stack with multi-caller or call-path on the same
        // modified function.
        ...
        const genTypes: Array<'new' | 'contract' | 'broad' | 'callpath'> = [];
        if (kind === 'added') { genTypes.push('new'); }
        if (kind === 'removed') { genTypes.push('callpath'); }
        if (kind === 'modified') {
          if (file.toLowerCase().endsWith('.py') && hasBrokenCallers) { genTypes.push('contract'); }
          if (impactedCount >= 2) { genTypes.push('broad'); } else { genTypes.push('callpath'); }
        }
        ...
        for (const genType of genTypes) {
          ...
          const entry: { symbol: string; file: string; status: string; testKind?: string; testFile?: string; steps: string[] } = { symbol, file, status: 'unknown', testKind: GEN_KIND_LABELS[genType], steps: [] };
          const cacheKind = GEN_TYPE_TO_CACHE_KIND[genType];

          // Reuse an already-generated test for this exact kind if the
          // target hasn't changed since — same cache as Step 1, scoped to
          // this one kind (validCachedGeneratedKinds), so re-running Auto
          // Mode never regenerates a test that's still valid.
          let genMsg: any;
          const cachedTestFile = validCachedGeneratedKinds(folder, cache, file, symbol)[cacheKind];
          if (cachedTestFile) {
            genMsg = { ok: true, path: cachedTestFile };
            entry.testFile = cachedTestFile;
            entry.steps.push(`Reusing existing ${GEN_KIND_LABELS[genType].toLowerCase()}: ${cachedTestFile} — target unchanged since it was generated, skipped regenerating.`);
          } else {
            const genReqId = 'automode_gen_' + (this.autoModeRequestSeq++) + '_' + Date.now();
            claudeCallsUsed++;
            progress(`Generating a ${GEN_KIND_LABELS[genType].toLowerCase()} for ${symbol}…`, genReqId, true);
            if (genType === 'new') { void this.generateNewFunctionTestForResult(r, genReqId); }
            else if (genType === 'contract') { void this.generateContractTestForResult(r, genReqId); }
            else if (genType === 'broad') { void this.generateBroadCoverageTestForResult(r, genReqId); }
            else { void this.generateCallPathTestForResult(r, genReqId); }
            genMsg = await this.awaitPostedResult(genReqId, 300000);
            ...
            entry.steps.push(`Generated: ${genMsg.path}`);
          }

          // Step 3 — run it, same per-language dispatch a manual Run click
          // uses, unless the cache already confirms this exact (target,
          // test) pair passed with coverage confirmed and neither has
          // changed since — then skip running it again entirely.
          const testFileLower = String(genMsg.path).toLowerCase();
          const isDirect = DIRECT_TEST_EXTENSIONS.some((ext) => testFileLower.endsWith(ext));
          const runOnce = async (): Promise<any> => {
            ...
            if (isDirect) {
              progress(`Running ${genMsg.path}…`, runReqId, false);
              void this.runTestFileForResult(runMessage);
            } else {
              claudeCallsUsed++;
              progress(`Running ${genMsg.path} via Claude…`, runReqId, false);
              void this.runTestViaClaudeForResult(runMessage, folder);
            }
            return this.awaitPostedResult(runReqId, 180000);
          };
          let runMsg: any;
          const cachedRun = validCachedRunForKind(folder, cache, file, symbol, cacheKind);
          if (cachedRun && cachedRun.passed && cachedRun.testFile === genMsg.path) {
            runMsg = { ok: true, passed: true, coverageConfirmed: cachedRun.coverageConfirmed, executed: cachedRun.executed };
            entry.steps.push(`Cached: already ran and passed with coverage confirmed on ${new Date(cachedRun.ranAt).toLocaleString()} — skipped re-running.`);
          } else {
          ...
          if (runMsg?.ok === true && runMsg?.passed === false && !runMsg?.environmentIssue) {
            const fixReqId = 'automode_fix_' + (this.autoModeRequestSeq++) + '_' + Date.now();
            claudeCallsUsed++;
            progress(`Test failed — asking Claude to fix ${symbol}…`, fixReqId, false);
      ...
      this.post({
        ...
        text: `Auto Mode finished — ${summary.length} test(s) processed across the queue, ${passedCount} covered/passing, ${claudeCallsUsed} Claude call(s) used.`,
        summary,
        claudeCallsUsed,
      });
    } catch (err: any) {
      this.post({ type: 'autoModeProgress', done: true, error: err?.message || String(err), claudeCallsUsed });
    } finally {
   ...
   * looks like.
   */
  private async runCommitCheck(hash: string, requestId?: string): Promise<void> {
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder || !hash) {
      return;
    }
    ...
    try {
      ...
      const summaryTiles = results.length ? buildChangesSummaryTiles(report, results, callChains) : [];
  ...
  private async runLatestCommitsCheck(): Promise<void> {
    const folder = vscode.workspace.workspaceFolders?.[0];
    if (!folder) {
      return;
    }
...
export function getHtml(host: string, port: number, cspSource: string): string {
...
<style>
  ...
  .defectRowIcon.is-fixed { color: var(--vscode-terminal-ansiGreen, #3fb950); }
  .defectRowFixedNote { opacity: 0.7; font-size: 11px; margin-top: 4px; }
  ...
  .defectRowKind { flex: 0 0 auto; font-size: 10px; text-transform: uppercase; letter-spacing: 0.03em; opacity: 0.75; border: 1px solid var(--vscode-panel-border); border-radius: 3px; padding: 0 4px; }
  ...
  .verdictCard { margin-top: 6px; font-size: 11px; padding: 3px 0; }
  .verdictCard:empty { display: none; }
  .verdictLabel { font-weight: 700; }
  .verdictDetail { opacity: 0.8; }
  .verdict-high .verdictLabel { color: var(--vscode-terminal-ansiGreen, #2ea043); }
  .verdict-medium .verdictLabel { color: var(--vscode-editorWarning-foreground); }
  .verdict-low .verdictLabel { color: var(--vscode-descriptionForeground); }
  ...
  .genIcon.is-pending { background: color-mix(in srgb, var(--vscode-charts-yellow, var(--vscode-editorWarning-foreground)) 22%, transparent); font-size: 9px; }
  ...
  .callChainsSection .changeSectionTitle { font-size: 12px; }
  .callChainTable { display: flex; flex-direction: column; gap: 6px; margin-top: 4px; }
  .callChainTableRow { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 8px; padding: 6px 8px; border: 1px solid var(--vscode-panel-border); border-radius: 4px; background: var(--vscode-editor-background); }
  .callChainInfo { display: flex; flex-direction: column; gap: 3px; min-width: 0; flex: 1 1 260px; }
  .callChainLabelRow { display: flex; flex-wrap: wrap; align-items: center; gap: 6px; }
  .callChainLabel { font-family: var(--vscode-editor-font-family, monospace); font-size: 11px; font-weight: 600; opacity: 0.95; }
  .callChainLabelClickable { cursor: pointer; }
  .callChainLabelClickable:hover { text-decoration: underline; color: var(--vscode-textLink-foreground); }
  .callChainDescription { font-size: 11px; line-height: 1.4; opacity: 0.78; }
  .callChainBoundaryBadge { font-size: 10px; opacity: 0.8; border: 1px solid var(--vscode-panel-border); border-radius: 3px; padding: 0 4px; flex: 0 0 auto; }
  .callChainActions { display: flex; align-items: center; gap: 6px; flex: 0 0 auto; }
  .callChainExistsBadge { font-size: 10px; color: var(--vscode-terminal-ansiGreen, #3fb950); white-space: nowrap; }
  ...
  .metricGrid { display: flex; gap: 4px; }
  .metric { flex: 1 1 0; min-width: 0; border: 1px solid var(--vscode-panel-border); padding: 3px 4px; border-radius: 3px; text-align: center; }
  .metricValue { display: block; font-size: 12px; font-weight: 700; }
  .metricLabel { display: block; font-size: 10px; opacity: 0.72; overflow-wrap: anywhere; }
  .summaryTileGrid { display: grid; grid-template-columns: repeat(auto-fit, minmax(78px, 1fr)); gap: 4px; margin-bottom: 8px; }
  .summaryTile { display: flex; flex-direction: column; gap: 1px; border: 1px solid var(--vscode-panel-border); border-radius: 5px; padding: 4px 7px; background: var(--vscode-editorWidget-background, var(--vscode-sideBar-background)); }
  .summaryTileValue { font-size: 13px; font-weight: 700; line-height: 1.15; font-variant-numeric: tabular-nums; }
  .summaryTileLabel { font-size: 8px; opacity: 0.72; overflow-wrap: anywhere; }
  .summaryTile-high { border-color: color-mix(in srgb, var(--vscode-errorForeground) 45%, var(--vscode-panel-border)); }
  .summaryTile-high .summaryTileValue { color: var(--vscode-errorForeground); }
  .summaryTile-primary { border-color: color-mix(in srgb, var(--vscode-focusBorder, #4daafc) 45%, var(--vscode-panel-border)); }
  ...
  #modeRow { display: flex; align-items: center; gap: 6px; margin-top: 4px; }
  ...
  /* Modes popup — styled after Claude Code's own Manual / Auto mode picker:
     a small trigger pill that opens a floating card of icon+title+description
     rows, the current choice tinted with a checkmark, instead of the old pair
     of separate pill/native-QuickPick controls. */
  .modesMenu { position: relative; display: inline-block; }
  .modesTrigger { font: inherit; display: inline-flex; align-items: center; gap: 5px; border: 1px solid var(--vscode-panel-border); border-radius: 3px; background: transparent; color: var(--vscode-foreground); padding: 2px 8px; font-size: 11px; cursor: pointer; opacity: 0.85; }
  .modesTrigger:hover { opacity: 1; background: var(--vscode-list-hoverBackground); }
  .modesTrigger.is-agent { opacity: 1; border-color: var(--vscode-button-background); font-weight: 600; }
  .modesTrigger.is-prompt-only { opacity: 1; border-color: var(--vscode-charts-yellow, var(--vscode-button-background)); font-weight: 600; }
  .modesChevron { opacity: 0.6; font-size: 9px; }
  .modesPopup { display: none; position: absolute; left: 0; bottom: calc(100% + 4px); z-index: 30; width: 270px; padding: 4px; border-radius: 8px; border: 1px solid var(--vscode-dropdown-border, var(--vscode-panel-border)); background: var(--vscode-dropdown-background, var(--vscode-editorWidget-background)); box-shadow: 0 8px 24px rgba(0,0,0,0.35); }
  .modesPopup.is-open { display: block; }
  .modesOption { display: flex; align-items: flex-start; gap: 8px; width: 100%; text-align: left; border: 0; border-radius: 6px; padding: 6px 8px; margin: 1px 0; background: transparent; color: var(--vscode-dropdown-foreground, var(--vscode-foreground)); font: inherit; cursor: pointer; }
  .modesOption:hover { background: var(--vscode-list-hoverBackground); }
  .modesOption.is-current { background: var(--vscode-list-inactiveSelectionBackground); }
  .modesOptionIcon { flex: 0 0 auto; width: 16px; text-align: center; line-height: 1.4; }
  .modesOptionBody { flex: 1 1 auto; min-width: 0; display: flex; flex-direction: column; gap: 1px; }
  .modesOptionTitle { font-size: 12px; font-weight: 600; }
  .modesOptionDesc { font-size: 10.5px; opacity: 0.75; line-height: 1.35; }
  .modesOptionCheck { flex: 0 0 auto; opacity: 0; font-size: 12px; padding-top: 2px; }
  .modesOption.is-current .modesOptionCheck { opacity: 1; }
  .modesOption:disabled { cursor: default; opacity: 0.6; }
  .modesOption.is-running { opacity: 1; background: color-mix(in srgb, #d29922 16%, transparent); }
  .modesOption.is-running .modesOptionTitle::after { content: ' — running…'; font-weight: 400; opacity: 0.85; }
  ...
  #autoPipelineProgressHead { display: flex; align-items: baseline; gap: 6px; }
  #autoPipelineProgressText { font-weight: 600; }
  /* Bounded + collapsible so a long run (dozens of changed symbols) can't
     push the rest of the panel out of view — matches the +/- pattern used
     by mcpUsageCard/featureListCard/etc., plus a scroll cap that keeps
     space bounded even while expanded. */
  #autoPipelineProgressDetails { margin-top: 4px; max-height: 220px; overflow-y: auto; }
  #autoPipelineProgress.is-collapsed #autoPipelineProgressDetails { display: none; }
  /* The currently-running step — elapsed seconds always, live Claude
     transcript (same .testRunOutput box a manual chip click gets) only
     when this step actually streams output. */
  #autoPipelineActiveStep { margin-top: 4px; padding-top: 4px; border-top: 1px solid var(--vscode-panel-border); opacity: 0.9; }
  #autoPipelineActiveText { font-style: italic; }
  #autoPipelineActiveStep .testRunOutput { margin-top: 4px; max-height: 160px; }
...
<body>
  ...
  <div id="chatPane">
    ...
    <div id="modeRow">
      <div class="modesMenu">
        <button id="modesBtn" type="button" class="modesTrigger" aria-haspopup="true" aria-expanded="false" title="Choose whether Claude can run, or start the Auto Mode pipeline.">
          <span id="modesBtnIcon">✋</span><span id="modesBtnLabel">Manual</span><span class="modesChevron">▾</span>
        </button>
        <div id="modesPopup" class="modesPopup" role="menu">
          <button type="button" id="modeManualOption" class="modesOption" data-mode="manual" role="menuitemradio">
            <span class="modesOptionIcon">✋</span>
            <span class="modesOptionBody">
              <span class="modesOptionTitle">Manual</span>
              <span class="modesOptionDesc">Nothing runs without a click. Claude is never invoked.</span>
            </span>
            <span class="modesOptionCheck">✓</span>
          </button>
          <!-- Prompt-Only mode hidden for now (row + underlying claudeAgentMode/setClaudeAgentMode plumbing left intact) — remove style="display:none;" to re-enable. -->
          <button type="button" id="modePromptOnlyOption" class="modesOption" data-mode="promptOnly" role="menuitemradio" style="display:none;" title="Requires the claude CLI running in a terminal (not just Claude Code's own VS Code panel) — CODEMD types into that terminal's input line.">
            <span class="modesOptionIcon">📋</span>
            <span class="modesOptionBody">
              <span class="modesOptionTitle">Use Coding Agent (Prompt-Only)</span>
              <span class="modesOptionDesc">CODEMD types the prompt into your own Claude Code terminal — you read it, edit it if you want, and press Enter yourself. CODEMD never runs Claude or spends any of your usage.</span>
            </span>
            <span class="modesOptionCheck">✓</span>
          </button>
    ...
    <div id="autoPipelineProgress" style="display:none;">
      <div id="autoPipelineProgressHead">
        <button id="autoPipelineProgressToggle" type="button" class="detailsToggle" aria-expanded="true" title="Hide individual results" style="display:none;">-</button>
        <span id="autoPipelineProgressText"></span>
      </div>
      <div id="autoPipelineActiveStep" style="display:none;">
        <span id="autoPipelineActiveText"></span>
      </div>
      <div id="autoPipelineProgressDetails"></div>
    </div>
  </div>
<script nonce="${nonce}">
  ...
  let defectCounts = { active: 0, fixed: 0, fixedCode: 0, fixedTest: 0 };
  ...
  // Disabled immediately (not just once the host acknowledges) so mashing
  // the button while a run is already in flight can't fire a second
  // 'generate' message — re-enabled on the 'generated'/error status that
  // marks the host's runGenerate as finished.
  function setGenerateButtonsBusy(busy) {
    generateBtn.disabled = busy;
    emptyStateRetryBtn.disabled = busy;
  }
  ...
  function renderChangeCard(item, card, r) {
    ...
    if (card.change) {
      // The single sentence saying what actually changed (e.g. "Signature
      // changed: added optional parameter: x") is the one fact this whole
      // card exists to convey — it used to go through appendCollapsibleSection
      // like every other bucket (Evidence, Files, ...), which put it behind a
      // "Change v" toggle and the same 0.88/0.85 dimming as low-priority
      // detail, so it read as no more important than anything else on the
      // card. Full-opacity and un-collapsible here since it's always one
      // short line (see buildModifiedChangeCard/buildAddedChangeCard) with
      // nothing to hide.
      const change = document.createElement('div');
      change.className = 'changeSummary';
  ...
  // Both "Generate regression test" and "Generate Test with Claude"
  ...
  // Same visible "…Ns" counter as startPendingTicker, but for buttons that
  // invoke Claude: CODEMD's own pre-Claude work (resolving real callers,
  // reading call-site snippets, queueing behind another in-flight Claude
  // call) is free and can itself take several seconds on a high-fan-in
  // function — starting the count at click time made that free CODEMD time
  // look like it was being spent "on Claude". The count now only starts once
  // Claude's own first live-output chunk actually arrives (see
  // claudeTickersByRequestId + the testGenProgress/fixTestProgress handlers
  // below), so the number shown is genuinely Claude's own wall-clock time.
  // The chip still updates immediately on click (a static "label…", no
  // ticking number yet) so a click never looks like nothing happened.
  function startClaudeTicker(chip, baseLabel) {
  ...
  // requestId -> the startClaudeTicker controller for that request — a
  // second index alongside each button's own *ChipsByRequestId map, looked
  // up by the shared testGenProgress/fixTestProgress handlers (which don't
  // otherwise know which of the several chip maps a given requestId belongs
  // to) so they can call beginIfNotStarted() the moment Claude's first chunk
  // of live output actually arrives.
  const claudeTickersByRequestId = new Map();

  // A test-gen chip could sit in "Generating… Ns" for minutes with no way
  // out (e.g. a Multi-Caller Test reading several real callers, or a
  // Call-Path Test on a high-fan-in function) — Fix-with-Claude already has
  // a Stop button for exactly this reason (see fixStopBtn); this gives every
  // Claude write-only test generator the same escape hatch, wired to the
  // host's activeTestGenCancelers via the cancelTestGeneration message.
  function createTestGenStopButton(requestId) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.className = 'btn is-outline testGenStopBtn';
    btn.textContent = 'Stop';
    btn.title = 'Stop this Claude test-generation run.';
    btn.addEventListener('click', (event) => {
      event.stopPropagation();
      btn.disabled = true;
      btn.textContent = 'Stopping…';
      vscode.postMessage({ type: 'cancelTestGeneration', requestId });
    });
    return btn;
  }

  function removeTestGenStopButton(entry) {
    if (entry && entry.stopBtn && entry.stopBtn.isConnected) { entry.stopBtn.remove(); }
  }

  // requestId -> the live <pre> a testGenProgress chunk should be appended
  // to. Shared by every "Claude write-only test generator" (call-path,
  // new-function, call-chain, contract, broad-coverage) so they don't each
  // reimplement the same live-transcript wiring the Fix-with-Claude flow
  // already has (fixOutputPre/fixTestProgress) — a user asked why those
  // generators only showed a static "Generating… Ns" with nothing else,
  // wanting to see Claude's actual output scroll by the same way it does
  // there, starting from the moment the button is clicked (not just once a
  // result comes back).
  ...
  // One row per entry in the Bugs, Issues & Defects list — either still
  // 'active' (needs a click) or 'fixed' (a bug CODEMD's generated tests
  // actually caught, code or test kind alike — see postDefects). Active rows
  // reuse the exact same fixTestFailure/
  // fixTestResult round trip a live test-run row's "Fix with Claude" button
  // uses (see createTestRunRow), so the outcome banner, live transcript, and
  // defect-status auto-resolution (server-side, see fixTestFailureForResult)
  // all behave identically here. No statusPill/statusDetail/runBtn for this
  ...
  function createDefectRow(item) {
    ...
    const row = document.createElement('div');
    row.className = 'defectRow' + (isFixed ? ' is-fixed' : '');

    const head = document.createElement('div');
    head.className = 'defectRowHead';

    const icon = document.createElement('span');
    icon.className = 'defectRowIcon' + (isFixed ? ' is-fixed' : '');
    icon.textContent = isFixed ? '✓' : '⚠';
    ...
    fixBtn.addEventListener('click', (event) => {
      ...
      const tickInterval = startClaudeTicker(fixBtn, 'Fixing');
      fixNote.textContent = '';
      fixNote.className = '';
      fixNote.style.display = 'none';
      ...
      claudeTickersByRequestId.set(fixRequestId, tickInterval);
  ...
  // Host-computed, disk-backed companion to generatedTestsBySymbol below —
  // survives a VS Code restart/webview reload (generatedTestsBySymbol does
  // not, since it's only ever populated by messages received this session).
  // Checked alongside generatedTestsBySymbol wherever a generator chip
  // decides whether to render, so a chip whose test already exists on disk
  // stays hidden even on a fresh session instead of re-inviting a duplicate
  // paid generation. kind is 'new_function'|'contract'|'broad_coverage'|
  // 'call_path'|'call_chain', matching the host's generatorKindFromTestFileName.
  function hasDiskGeneratedKind(r, kind) {
    return !!(r && r.testStatus && Array.isArray(r.testStatus.generatedKinds) && r.testStatus.generatedKinds.indexOf(kind) !== -1);
  }
  const generatedTestsBySymbol = new Map();
  // Last "Run" outcome per changed symbol (passed/coverageConfirmed/etc.) —
  // feeds the verdict card below alongside generatedTestsBySymbol. Same
  // survives-a-rebuild rationale as generatedTestsBySymbol.
  const testRunsBySymbol = new Map();
  // Live verdict-card DOM elements currently on screen, keyed by symbol —
  // NOT meant to survive a rebuild (appendResultItems clears the parent and
  // repopulates this on every render); lets an async result that arrives
  // after the card is on screen find it again and update in place. A symbol
  // not currently rendered is simply a no-op update — its data is still
  // safe in generatedTestsBySymbol/testRunsBySymbol for next time it renders.
  const verdictCardElsBySymbol = new Map();
  // File each symbol's verdict card belongs to, keyed the same way as
  // verdictCardElsBySymbol. computeVerdictForSymbol only ever gets a bare
  // symbol key (updateVerdictCard's callers don't carry the change's r.file
  // around), so this is populated once at card-build time and looked up by
  // symbol rather than threaded through every call site.
  const verdictFileBySymbol = new Map();
  // Existing-tests-found-automatically container per symbol (parallel to
  // verdictCardElsBySymbol) plus the request bookkeeping needed since this
  // check has no button to click — it's auto-triggered once per symbol per
  // session (existingTestsRequestedSymbols guards against re-firing on every
  // periodic auto-refresh re-render).
  const existingTestsCardElsBySymbol = new Map();
  const existingTestsRequestedSymbols = new Set();
  ...
  let autoMode = 'manual';
  // 'manual' | 'promptOnly' | 'agent' — mirrors codemdGraphs.claudeAgentMode,
  // synced from the host via 'modesConfig' the same way autoMode is, and
  // also updated optimistically on click (the Modes popup's three rows post
  // 'setClaudeAgentMode' directly). 'promptOnly': CODEMD types the prompt
  // into the user's own Claude terminal instead of running Claude itself —
  // see sendPromptToUsersClaudeTerminal on the host side.
  let claudeAgentMode = 'manual';
  ...
  // Commits / Analyze Commit) — reset in appendResultItems so one large
  // batch can't silently fire a wall of Claude calls; capped by
  // maxAutoWritesPerBatch.
  let autoWriteBatchCount = 0;
  // How many "Check tests" auto-fires (Auto-Run Tests pill, local pytest —
  // see fireCheckTests) have fired in the CURRENT top-level result
  // render. Unlike Write:Auto's Claude calls, this was left uncapped on the
  // assumption that a local-only check is cheap — but a large diff or a
  // first-time scan can render hundreds of changed/added Python symbols in
  // one batch, each firing automatically, each running up to 3 matched test
  // files in parallel, which floods the OS with coverage-run/pytest
  // processes. Capped by the same maxAutoWritesPerBatch setting; reset
  // alongside autoWriteBatchCount in appendResultItems. Symbols beyond the
  // cap fall back to the manual "Check tests" chip instead of being
  // silently skipped.
  let autoRunBatchCount = 0;
  // Write:Auto needs each symbol's existing-tests verdict before deciding
  // whether it's even worth generating (skip if an existing test already
  // covers it with HIGH confidence) — but that check resolves
  // asynchronously and may not be in yet when a chip is first created, so
  // queue the fire function here and run it once existingTestsResult
  // arrives for that symbol. A symbol can queue more than one entry (e.g.
  // both a contract and a broad-coverage chip on the same card).
  const pendingAutoWrites = new Map();

  // Modes popup — Manual / Prompt-Only / Use Coding Agent / Auto Mode in one
  // place, styled after Claude Code's own mode picker (icon + title +
  // description rows, current choice checked). The first three are the
  // persistent states of codemdGraphs.claudeAgentMode; Auto Mode is the
  // one-click pipeline trigger below (autoPipelineBtn), just presented here
  // as a fourth row instead of its own separate button.
  const modesBtn = document.getElementById('modesBtn');
  const modesBtnIcon = document.getElementById('modesBtnIcon');
  const modesBtnLabel = document.getElementById('modesBtnLabel');
  const modesPopup = document.getElementById('modesPopup');
  const modeManualOption = document.getElementById('modeManualOption');
  ...
  function renderModePills() {
    modesBtnIcon.textContent = MODE_ICONS[claudeAgentMode] || MODE_ICONS.manual;
    ...
    modeManualOption.classList.toggle('is-current', claudeAgentMode === 'manual');
    modePromptOnlyOption.classList.toggle('is-current', claudeAgentMode === 'promptOnly');
    modeAgentOption.classList.toggle('is-current', claudeAgentMode === 'agent');
  }
  modesBtn.addEventListener('click', (event) => {
    event.stopPropagation();
    const opening = !modesPopup.classList.contains('is-open');
    modesPopup.classList.toggle('is-open', opening);
    modesBtn.setAttribute('aria-expanded', opening ? 'true' : 'false');
  });
  document.addEventListener('click', (event) => {
    if (modesPopup.classList.contains('is-open') && !event.target.closest('.modesMenu')) {
      closeModesPopup();
    }
  });
  function selectClaudeAgentMode(mode) {
    claudeAgentMode = mode;
    renderModePills();
    closeModesPopup();
    vscode.postMessage({ type: 'setClaudeAgentMode', value: mode });
  }
  modeManualOption.addEventListener('click', () => selectClaudeAgentMode('manual'));
  modePromptOnlyOption.addEventListener('click', () => selectClaudeAgentMode('promptOnly'));
  modeAgentOption.addEventListener('click', () => selectClaudeAgentMode('agent'));
  renderModePills();

  // ⚡ Auto Mode — one click, the whole comprehensive pipeline (check
  // existing tests -> generate every applicable test -> run -> fix a
  // genuine failure -> re-run, for every generator on every medium/high
  // change) runs serially, driven entirely on the extension-host side (see
  // runAutoMode); this side just renders the autoModeProgress messages it
  // posts along the way.
  ...
  const autoPipelineProgressText = document.getElementById('autoPipelineProgressText');
  const autoPipelineProgressDetails = document.getElementById('autoPipelineProgressDetails');
  const autoPipelineProgressToggle = document.getElementById('autoPipelineProgressToggle');
  const autoPipelineActiveStep = document.getElementById('autoPipelineActiveStep');
  const autoPipelineActiveText = document.getElementById('autoPipelineActiveText');
  // Tracks whichever host-side step (generate/run/fix/existing-check) is
  // currently in flight, keyed by its requestId — same requestIds
  // runAutoMode already uses for awaitPostedResult, just also surfaced here
  // so this panel can show a live elapsed-seconds ticker for every step and,
  // for the ones that stream (generation calls), the same live Claude
  // transcript box a manual chip click gets (createLiveClaudeOutputEl,
  // fed by the existing generic testGenProgress channel — no new host-side
  // streaming plumbing needed, just registering a box for this requestId).
  let autoPipelineActiveRequestId = null;
  let autoPipelineActiveTicker = null;
  let autoPipelineActiveStartedAt = 0;
  let autoPipelineActiveBaseText = '';
  let autoPipelineActivePre = null;
  function tickAutoPipelineActiveStep() {
    const secs = Math.max(0, Math.floor((Date.now() - autoPipelineActiveStartedAt) / 1000));
    autoPipelineActiveText.textContent = autoPipelineActiveBaseText + ' (' + secs + 's)';
  }
  function stopAutoPipelineActiveStep() {
    if (autoPipelineActiveTicker) { clearInterval(autoPipelineActiveTicker); autoPipelineActiveTicker = null; }
    if (autoPipelineActiveRequestId) { testGenOutputPreByRequestId.delete(autoPipelineActiveRequestId); }
    autoPipelineActiveRequestId = null;
    autoPipelineActivePre = null;
    autoPipelineActiveStep.style.display = 'none';
    autoPipelineActiveText.textContent = '';
  }
  function updateAutoPipelineActiveStep(msg) {
    const newReqId = msg.activeRequestId || null;
    if (newReqId !== autoPipelineActiveRequestId) {
      if (autoPipelineActiveTicker) { clearInterval(autoPipelineActiveTicker); autoPipelineActiveTicker = null; }
      if (autoPipelineActiveRequestId) { testGenOutputPreByRequestId.delete(autoPipelineActiveRequestId); }
      if (autoPipelineActivePre) { autoPipelineActivePre.remove(); autoPipelineActivePre = null; }
      autoPipelineActiveRequestId = newReqId;
      if (newReqId) {
        autoPipelineActiveStartedAt = Date.now();
        autoPipelineActiveStep.style.display = '';
        autoPipelineActiveTicker = setInterval(tickAutoPipelineActiveStep, 1000);
        if (msg.activeHasOutput) {
          autoPipelineActivePre = createLiveClaudeOutputEl(newReqId);
          autoPipelineActiveStep.appendChild(autoPipelineActivePre);
        }
      } else {
        autoPipelineActiveStep.style.display = 'none';
      }
    ...
    if (newReqId) {
      autoPipelineActiveBaseText = msg.text || '';
      tickAutoPipelineActiveStep();
    }
  }
  let autoPipelineRunning = false;
  autoPipelineProgressToggle.addEventListener('click', () => {
    const collapsed = autoPipelineProgress.classList.toggle('is-collapsed');
    autoPipelineProgressToggle.textContent = collapsed ? '+' : '-';
  ...
  function startAutoPipeline() {
    ...
    autoPipelineProgressToggle.style.display = 'none';
    autoPipelineProgressDetails.textContent = '';
  ...
  function renderAutoPipelineSummary(summary) {
    (summary || []).forEach((entry) => {
      ...
      name.textContent = entry.symbol + (entry.testKind ? ' — ' + entry.testKind : '');
      ...
      autoPipelineProgressDetails.appendChild(row);
    });
  }
  ...
  const DIRECT_RUN_EXTS = ['.py', '.go', '.rs', '.java', '.cs', '.kt', '.kts'];
  ...
  function createTestRunRow(file, nodeId, label, openLine, targetInfo, onResult) {
    ...
    // Only shown when a run fails specifically because this JS/TS repo has
    // no test runner at all (msg.noTestRunner — see testRunResult below).
    // Deterministic, no Claude call: picks one lightweight framework from a
    // fixed rule and installs it via "npm install --save-dev" (JS/TS) or a
    // pom.xml/build.gradle dependency edit (Java), visibly, so it's a real,
    // permanent addition rather than the kind of silent improvised install
    // "Fix with Claude" is explicitly barred from doing.
    const setupBtn = document.createElement('button');
    setupBtn.type = 'button';
    setupBtn.className = 'btn is-primary';
    setupBtn.textContent = '⚙ Set up test runner (free)';
    setupBtn.title = 'This repo has no test runner for this language yet. Installs one lightweight, deterministically-chosen framework and minimal config — visibly, as a real project dependency, never guessed at by Claude. Only happens when you click this.';
    setupBtn.style.display = 'none';

    // Only ever shown after a real pass (see the testRunResult/fixTestResult
    // handlers below, which flip this on). Clicking it just hides this one
    // row — no memory of it anywhere, nothing about the test itself changes.
    // A fresh render only ever happens when the user clicks "Check
    // Uncommitted Edits" themselves (there's no silent background refresh —
    // see onWorkspaceFileChanged above), so this naturally reappears the
    // next time that runs and rebuilds the row from scratch, or whenever the
    // underlying function changes enough to reshape this card. Nothing to
    // reset on purpose; the DOM's own lifecycle already does it.
    const resolveBtn = document.createElement('button');
    resolveBtn.type = 'button';
    resolveBtn.className = 'btn is-outline';
    resolveBtn.textContent = '✓ Resolve';
    resolveBtn.title = 'This test passed — hide it for now. It still exists and still runs as part of "Run all"; this only hides the row until the next "Check Uncommitted Edits".';
    ...
    setupBtn.addEventListener('click', (event) => {
      event.stopPropagation();
      if (setupBtn.dataset.pending === '1') { return; }
      setupBtn.dataset.pending = '1';
      const tickInterval = startPendingTicker(setupBtn, 'Setting up');
      const setupRequestId = 'testsetup_' + (testRunRequestSeq++) + '_' + Date.now();
      jsTestRunnerSetupByRequestId.set(setupRequestId, { setupBtn, runBtn, statusDetail, tickInterval, symbolKey: (targetInfo && targetInfo.symbol) || '', row });
      vscode.postMessage({ type: 'setUpTestRunner', requestId: setupRequestId, file: (targetInfo && targetInfo.path) || file });
    });
    ...
    fixBtn.addEventListener('click', (event) => {
      ...
      fixBtn.dataset.pending = '1';
      const tickInterval = startClaudeTicker(fixBtn, isEnvIssue ? 'Fixing environment' : 'Fixing');
      fixNote.textContent = '';
      fixNote.className = '';
      fixNote.style.display = 'none';
  ...
  function buildGeneratedTestNote(opts) {
    ...
    if (opts.source === 'claude') {
      if (opts.sentToTerminal) {
        const head = document.createElement('div');
        head.className = 'testCardTitle';
        const icon = document.createElement('span');
        icon.className = 'genIcon is-pending';
        icon.textContent = '📋';
        head.appendChild(icon);
        head.appendChild(document.createTextNode(
          opts.ok
            ? 'Prompt sent to your Claude Code terminal — review it there, then press Enter to run it.'
            + (opts.path ? ' It asks Claude to save the test to ' + opts.path + '.' : '')
            : (opts.error || 'Could not send the prompt to your Claude Code terminal.'),
        ));
        note.appendChild(head);
        if (opts.ok) {
          const hint = document.createElement('div');
          hint.className = 'plainSummary';
          hint.textContent = 'CODEMD is not running Claude and has no way to know what happens next — after Claude finishes there, click Regenerate/Check to see the new test.';
      ...
      if (statsParts.length) {
        ...
        if (typeof opts.generationCostUsd === 'number') {
          meta.title = 'You’re not actually being charged ~$' + opts.generationCostUsd.toFixed(2) + ' for this — it’s just an estimate of what this one test would cost under Claude API / Console pay-as-you-go pricing. Nothing is billed per call on a Claude Pro/Max subscription. Full log: .codemd/reports/';
        }
        head.appendChild(meta);
      }
      note.appendChild(head);
  ...
  // Rolls up every signal already collected for one changed symbol —
  // which of the four Claude generators produced a test (generatedTestsBySymbol,
  // keyed by the bare symbol plus one of '#newfunc'/'#contract'/'#broadcoverage';
  // the bare key covers both the call-chain generator and the mechanical
  // regression-test path) and the last "Run" outcome (testRunsBySymbol) —
  // into one LOW/MEDIUM/HIGH confidence label. No Claude call and no new
  // data: everything here was already being computed, just scattered.
  function computeVerdictForSymbol(symbolKey) {
    const file = verdictFileBySymbol.get(symbolKey) || '';
    const fileSuffix = file ? ' (' + file + ')' : '';
    const kinds = [];
    if (generatedTestsBySymbol.has(symbolKey)) { kinds.push('call-path'); }
    if (generatedTestsBySymbol.has(symbolKey + '#newfunc')) { kinds.push('new-function'); }
    if (generatedTestsBySymbol.has(symbolKey + '#contract')) { kinds.push('contract'); }
    if (generatedTestsBySymbol.has(symbolKey + '#broadcoverage')) { kinds.push('broad-coverage'); }
    const existing = generatedTestsBySymbol.get(symbolKey + '#existing');
    if (existing && Array.isArray(existing.matches) && existing.matches.length) { kinds.push('existing'); }
    const run = testRunsBySymbol.get(symbolKey) || null;
    let confidence = 'LOW';
    let detail = 'No test generated or found yet for this change.';
    if (kinds.length && !run) {
      confidence = 'MEDIUM';
      detail = kinds.length + ' test(s) available (' + kinds.join(', ') + ') — not run yet.';
    } else if (kinds.length && run) {
      if (run.passed && run.coverageConfirmed && run.executed) {
        confidence = 'HIGH';
        detail = kinds.length + ' test(s) (' + kinds.join(', ') + ') · Executed ✓ · Coverage confirmed.';
      } else if (run.passed) {
        confidence = 'MEDIUM';
        detail = kinds.length + ' test(s) (' + kinds.join(', ') + ') · Passed, but execution of the change was not confirmed.';
      } else {
        confidence = 'MEDIUM';
        detail = kinds.length + ' test(s) (' + kinds.join(', ') + ') · Last run failed.';
      }
    }
    return { kinds, run, confidence, detail: detail + fileSuffix };
  }

  function renderVerdictCardContent(el, symbolKey) {
    const v = computeVerdictForSymbol(symbolKey);
    el.className = 'verdictCard verdict-' + v.confidence.toLowerCase();
    el.textContent = '';
    const label = document.createElement('span');
    label.className = 'verdictLabel';
    label.textContent = 'Verification: ' + v.confidence;
    const detail = document.createElement('span');
    detail.className = 'verdictDetail';
    detail.textContent = ' — ' + v.detail;
    el.appendChild(label);
    el.appendChild(detail);
  }

  // Called after any signal that could change a symbol's verdict (a test
  // generated, a test run, an existing-tests check) finishes. A no-op if
  // that symbol's card isn't currently on screen (e.g. a different panel is
  // showing) — its data is still saved for whenever it renders again.
  function updateVerdictCard(symbolKey) {
    if (!symbolKey) { return; }
    const el = verdictCardElsBySymbol.get(symbolKey);
    if (el && el.isConnected) { renderVerdictCardContent(el, symbolKey); }
  }

  // Actually fires (or skips) a queued auto-write once we know the verdict
  // for its symbol. Re-checks the batch cap here rather than only at
  // queue time, since queueing happens synchronously while rendering a
  // whole batch but firing happens later, one existingTestsResult at a
  // time, in whatever order those round-trips land.
  function tryAutoWrite(symbolKey, fireFn, actionsEl) {
    if (autoWriteBatchCount >= maxAutoWritesPerBatch) { return; }
    const verdict = computeVerdictForSymbol(symbolKey);
    if (verdict.confidence === 'HIGH') {
      if (actionsEl) {
        const note = document.createElement('div');
        note.className = 'plainSummary';
        note.textContent = 'Write:Auto skipped this one — an existing test already covers it.';
  ...
  // Called right after each of the three unconditional Claude generator
  // chips (New-Function / Contract / Broad-Coverage) is appended. Only
  // acts when Write:Auto is on; otherwise the chip just sits there exactly
  // as it does in Manual mode, waiting for a click. fireFn is the same
  // function the chip's own click listener calls, so an auto-fire behaves
  // identically to a manual one.
  function maybeAutoWriteTest(symbolKey, savedKey, fireFn, actionsEl) {
    if (writeMode !== 'auto') { return; }
    if (generatedTestsBySymbol.has(savedKey)) { return; } // already generated/restored this session
    const existing = generatedTestsBySymbol.get(symbolKey + '#existing');
    if (existing) {
      tryAutoWrite(symbolKey, fireFn, actionsEl);
    } else {
      const queued = pendingAutoWrites.get(symbolKey) || [];
      queued.push(() => tryAutoWrite(symbolKey, fireFn, actionsEl));
      pendingAutoWrites.set(symbolKey, queued);
    }
  }

  // No Claude call (pure local search + pytest), so unlike the four
  // generator notes above this renders quietly — one pill row per matched
  // existing test file, same visual language as a "Run this test" row, but
  // never a loading spinner the way a paid Claude call gets one.
  function renderExistingTestRow(container, m) {
    const row = document.createElement('div');
    row.className = 'testRunRow';
    const nameEl = document.createElement('span');
    nameEl.className = 'testMatchName';
    nameEl.textContent = m.file;
    nameEl.title = 'Open ' + m.file;
    nameEl.addEventListener('click', (event) => {
      event.stopPropagation();
      vscode.postMessage({ type: 'openFile', file: m.file, line: '' });
    });
    row.appendChild(nameEl);
    const pill = document.createElement('span');
    const notRelevant = m.ok && m.coverageConfirmed && !m.executed;
    const failed = !m.ok || !m.passed;
    if (!m.ok) {
      pill.className = 'statusPill is-warn';
      pill.textContent = '⚠ ' + (m.error || 'Could not run');
    } else if (notRelevant) {
      // Proven (by coverage tracing, not a guess) that this test never ran
      // the changed lines — a pass here says nothing about the change, so
      // it must not read as coverage. The pass/fail fact is demoted to
      // parenthetical detail instead of driving the badge color/icon.
      pill.className = 'statusPill is-neutral';
      pill.textContent = '○ Not relevant to this change (' + (m.passed ? 'passed' : 'failed') + ')';
    } else {
      pill.className = 'statusPill ' + (m.passed ? 'is-good' : 'is-bad');
      pill.textContent = (m.passed ? '✓ Passed' : '✗ Failed')
    ...
    // Only worth surfacing on a failure; a passing row has nothing to add.
    if (failed && m.output) {
      const toggle = document.createElement('button');
      toggle.type = 'button';
      toggle.className = 'outputLink';
      ...
      toggle.addEventListener('click', (event) => {
        ...
        const showing = pre.style.display !== 'none';
        pre.style.display = showing ? 'none' : 'block';
        toggle.textContent = showing ? 'View output' : 'Hide output';
      });
  ...
  function renderExistingTestsCard(el, matches) {
    el.textContent = '';
    if (!matches || !matches.length) { return; }
    el.className = 'testCard';
    const title = document.createElement('div');
    title.className = 'testCardTitle';
    title.textContent = 'Existing test(s) found for this change';
    el.appendChild(title);
    // Relevance is a proven fact here (coverage-traced, not guessed), so a
    // test CODEMD already confirmed doesn't touch the changed code shouldn't
    // sit as a top-level row next to ones that do — it's collapsed evidence
    // ("we checked and ruled this out"), not coverage for the change.
    const relevant = matches.filter((m) => !(m.ok && m.coverageConfirmed && !m.executed));
    const notRelevant = matches.filter((m) => m.ok && m.coverageConfirmed && !m.executed);
    relevant.forEach((m) => renderExistingTestRow(el, m));
    if (notRelevant.length) {
      ...
      toggle.textContent = '+';
      toggle.setAttribute('aria-expanded', 'false');
      toggle.title = 'Show test(s) checked and confirmed NOT to exercise this change';
      const label = document.createElement('span');
      label.className = 'testMatchName';
      label.style.opacity = '0.7';
      label.textContent = notRelevant.length + ' other test(s) checked — not relevant to this change';
      const summaryRow = document.createElement('div');
      summaryRow.className = 'testRunRow';
      summaryRow.appendChild(toggle);
      summaryRow.appendChild(label);
      el.appendChild(summaryRow);
      const group = document.createElement('div');
      group.style.display = 'none';
      notRelevant.forEach((m) => renderExistingTestRow(group, m));
      el.appendChild(group);
      toggle.addEventListener('click', (event) => {
        event.stopPropagation();
        const showing = group.style.display !== 'none';
        group.style.display = showing ? 'none' : 'block';
  ...
  function resolveChipTestResult(chipsByRequestId, msg, resetText, successText, hideOnSuccess) {
    ...
    removeTestGenStopButton(entry);
    chip.dataset.pending = '0';
    if (msg.ok && msg.path && hideOnSuccess) {
      // A test now exists — leave the "Test written" note (below) as the
      // only trace instead of a button that still reads "Generate…" and
      // invites clicking again for another paid Claude call.
      chip.remove();
    } else {
      // A successful generation means a test now exists where the button's
      // original label ("Generate ...") implied none did — resetting straight
      // back to that same label (the old behavior) made it look like nothing
      // happened, even though the "Test written" note right below it says
      // otherwise. successText (e.g. "Regenerate") is only passed by callers
      // that have an equivalent post-success label; others keep resetText.
      chip.textContent = (msg.ok && msg.path && successText) ? successText : resetText;
    }
    const opts = {
      source: 'claude',
      ok: msg.ok,
      sentToTerminal: msg.sentToTerminal,
      error: msg.error,
      path: msg.path,
  ...
  // (Python only — see the sequential-reveal comment where "Generate
  // regression test" is built) or immediately for every other supported
  // language, which has no mechanical path at all. genActions may already
  // hold a leftover regressionTestNote from a failed mechanical attempt;
  // this is appended after it, not in place of it, so the "why this
  // appeared" message stays visible.
  //
  // What this button actually asks Claude to do differs by *kind*, not just
  // wording, so the tooltip has to branch on it rather than use one
  // generic blurb for every case:
  //  - a REMOVED function no longer exists to read or test — Claude is
  //    instead pointed at its former callers to confirm they still work
  //    correctly now that the call is gone.
  //  - everything else (an existing untested function, reached via "Check
  //    tests" — a new function has its own dedicated "Generate test for new
  //    function" chip elsewhere) is a direct test of the function itself,
  //    grounded in a real caller.
  function appendClaudeFallbackChip(genActions, gapResult, symbol, afterMechanicalFailure) {
    if (genActions.querySelector('.actionChipClaudeFallback')) { return; }
    const isRemoved = !!(gapResult && gapResult.changeCard && gapResult.changeCard.kind === 'removed');
    ...
    claudeChip.textContent = '🤖 Generate Test with Claude';
    if (isRemoved) {
      claudeChip.title = 'This function was removed, so there is nothing left to test directly. '
        + 'Instead, the local Claude Code CLI will read the code that used to call it and write a test '
        + 'confirming that code still works correctly now the call is gone — e.g. no leftover reference '
        + 'to the removed function, no broken fallback. Usually 30-90s, can take up to 3 minutes; the '
        + 'chip shows elapsed seconds while it runs, and it is always safe to click again if it times out.';
    } else if (afterMechanicalFailure) {
      ...
      claudeChip.title = 'This language has no free mechanical test generator, so this goes straight to '
        + 'the local Claude Code CLI: it reads the function and a real caller and writes a test by hand, '
        + 'grounded in how the function is actually called in this codebase. Usually 30-90s, can take up '
        + 'to 3 minutes for a large file with many callers; the chip shows elapsed seconds while it runs, '
        + 'and it is always safe to click again if it times out.';
    }
    claudeChip.addEventListener('click', (event) => {
      ...
      const tickInterval = startClaudeTicker(claudeChip, 'Asking Claude');
      ...
      const existingOutput = genActions.nextElementSibling && genActions.nextElementSibling.classList.contains('testRunOutput')
        ? genActions.nextElementSibling
        : null;
      if (existingOutput) { existingOutput.remove(); }
      attachLiveClaudeOutput(genActions, requestId);
      const stopBtn = createTestGenStopButton(requestId);
      genActions.appendChild(stopBtn);
      claudeTickersByRequestId.set(requestId, tickInterval);
  ...
  // Commit" button — both need the exact same requestId/tracking-map dance
  // so renderSearchResult can insert the result card directly below this
  // row (commitDetailRowsByRequestId) rather than at the bottom of the panel.
  function triggerAnalyzeCommit(item, hash) {
    const requestId = 'commitdetail_' + (commitDetailRequestSeq++) + '_' + Date.now();
    commitDetailRowsByRequestId.set(requestId, item);
    vscode.postMessage({ type: 'analyzeCommit', hash, requestId });
  }
  ...
  function appendResultItem(parent, r) {
    ...
    if (r.changeCard) {
      ...
      if (r.commitHash) {
        // The row itself is also clickable (see the commitHash handler
        // below) to run the same analysis, but that wasn't discoverable —
        // this makes it an explicit, visible action instead of a hidden
        // whole-row click.
        const analyzeBtn = document.createElement('button');
        analyzeBtn.type = 'button';
        analyzeBtn.className = 'actionChip actionChipClickable actionChipPrimary';
        analyzeBtn.style.font = 'inherit';
        analyzeBtn.textContent = 'Analyze Commit';
        analyzeBtn.title = 'Show this commit\\'s impact graph and diff.';
        analyzeBtn.addEventListener('click', (event) => {
          event.stopPropagation();
          triggerAnalyzeCommit(item, r.commitHash);
        });
    ...
    if (graphable) {
      ...
      // Existing-tests signal (Python/Go/C#/Java/Kotlin, run locally with
      // real coverage confirmation — feeds the Verification card below and
      // Write:Auto's skip-if-covered check) is set up here, before "Check
      // tests," so a single "Check tests" click can trigger it too. This
      // used to be its own separate "Check existing tests" button — same
      // find_tests discovery call under the hood as "Check tests," just
      // language-gated and actually test-executing — which meant two
      // confusingly similar buttons sat side by side on every card. Merging
      // the trigger (not the underlying code paths, which still do
      // genuinely different things — see renderExistingTestsCard vs the
      // per-test testGapResult match list) removes that duplication.
      let existingEl = null;
      let fireExistingTestsCheck = null;
      if (symbolic && r.changeCard && (r.changeCard.kind === 'modified' || r.changeCard.kind === 'added')) {
        existingEl = document.createElement('div');
        existingTestsCardElsBySymbol.set(graphable, existingEl);
        const savedExisting = generatedTestsBySymbol.get(graphable + '#existing');
        if (savedExisting) {
          renderExistingTestsCard(existingEl, savedExisting.matches);
        } else if (r.file && /\.(py|go|cs|java|kts?)$/i.test(String(r.file))) {
          fireExistingTestsCheck = () => {
            if (existingTestsRequestedSymbols.has(graphable)) { return; }
            existingTestsRequestedSymbols.add(graphable);
            const requestId = 'existingtests_' + (existingTestsRequestSeq++) + '_' + Date.now();
            existingTestsRequestsByRequestId.set(requestId, { symbol: graphable });
            vscode.postMessage({ type: 'runExistingTestsForResult', result: r, requestId });
          };
        }
      }
      ...
      if (symbolic) {
        ...
        testsChip.addEventListener('click', (event) => {
          event.stopPropagation();
          fireCheckTests();
          // Manual click only, not the auto-fire path below — the
          // Auto-Run Tests pill already drives an equivalent check when
          // it fires fireCheckTests() on its own, so triggering this here
          // too would duplicate that specific run a second time.
      ...
      if (symbolic && r.changeCard && r.changeCard.kind === 'added' && !generatedTestsBySymbol.has(graphable + '#newfunc') && !hasDiskGeneratedKind(r, 'new_function')) {
        const newFuncChip = document.createElement('span');
        newFuncChip.className = 'actionChip actionChipClickable actionChipPrimary';
        newFuncChip.textContent = '🆕 Generate test for new function with Claude';
        ...
        const fireNewFuncTest = () => {
          if (newFuncChip.dataset.pending === '1') { return; }
          newFuncChip.dataset.pending = '1';
          const tickInterval = startClaudeTicker(newFuncChip, 'Generating');
          ...
          claudeTickersByRequestId.set(requestId, tickInterval);
          newFunctionTestChipsByRequestId.set(requestId, { chip: newFuncChip, actionsEl: actions, testingArea, tickInterval, symbolKey: graphable + '#newfunc', stopBtn });
      ...
      if (symbolic && r.changeCard && r.changeCard.kind === 'modified' && Array.isArray(r.changeCard.callSiteIssues) && r.changeCard.callSiteIssues.length && !generatedTestsBySymbol.has(graphable + '#contract') && !hasDiskGeneratedKind(r, 'contract')) {
        const contractChip = document.createElement('span');
        contractChip.className = 'actionChip actionChipClickable actionChipPrimary';
        contractChip.textContent = '⚠ Generate contract test(s) with Claude';
        ...
        const fireContractTest = () => {
          if (contractChip.dataset.pending === '1') { return; }
          contractChip.dataset.pending = '1';
          const tickInterval = startClaudeTicker(contractChip, 'Generating');
          const existingNote = testingArea.querySelector(':scope > .generatedTestNote');
          if (existingNote) { existingNote.remove(); }
          ...
          actions.appendChild(stopBtn);
          claudeTickersByRequestId.set(requestId, tickInterval);
          contractTestChipsByRequestId.set(requestId, { chip: contractChip, actionsEl: actions, testingArea, tickInterval, symbolKey: graphable + '#contract', stopBtn });
      ...
      // regular Call Path Test only ever reads one caller. Caller count
      // alone isn't a good enough signal on its own, though — a Low-risk
      // function (e.g. a couple of trivial call sites) can easily clear
      // "2+ callers" while still being genuinely low-value to spend a
      ...
      if (symbolic && r.changeCard && r.changeCard.kind === 'modified' && broadCoverageRiskOk && Array.isArray(r.changeCard.directCallers) && r.changeCard.directCallers.length >= 2 && !generatedTestsBySymbol.has(graphable + '#broadcoverage') && !hasDiskGeneratedKind(r, 'broad_coverage')) {
        ...
        const fireBroadCoverageTest = () => {
          ...
          const tickInterval = startClaudeTicker(broadChip, 'Generating');
          const existingNote = testingArea.querySelector(':scope > .generatedTestNote');
          if (existingNote) { existingNote.remove(); }
          ...
          actions.appendChild(stopBtn);
          claudeTickersByRequestId.set(requestId, tickInterval);
          broadCoverageTestChipsByRequestId.set(requestId, { chip: broadChip, actionsEl: actions, testingArea, tickInterval, symbolKey: graphable + '#broadcoverage', stopBtn });
      ...
      // verdict / existing-tests check — both roll up signals that only
      // exist for an actual changed function. existingEl was built above,
      // before "Check tests," so its click can drive this too.
      if (existingEl) {
        testingArea.appendChild(existingEl);

        if (r.file) { verdictFileBySymbol.set(graphable, String(r.file)); }
        const verdictEl = document.createElement('div');
        verdictCardElsBySymbol.set(graphable, verdictEl);
        renderVerdictCardContent(verdictEl, graphable);
  ...
  function appendResultItems(parent, results) {
    parent.textContent = '';
    // A fresh top-level render (Check Uncommitted Edits / Check Latest
    // Commits / Analyze Commit) is its own batch for the auto-write cap —
    // a big commit shouldn't get fewer auto-writes just because an earlier,
    // unrelated batch already used up the count this session.
    autoWriteBatchCount = 0;
    autoRunBatchCount = 0;
  ...
  // At-a-glance counts (see buildChangesSummaryTiles on the extension host)
  // rendered as tiles instead of a prose bullet list, so the handful of
  // numbers worth knowing before scrolling don't require reading sentences.
  function renderSummaryTiles(parent, tiles) {
    const grid = document.createElement('div');
    grid.className = 'summaryTileGrid';
    tiles.forEach((tile) => {
      const cell = document.createElement('div');
      cell.className = 'summaryTile summaryTile-' + (tile.tone || 'neutral');
      const value = document.createElement('span');
      value.className = 'summaryTileValue';
      value.textContent = String(tile.value);
      const label = document.createElement('span');
      label.className = 'summaryTileLabel';
  ...
  // Seconds only, no "Xm Ys" breakdown — a plain number is faster to read at
  // a glance and matches the per-chip "…Ns" tickers elsewhere in this panel.
  ...
  function renderUsageTileSection(parent, heading, liveNodes, renderValues, resetTitle) {
    ...
    [
      ...
      const cell = document.createElement('div');
      cell.className = 'summaryTile summaryTile-neutral';
      if (tooltip) { cell.title = tooltip; }
      const value = document.createElement('span');
      value.className = 'summaryTileValue';
      const labelEl = document.createElement('span');
      labelEl.className = 'summaryTileLabel';
  ...
  // prompt asks Claude to do (see that function's doc comment on the extension
  // host) — so a user deciding whether to click "Generate Call-Path Test" knows
  // what test they're about to get without reading the prompt itself.
  function describeCallChainTest(chain) {
    const callerTail = String(chain.callerLabel || chain.callerSymbol || '').split('.').pop() || '';
    const calleeTail = String(chain.calleeLabel || chain.calleeSymbol || '').split('.').pop() || '';
    const boundaryNote = chain.crossesModuleBoundary
      ? ' "' + calleeTail + '" is treated as a public contract here (the call crosses a module boundary), not an internal detail.'
      : '';
    return 'Writes one test that calls "' + callerTail + '" the way it is really used, then asserts on an effect that only shows up '
      + 'after it reaches "' + calleeTail + '" — catching an interaction bug a unit test on either function alone would miss.' + boundaryNote;
  }

  // Opens a graph focused on both functions in a call-chain row, reusing the
  // same /search-result-graph flow a single result's "View impact graph"
  // action uses (see runResultGraph on the extension host): the caller is the
  // graph root/highlight, the callee is passed as an impact node so it's
  // pulled into the same subgraph and highlighted too.
  function openCallChainGraph(chain) {
    vscode.postMessage({
      type: 'graphForResult',
      result: {
        graphSymbol: chain.callerSymbol,
        fullName: chain.callerSymbol,
        symbol: chain.callerLabel || chain.callerSymbol,
        name: chain.callerLabel || chain.callerSymbol,
        file: chain.callerFile,
        impactNodes: chain.calleeSymbol ? [chain.calleeSymbol] : [],
        impactFiles: chain.calleeFile ? [chain.calleeFile] : [],
      },
    });
  }
  ...
  function renderCallChainsSection(parent, chains) {
    ...
    table.className = 'callChainTable';
    chains.forEach((chain) => {
      const row = document.createElement('div');
      row.className = 'callChainTableRow';

      const info = document.createElement('div');
      info.className = 'callChainInfo';

      const labelRow = document.createElement('div');
      labelRow.className = 'callChainLabelRow';
      const label = document.createElement('span');
      label.className = 'callChainLabel callChainLabelClickable';
      label.textContent = (chain.callerLabel || chain.callerSymbol) + ' → ' + (chain.calleeLabel || chain.calleeSymbol);
      label.title = 'View both functions in the graph — '
        + (chain.callText ? 'calls it as ' + chain.callText + '(...) ' : '') + 'at ' + chain.file + ':' + chain.line;
      label.addEventListener('click', (event) => {
        event.stopPropagation();
        openCallChainGraph(chain);
      });
      ...
      if (chain.crossesModuleBoundary) {
        ...
        labelRow.appendChild(boundaryBadge);
      }
      info.appendChild(labelRow);

      const description = document.createElement('div');
      description.className = 'callChainDescription';
      description.textContent = describeCallChainTest(chain);
      info.appendChild(description);
      row.appendChild(info);

      const actions = document.createElement('div');
      actions.className = 'callChainActions';

      // The generator's output filename is fully deterministic (see
      // existingCallChainTestFile on the extension host) — if it already
      // exists, lead with opening it instead of burying "a test already
      // covers this" behind another 10-100+s regeneration.
      if (chain.existingTestPath) {
        const existsBadge = document.createElement('span');
        existsBadge.className = 'callChainExistsBadge';
      ...
      const chip = document.createElement('span');
      chip.className = chain.existingTestPath ? 'actionChip actionChipClickable' : 'actionChip actionChipClickable actionChipPrimary';
      chip.textContent = chain.existingTestPath ? 'Regenerate' : 'Generate Call-Path Test';
      ...
      chip.addEventListener('click', (event) => {
        ...
        const tickInterval = startClaudeTicker(chip, 'Generating');
        ...
        attachLiveClaudeOutput(row, requestId);
        const stopBtn = createTestGenStopButton(requestId);
        actions.appendChild(stopBtn);
        claudeTickersByRequestId.set(requestId, tickInterval);
        chainTestChipsByRequestId.set(requestId, { chip, rowEl: row, tickInterval, symbolKey: chain.callerSymbol, stopBtn });
  ...
  function renderSearchResult(msg) {
    ...
    // A query header doubles as a collapse toggle for everything below it in
    // this one result block (summary tiles, the answer, Call-Path
    // Opportunities, the result list itself) — "Check Uncommitted Edits" in
    // particular can return a long batch of change cards with nothing to
    // shrink it down to one line once you're done reading it, short of
    // collapsing every card inside it one at a time. Starts expanded so
    // nothing that used to be visible by default becomes hidden by this
    // change; Call-Path Opportunities keeps its own nested collapse toggle
    ...
    if (msg.error) {
      ...
      if (Array.isArray(msg.summaryTiles) && msg.summaryTiles.length) {
        ...
        renderSessionUsageTiles(summaryBody);
    ...
    if (anchorRow && anchorRow.isConnected) {
      ...
      if (isError) { setGenerateButtonsBusy(false); }
      ...
      // Prompt-Only is hidden from the Modes popup for now (see
      // modePromptOnlyOption's style="display:none;" above) — coerce any
      // already-persisted 'promptOnly' setting down to 'manual' here too, so
      // nobody is stuck on a mode they can no longer pick in the UI.
      claudeAgentMode = msg.claudeAgentMode === 'agent' ? msg.claudeAgentMode : 'manual';
      renderModePills();
    } else if (msg.type === 'autoModeProgress') {
      autoPipelineProgress.style.display = '';
      updateAutoPipelineActiveStep(msg);
      if (msg.done) {
        stopAutoPipelineActiveStep();
        ...
        autoPipelineProgressDetails.textContent = '';
        if (msg.error) {
          autoPipelineProgressText.textContent = '⚠ ' + msg.error;
          autoPipelineProgressToggle.style.display = 'none';
        } else if (Array.isArray(msg.summary) && msg.summary.length) {
          autoPipelineProgressText.textContent = msg.text || 'Auto Mode finished.';
          renderAutoPipelineSummary(msg.summary);
          autoPipelineProgress.classList.remove('is-collapsed');
          autoPipelineProgressToggle.style.display = '';
          autoPipelineProgressToggle.textContent = '-';
          autoPipelineProgressToggle.title = 'Hide individual results';
          autoPipelineProgressToggle.setAttribute('aria-expanded', 'true');
        } else {
          autoPipelineProgressText.textContent = msg.text || 'Auto Mode finished — nothing to do.';
          autoPipelineProgressToggle.style.display = 'none';
        }
      } else {
        autoPipelineProgressText.textContent = (msg.text || 'Auto Mode running…')
          + (typeof msg.claudeCallsUsed === 'number' ? ' · ' + msg.claudeCallsUsed + ' Claude call(s) so far' : '');
      }
      ...
      const estimatedGraphSeconds = Object.values(toolsByClient)
        .reduce((sum, tools) => sum
          ...
          + (Number(tools?.codemd_get_impact_radius || 0) * 30)
          // Overview/discovery calls (search, read, semantic search, find
          // tests, review changes) are cheaper to hand-replicate than a
          // callgraph traversal, but still real time saved over grepping/
          // reading files by hand — weight them lightly rather than zeroing
          // them out. codemd_status is a pure health check, not work, and
          // stays excluded.
          + (Object.entries(tools || {})
              .filter(([toolName]) => overviewTools.has(toolName))
              .reduce((overviewSum, [, calls]) => overviewSum + Number(calls || 0), 0) * 5), 0);
      ...
      mcpUsageCard.title = usageTitle + ' Conservative estimate: callers/callees = 15s, call paths/impact radius = 30s, search/read/semantic-search/find-tests/review-changes = 5s. Status calls excluded. Not directly measured.';
      ...
      if (count > 0 && matches.length && testingArea) {
        ...
        matches.forEach((match) => {
          ...
          const kindLabel = (match && typeof match === 'object' && match.kindLabel) ? String(match.kindLabel) : 'Test';
          const testNames = (match && Array.isArray(match.testNames)) ? match.testNames : [];
          const group = document.createElement('div');
          group.className = 'testMatchGroup testCard';

          // Category first (what kind of evidence this is, ranked by
          // importance server-side) — the raw generated-tests file path is
          // real but means nothing to skim, so it's a click-to-open detail
          // under the label rather than the heading itself.
          const kindEl = document.createElement('div');
          kindEl.className = 'testMatchKind';
          kindEl.textContent = kindLabel;
          group.appendChild(kindEl);
        ...
        if (testingArea) {
          ...
          // The mechanical chip (deletion-report.py's literal-replay path)
          // only understands Python — shown up front there since it's free,
          // instant, and requires no AI call, with the Claude fallback
          // revealed only once it actually reports it can't help (its real
          // callers all pass a variable/computed value instead of a
          // literal), so that failure message explains why the fallback
          // just appeared instead of dumping both options at once. Every
          // other language has no mechanical replay generator, so it goes
          // straight to the Claude fallback (generateCallPathTestForResult
          // on the host) — deliberately NOT gated by a client-side
          // extension check here: the host's own testLanguageFor is already
          // the single source of truth for which languages are supported
          // (Python, JS/TS, Java, C#, Go, Rust, Kotlin as of this writing),
          // and it reports a clear error if this ever gets clicked for a
          // genuinely unsupported one (C++, PHP, ...) rather than this list
          // silently drifting out of sync with the host's own list, which is
          // exactly what happened here before: this used to hardcode a
          // JS/TS-only regex, so Java/C#/Go/Rust/Kotlin files got no
          // generate-test affordance at all despite the host fully
          // supporting them.
          const gapFile = String(gapResult && gapResult.file || '');
          if (isNewFunction) {
            // A new function already gets its own always-visible "🆕
            // Generate test for new function" chip in the result's own
            // action row (see the r.changeCard.kind === 'added' branch
            // above) — that chip calls Claude the same way the generic
            // fallback below would, from a prompt that is effectively the
            // same request. Adding a second, differently-labeled
            // Claude-calling button here duplicated that exact job with no
            // real distinction (neither is "mechanical" — both are Claude),
            // which just left two buttons doing the same thing side by
            ...
            mechChip.textContent = 'Generate Test with CODEMD (free)';
      ...
      if (msg.sentToTerminal) {
        statusPill.textContent = msg.ok ? '📋 Sent to terminal' : '⚠ Couldn\\'t send';
        statusPill.className = 'statusPill is-warn';
        statusPill.style.display = '';
        statusDetail.textContent = msg.ok
          ? 'Prompt sent to your Claude Code terminal — review it there, then press Enter to run it. CODEMD will not know the result automatically.'
          : (msg.error || 'Could not send the prompt to your Claude Code terminal.');
        statusDetail.className = 'statusDetail';
      ...
      if (!msg.ok) {
        ...
        if (fixBtn) {
          ...
          // includes "install a missing dependency", which for this specific
          // failure would mean freelancing an entire test framework into the
          // repo, exactly what CODEMD is deliberately not doing here. Hide
          // the button (setupBtn above offers the real, deterministic fix
          // instead) rather than let one more click reopen that loophole.
          fixBtn.style.display = msg.noTestRunner ? 'none' : '';
      ...
      // own cold-start flake, retried once above, is the last thing worth
      // re-triggering on purpose). Direct (local, free) runners keep the
      // button, and a failed run of either kind keeps it too so retrying is
      // still one click away.
      if (runBtn) {
        runBtn.style.display = (msg.passed && msg.viaClaude) ? 'none' : '';
      }
      ...
      statusPill.style.display = '';
      let detailText = '';
      // Three real tiers here, not two: confirmed-executed is the strongest
      // signal this UI can show (coverage.py proved the target ran) but used
      // to render with the exact same dim, low-opacity styling as "best
      // effort — can't tell either way," so the one result worth trusting
      // most looked no more important than the one that says nothing. Now
      // 'good' (confirmed executed) and 'warn' (confirmed did NOT execute)
      // both get full-opacity, colored treatment; only the genuinely
      // inconclusive "best effort" case stays dim.
      let detailTone = '';
      ...
      statusDetail.style.display = '';
      if (symbolKey) {
        testRunsBySymbol.set(symbolKey, {
          passed: !!msg.passed,
          coverageConfirmed: !!msg.coverageConfirmed,
          executed: !!msg.executed,
          executedInRange: msg.executedInRange,
          trackedInRange: msg.trackedInRange,
        });
      ...
      if (!entry) { return; }
      const { setupBtn, runBtn, statusDetail, tickInterval } = entry;
      if (tickInterval) { clearInterval(tickInterval); }
      setupBtn.dataset.pending = '0';
      if (msg.ok) {
        // Leave the button visible but done — "Run again" below is the next
        // real step, and re-clicking Set up would just no-op against
        // detectJsTestRunner now finding what this just installed.
        setupBtn.textContent = msg.alreadyPresent ? '✓ Already set up' : ('✓ ' + (msg.runner || 'Test runner') + ' installed');
        setupBtn.className = 'btn is-outline';
        statusDetail.textContent = msg.summary || 'Done — click "Run again" to try the test now.';
        statusDetail.className = 'statusDetail';
        statusDetail.style.display = '';
      } else {
        setupBtn.textContent = '⚙ Set up test runner (free)';
        statusDetail.textContent = msg.error || 'Could not set up a test runner.';
        statusDetail.className = 'statusDetail is-notable';
        statusDetail.style.display = '';
      }
      if (runBtn) {
        // This whole flow only ever exists for the "no JS/TS runner
        // detected" case (see runTestViaClaudeForResult) — running the test
        // it just set up always means Claude first, same as the row's own
        // initial '🤖 Ask Claude to run'.
        runBtn.textContent = '🤖 Ask Claude to run again';
        runBtn.className = 'btn is-outline';
      }
    } else if (msg.type === 'fixTestProgress') {
      // First live chunk for this requestId = Claude has actually started
      // (see claudeTickersByRequestId/startClaudeTicker) — this is the
      // earliest point the visible "Fixing… Ns" counter should start moving.
      ...
      if (msg.sentToTerminal) {
        ...
        renderFixBanner(fixNote, {
          tone: msg.ok ? 'warn' : 'bad',
          icon: '📋',
          lede: msg.ok ? 'Prompt sent to your Claude Code terminal' : 'Could not send prompt to terminal',
          detail: msg.ok
            ? ' — review it there, then press Enter to run it. CODEMD will not know the result automatically.'
      ...
      if (statusPill && passed !== null) {
        ...
        if (runBtn) {
          // Claude's own rerun (inside this same fix flow) just gave a real
          // pass/fail verdict — for a true "passed" that already IS the
          // confirmation, so a "Run again" button here only invites a
          // redundant, and for a Claude-discovered command a paid,
          // re-verification of something already verified. It also risks
          // re-triggering the Vitest/Vite cold-start race (see the retry in
          // runTestViaClaudeForResult) and reading as "it broke again" when
          // nothing did. A false/null "passed" still gets the button back
          // since there's real work left to retry.
          if (passed === true) {
        ...
        if (resolveBtn) { resolveBtn.style.display = passed === true ? '' : 'none'; }
      ...
      if (filesChanged.length) {
        ...
        renderFixBanner(fixNote, {
          ...
          icon,
          lede,
          // "Click Run again to confirm" only makes sense when there's
          // something left to confirm — passed === true already came from
          // Claude's own rerun above (and the button itself is now hidden
          // for that case, see above), so repeating the CTA here would tell
          // the user to do a thing that both already happened and isn't offered.
          detail: verdict + '. ' + (msg.summary || '') + (passed === true ? '' : ' Click "Run again" to confirm.') + durationSuffix,
      ...
      if (tickInterval) { clearInterval(tickInterval); }
      chip.dataset.pending = '0';
      chip.textContent = 'Generate Test with CODEMD (free)';
      const opts = {
        source: 'mechanical',
        ok: msg.ok,
      ...
      removeTestGenStopButton(entry);
      chip.dataset.pending = '0';
      // Unlike its three siblings (new-function/contract/broad-coverage,
      // all routed through resolveChipTestResult's hideOnSuccess), this
      // handler used to always reset the label back to '🤖 Generate Test
      // with Claude' even after a successful write — leaving a completed,
      // still-clickable (paid) button sitting right next to its own "Test
      // written" note below. A test now exists, so remove it the same way
      // the other three already do; only put the label back for a real
      // failure, where retrying still makes sense.
      if (msg.ok && msg.path) {
        chip.remove();
      } else {
        chip.textContent = '🤖 Generate Test with Claude';
      }
      const opts = {
        source: 'claude',
        ok: msg.ok,
      ...
      existingTestsRequestsByRequestId.delete(msg.requestId);
      if (!entry) { return; }
      if (msg.ok) {
        const matches = Array.isArray(msg.matches) ? msg.matches : [];
        generatedTestsBySymbol.set(entry.symbol + '#existing', { matches });
        // Feed a real pass into testRunsBySymbol too — computeVerdictForSymbol
        // only reaches HIGH confidence with both a test AND a recorded run,
        // and this check already ran pytest for real (unlike the manual "Run"
        // button, nothing else populates testRunsBySymbol for it otherwise).
        const confirmedMatch = matches.find((m) => m.ok && m.passed && m.coverageConfirmed && m.executed);
        if (confirmedMatch) {
          testRunsBySymbol.set(entry.symbol, {
            passed: true,
            coverageConfirmed: true,
            executed: true,
            executedInRange: confirmedMatch.executedInRange,
            trackedInRange: confirmedMatch.trackedInRange,
          });
        }
      ...
      // auto-write forever. See maybeAutoWriteTest.
      const queuedAutoWrites = pendingAutoWrites.get(entry.symbol);
      if (queuedAutoWrites) {
        pendingAutoWrites.delete(entry.symbol);
        queuedAutoWrites.forEach((fn) => fn());
      }
    } else if (msg.type === 'newFunctionTestResult') {
      resolveChipTestResult(newFunctionTestChipsByRequestId, msg, '🆕 Generate test for new function', null, true);
    } else if (msg.type === 'callChainTestResult') {
      resolveChipTestResult(chainTestChipsByRequestId, msg, 'Generate Call-Path Test', 'Regenerate');
    } else if (msg.type === 'contractTestResult') {
      resolveChipTestResult(contractTestChipsByRequestId, msg, '⚠ Generate contract test(s)', null, true);
    } else if (msg.type === 'broadCoverageTestResult') {
      resolveChipTestResult(broadCoverageTestChipsByRequestId, msg, '🔀 Generate Multi-Caller Test(s)', null, true);
      ...
      defectCounts = msg.counts && typeof msg.counts === 'object'
        ? {
          active: Number(msg.counts.active) || 0,
          fixed: Number(msg.counts.fixed) || 0,
          fixedCode: Number(msg.counts.fixedCode) || 0,
          fixedTest: Number(msg.counts.fixedTest) || 0,
        }
        : { active: 0, fixed: 0, fixedCode: 0, fixedTest: 0 };
      // The title itself carries the tally (Fixed: N, Active: M) so that's
      // visible even while the card is collapsed. The body below lists every
      // bug, active or fixed, code or test kind alike, sorted with active
      // ones on top; each fixed row is tagged with its kind badge (see
      // createDefectRow) so it's clear whether the generated test caught a
      // real bug in the code or just had a bad assumption of its own. The
      // code/test split in the title answers "how many of these were real
      // bugs versus just a bad assumption in the generated test itself."
      const fixedBreakdown = (defectCounts.fixedCode || defectCounts.fixedTest)
        ? ' — ' + defectCounts.fixedCode + ' code, ' + defectCounts.fixedTest + ' test'
        : '';
      defectsLabel.textContent = 'Bugs, Issues & Defects (Fixed: ' + defectCounts.fixed + fixedBreakdown + ', Active: ' + defectCounts.active + ')';
      const items = Array.isArray(msg.items) ? msg.items : [];
      if (!items.length) {
